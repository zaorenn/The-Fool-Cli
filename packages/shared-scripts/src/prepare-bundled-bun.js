...
/**
 * Prepare bundled bun runtime for packaging.
 ...
 * @param {string} [options.version] - Bun runtime version (default: 'latest')
 * @returns {{ prepared: boolean; dir?: string; files?: string[]; sourceType?: string; reason?: string }}
 */
function prepareBundledBun(options = {}) {
  const projectRoot = options.projectRoot || path.resolve(__dirname, '..', '..', '..');
  const platform = options.platform || process.platform;
  const arch = options.arch || process.env.npm_config_target_arch || process.arch;
  const runtimeKey = `${platform}-${arch}`;
  const runtimeVersion = options.version || getRuntimeVersion();
