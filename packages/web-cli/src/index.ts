import { startWebHost, startStaticServer } from '@fool/web-host';
import type { WebHostHandle, StaticServerHandle } from '@fool/web-host';
...
// tarball layout:
//   fool-web/
//   ├── fool-web              ← bun-compiled standalone binary (process.execPath)
//   ├── package.json             ← for runtime version lookup
//   ├── bundled-foolcore/<plat-arch>/foolcore[.exe]
//   └── static/                  ← SPA assets
//
...
const isPackaged = (() => {
  ...
  return exeName === 'fool-web' || exeName === 'fool-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'foolcore.exe' : 'foolcore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'fool-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.FOOL_BACKEND_BIN;
  if (envOverride) return path.resolve(envOverride);
  const platArch = `${process.platform}-${process.arch}`;
  const bundled = path.join(cliRoot, 'bundled-foolcore', platArch, BACKEND_BINARY);
  return bundled;
}
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.FOOL_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.fool-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.FOOL_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.FOOL_ALLOW_REMOTE ?? process.env.FOOL_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[fool-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[fool-web] version    : ${version}`);
  console.log(`[fool-web] data dir   : ${dataDir}`);
  console.log(`[fool-web] log dir    : ${logDir}`);
  console.log(`[fool-web] static dir : ${staticDir}`);
  console.log(`[fool-web] backend bin: ${backendBin}`);
  console.log(`[fool-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download foolcore and set FOOL_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('The Fool WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[fool-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[fool-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[fool-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[fool-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `fool-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[fool-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set FOOL_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[fool-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[fool-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[fool-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[fool-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[fool-web] username: ${username}`);
    console.log(`[fool-web] new password: ${newPassword}`);
    console.log('[fool-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: fool-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.fool-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.fool-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  FOOL_PORT, FOOL_ALLOW_REMOTE, FOOL_DATA_DIR, FOOL_LOG_DIR,
  FOOL_BACKEND_BIN, FOOL_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: fool-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[fool-web] fatal:', err.message);
