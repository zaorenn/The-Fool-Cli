...
// Read the real The Fool version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@fool/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@fool/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@fool/web-host'] }),
        ...(isDevelopment
          ? [
              {
      ...
      build: {
        ...
        rollupOptions: {
          input: {
            ...
            voiceCaptionPreload: resolve('packages/desktop/src/preload/voiceCaptionPreload.ts'),
          },
        },
      },
    },
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another The Fool clone is running),
      ...
      build: {
        ...
        rollupOptions: {
          input: {
            ...
            'voice-caption': resolve(rendererRoot, 'voice/caption.html'),
          },
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.FOOL_MULTI_INSTANCE': JSON.stringify(process.env.FOOL_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real The Fool version (root package.json) so renderer code
