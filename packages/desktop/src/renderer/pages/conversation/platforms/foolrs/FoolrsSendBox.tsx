...
import { useFoolrsMessage } from './useFoolrsMessage';
...
const useFoolrsSendBoxDraft = getSendBoxDraftHook('foolrs', {
  _type: 'foolrs',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = useFoolrsSendBoxDraft(conversation_id);
...
const FoolrsSendBox: React.FC<{
  conversation_id: string;
  modelSelection: FoolrsModelSelection;
  session_mode?: string;
  agent_name?: string;
  ...
  const { thought, running, turnStartedAtMs, setActiveMsgId, setWaitingResponse, resetState } = useFoolrsMessage(
    conversation_id,
    {
    ...
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        if (teamSendMessage) {
          ...
          if (files.length > 0) {
            emitter.emit('foolrs.workspace.refresh');
          }
          return;
        }
  ...
  useEffect(() => {
    ...
    const storageKey = `foolrs_initial_message_${conversation_id}`;
    const processedKey = `foolrs_initial_processed_${conversation_id}`;
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[FoolrsSendBox] Failed to send initial message:', error);
  ...
  const onSendHandler = async (message: string) => {
    ...
    clearFiles();
    emitter.emit('foolrs.selected.file.clear');
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[FoolrsSendBox] Failed to switch mode via sheet:', error);
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[FoolrsSendBox] stop request failed', error);
    ...
    <div className={`${sendBoxWidthClass} flex flex-col mt-auto mb-16px`}>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('foolrs.selected.file', items, conversation_id);
          setAtPath(items);
        }}
        ...
        placeholder={
          current_model?.use_model
            ? t('acp.sendbox.placeholder', {
                backend: agent_name || 'The Fool CLI',
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`foolrs-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`foolrs-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
                          const newAtPath = atPath.filter((v) => (typeof v === 'string' ? true : v.path !== item.path));
                          emitter.emit('foolrs.selected.file', newAtPath, conversation_id);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
