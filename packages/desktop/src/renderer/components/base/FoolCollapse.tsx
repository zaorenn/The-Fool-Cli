 ...
 */
export interface FoolCollapseProps {
 ...
 */
export interface FoolCollapseItemProps {
 ...
 */
const FoolCollapseItem: React.FC<FoolCollapseItemProps> = ({ children }) => <>{children}</>;
FoolCollapseItem.displayName = 'FoolCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <FoolCollapse defaultActiveKey={['1']}>
 *   <FoolCollapse.Item name="1" header="面板1">
 *     内容1
 *   </FoolCollapse.Item>
 *   <FoolCollapse.Item name="2" header="面板2">
 *     内容2
 *   </FoolCollapse.Item>
 * </FoolCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <FoolCollapse accordion defaultActiveKey="1">
 *   <FoolCollapse.Item name="1" header="面板1">内容1</FoolCollapse.Item>
 *   <FoolCollapse.Item name="2" header="面板2">内容2</FoolCollapse.Item>
 * </FoolCollapse>
 *
 * // 自定义图标 / Custom icon
 * <FoolCollapse
 ...
 * ```
 */
const FoolCollapseComponent: React.FC<FoolCollapseProps> & { Item: typeof FoolCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<FoolCollapseItemProps> => {
      return React.isValidElement(child) && child.type === FoolCollapseItem;
    });
  }, [children]);
...
FoolCollapseComponent.Item = FoolCollapseItem;

export default FoolCollapseComponent;
