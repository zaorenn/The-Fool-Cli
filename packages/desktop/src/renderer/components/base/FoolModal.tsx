...
/** FoolModal 组件 Props */
export interface FoolModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'fool-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'fool-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <FoolModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </FoolModal>
 *
 * // 预设尺寸 / Preset size
 * <FoolModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </FoolModal>
 *
...
const FoolModal: React.FC<FoolModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'fool-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('fool-modal', isStandard && 'fool-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('fool-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'fool-modal-body-content',
...
FoolModal.displayName = 'FoolModal';

export default FoolModal;
