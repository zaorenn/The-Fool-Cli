 ...
 * Legacy backend/custom-agent migration is handled by foolCore during channel
...
export function getDefaultChannelAssistant(assistants: Assistant[]): Assistant | undefined {
  return (
    assistants.find((assistant) => assistant.source === 'generated' && isFoolrsAssistant(assistant)) ||
    assistants.find((assistant) => isFoolrsAssistant(assistant)) ||
    assistants[0]
  );
}
