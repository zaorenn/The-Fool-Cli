...
import { FOOL_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理The Fool时间戳后缀，返回原始文件名
export function cleanFoolTimestamp(file_name: string): string {
  return file_name.replace(FOOL_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanFoolTimestamp(file_name);
}
...
class FileServiceClass {
   ...
   * directory (`temp_dir/fool/...`). Passing the raw device path (the old
