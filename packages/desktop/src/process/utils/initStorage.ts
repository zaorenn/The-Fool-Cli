...
const STORAGE_PATH = {
  config: 'fool-config.txt',
  chatMessage: 'fool-chat-message.txt',
  chat: 'fool-chat.txt',
  env: '.fool-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[The Fool] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[The Fool] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[The Fool] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('fool.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'fool-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'fool-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'fool-chat-history', conversation_id + '.txt'));
};
...
const conversationHistoryProxy = (options: typeof _chatMessageFile, dir: string) => {
  return {
    ...
    backup(conversation_id: string) {
      const storage = buildMessageListStorage(conversation_id, dir);
      return storage.backup(path.join(dir, 'fool-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt'));
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[The Fool] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/fool-app/assets/builtin-assistants/` and
 * `crates/fool-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[The Fool:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see fool-assistant crate) and
  ...
  try {
    ...
    console.error('[The Fool] Failed to ensure assistant dirs:', error);
  }
