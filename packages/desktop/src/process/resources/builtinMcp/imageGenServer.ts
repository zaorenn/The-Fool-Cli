...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.FOOL_IMG_PLATFORM;
  const base_url = process.env.FOOL_IMG_BASE_URL;
  const api_key = process.env.FOOL_IMG_API_KEY;
  const model = process.env.FOOL_IMG_MODEL;
