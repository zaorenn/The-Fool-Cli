!ifndef FOOL_INSTALLER_PROCESS_CONTROL_NSH
!define FOOL_INSTALLER_PROCESS_CONTROL_NSH

Var /GLOBAL FoolStopResult
Var /GLOBAL FoolLockerResult
Var /GLOBAL FoolLockerList
Var /GLOBAL FoolLockerListZh
Var /GLOBAL FoolLockerListEn
Var /GLOBAL FoolLockerListFile
Var /GLOBAL FoolCurrentOutDir

!macro FOOL_FIND_APP_PROCESS _RETURN
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
    ...
    function Test-FoolOwnedProcess($$proc) { \
    ...
    $$hits = @(Get-CimInstance -ClassName Win32_Process | Where-Object { Test-FoolOwnedProcess $$_ }); \
    $$payload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'process-find'; ownedPrefix = $$ownedPrefix; installerPid = $$installerPid; hits = $$hits.Count; owned = ($$hits.Count -gt 0) }; \
    Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$payload | ConvertTo-Json -Compress -Depth 8); \
    if ($$hits.Count -gt 0) { $$hitPayload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'process-find-hits'; processes = @($$hits | Select-Object ProcessId,ParentProcessId,Name,ExecutablePath,Path,CommandLine) }; Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$hitPayload | ConvertTo-Json -Compress -Depth 10); exit 0 } \
    exit 1 \
  }"`
  Pop ${_RETURN}
!macroend

!macro FOOL_STOP_APP_PROCESSES
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
    $$ErrorActionPreference = 'SilentlyContinue'; \
    $$log = '$FoolSessionLogPath'; \
    if (-not $$log) { $$log = Join-Path $$env:TEMP '${FOOL_FALLBACK_LOG}' }; \
    ...
    $$owned = @($$all | Where-Object { Test-FoolOwnedProcess $$_ }); \
    $$ids = @($$owned | ForEach-Object { [int]$$_.ProcessId }); \
    $$frontier = @($$ids); \
    while ($$frontier.Count -gt 0) { \
      $$children = @($$all | Where-Object { $$frontier -contains [int]$$_.ParentProcessId -and [int]$$_.ProcessId -ne [int]$$installerPid } | Where-Object { Test-FoolOwnedProcess $$_ }); \
      $$childIds = @($$children | ForEach-Object { [int]$$_.ProcessId }); \
      $$ids = @($$ids + $$childIds | Select-Object -Unique); \
      $$frontier = $$childIds; \
    } \
    $$payload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'process-stop'; ids = @($$ids); result = 'start' }; \
    Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$payload | ConvertTo-Json -Compress -Depth 8); \
    foreach ($$id in ($$ids | Sort-Object -Descending)) { Stop-Process -Id $$id -Force -ErrorAction SilentlyContinue } \
    $$payload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'process-stop'; ids = @($$ids); result = 'done' }; \
    Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$payload | ConvertTo-Json -Compress -Depth 8); \
    exit 0 \
  }"`
  Pop $FoolStopResult
...
!macro FOOL_QUERY_LOCKERS_INLINE_LEGACY _TARGET_PATH _RETURN
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
    $$ErrorActionPreference = 'SilentlyContinue'; \
    $$log = '$FoolSessionLogPath'; \
    if (-not $$log) { $$log = Join-Path $$env:TEMP '${FOOL_FALLBACK_LOG}' }; \
    ...
    $$currentOutDir = '$FoolCurrentOutDir'; \
    $$lockerListPath = '$PLUGINSDIR\fool-rm-lockers.txt'; \
    [System.IO.File]::WriteAllText($$lockerListPath, '', (New-Object System.Text.UTF8Encoding $$false)); \
    try { \
    function Test-FoolSamePath($$left, $$right) { \
    ...
    $$installerSelfLock = (Test-FoolSamePath $$currentOutDir $$targetPath) -or (Test-FoolSamePath $$currentOutDir $$instDir); \
      ...
      $$result = [The Fool.RestartManager.Native]::RmStartSession([ref]$$session, 0, $$key); \
      ...
      try { \
        ...
        if ($$targetPath -and (Test-Path -LiteralPath $$targetPath -PathType Leaf)) { \
          ...
          $$knownRelative = @('${FOOL_APP_EXECUTABLE_FILENAME}', '${UNINSTALL_FILENAME}', 'resources\app.asar', 'resources\app-update.yml', 'resources\bundled-foolcore\win32-x64\foolcore.exe'); \
        ...
        if ($$resources.Count -eq 0) { \
          if ($$installerSelfLock -and $$installerPid -gt 0) { \
            $$lockerText = 'The Fool installer(' + $$installerPid + ')'; \
            [System.IO.File]::WriteAllText($$lockerListPath, $$lockerText, (New-Object System.Text.UTF8Encoding $$false)); \
            $$selfLockers = @([pscustomobject]@{ name = 'The Fool installer'; pid = [int]$$installerPid }); \
        ...
        for ($$i = 0; $$i -lt $$resources.Count; $$i += 256) { \
          ...
          $$result = [The Fool.RestartManager.Native]::RmRegisterResources($$session, [uint32]$$chunk.Count, $$chunk, 0, [IntPtr]::Zero, 0, $$null); \
        ...
        for ($$attempt = 0; $$attempt -lt 6; $$attempt++) { \
          ...
          $$needed = [uint32]0; $$count = [uint32]0; $$reasons = [uint32]0; \
          $$result = [The Fool.RestartManager.Native]::RmGetList($$session, [ref]$$needed, [ref]$$count, $$null, [ref]$$reasons); \
        ...
        if ($$result -eq $$ERROR_MORE_DATA -or $$needed -gt 0) { \
          for ($$attempt = 0; $$attempt -lt 6; $$attempt++) { \
            ...
            $$result = [The Fool.RestartManager.Native]::RmGetList($$session, [ref]$$needed, [ref]$$count, $$apps, [ref]$$reasons); \
        ...
        if ($$lockers.Count -eq 0 -and $$installerSelfLock -and $$installerPid -gt 0) { $$lockers = @([pscustomobject]@{ name = 'The Fool installer'; pid = [int]$$installerPid }) }; \
      ...
      } finally { [void][The Fool.RestartManager.Native]::RmEndSession($$session) } \
    } catch { \
      $$payload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'rm-error'; target = $$targetPath; error = $$_.Exception.Message }; \
      Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$payload | ConvertTo-Json -Compress -Depth 8); \
      exit 1 \
    } \
  }"`
...
!macro FOOL_QUERY_LOCKERS _TARGET_PATH _RETURN
  InitPluginsDir
  File /oname=$PLUGINSDIR\fool-query-lockers.ps1 "${PROJECT_DIR}\resources\windows\support\query-lockers.ps1"
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "$PLUGINSDIR\fool-query-lockers.ps1" -LogPath "$FoolSessionLogPath" -InstDir "$INSTDIR" -TargetPath "${_TARGET_PATH}" -LockerListPath "$PLUGINSDIR\fool-rm-lockers.txt" -Session "$FoolSessionId" -Version "${VERSION}" -Arch "${FOOL_TARGET_ARCH}" -Updated "$FoolIsUpdated" -CurrentOutDir "$FoolCurrentOutDir"`
  Pop ${_RETURN}
!macroend

!macro FOOL_CAPTURE_FAILED_PATH_LOCKERS _FAILED_PATH
  !insertmacro FOOL_QUERY_LOCKERS "${_FAILED_PATH}" $FoolLockerResult
  ...
  ${If} $FoolLockerList == ""
    ...
    ${Else}
      StrCpy $FoolLockerList "${FOOL_MSG_LOCKER_UNKNOWN_EN}"
      StrCpy $FoolLockerListZh "${FOOL_MSG_LOCKER_UNKNOWN_ZH}"
      StrCpy $FoolLockerListEn "${FOOL_MSG_LOCKER_UNKNOWN_EN}"
    ${EndIf}
  ${Else}
    ...
    ${If} ${Silent}
      nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
        ...
        try { \
          ...
          $$logPayload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'marker-write'; result = 'ok'; path = $$marker; marker = $$payload }; \
          Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$logPayload | ConvertTo-Json -Compress -Depth 8) \
        } catch { \
          $$logPayload = [ordered]@{ schemaVersion = 1; ts = (Get-Date -Format o); session = '$FoolSessionId'; version = '${VERSION}'; arch = '${FOOL_TARGET_ARCH}'; updated = ('$FoolIsUpdated' -eq '1'); instDir = '$INSTDIR'; event = 'marker-write'; result = 'failed'; path = $$marker; error = $$_.Exception.Message }; \
          Add-Content -LiteralPath $$log -Encoding UTF8 -Value ($$logPayload | ConvertTo-Json -Compress -Depth 8) \
        } \
      }"`
      Pop $9
...
!macro customCheckAppRunning
  Var /GLOBAL FoolCheckResult
  Var /GLOBAL FoolCloseRetries
  InitPluginsDir
  !insertmacro FOOL_SESSION_BEGIN

  !insertmacro FOOL_WAIT_FOR_UPDATED_APP_EXIT
  !insertmacro FOOL_FIND_APP_PROCESS $FoolCheckResult
  ${If} $FoolCheckResult == 0
    MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "$(appRunning)" /SD IDOK IDOK fool_do_stop_process
    !insertmacro FOOL_CLEAR_ACTIVE_INSTALLER_MARKER
    Quit

    fool_do_stop_process:
      DetailPrint "$(appClosing)"
      !insertmacro FOOL_STOP_APP_PROCESSES
      StrCpy $FoolCloseRetries 0

    fool_wait_for_close:
      Sleep 1000
      !insertmacro FOOL_FIND_APP_PROCESS $FoolCheckResult
      ${If} $FoolCheckResult == 0
        IntOp $FoolCloseRetries $FoolCloseRetries + 1
        ${If} $FoolCloseRetries > 10
          MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "${FOOL_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH}$\r$\n$\r$\n${FOOL_MSG_MAY_USE_INSTALL_DIR_ZH}$\r$\n$INSTDIR$\r$\n$\r$\n${FOOL_MSG_RETRY_AFTER_CLOSING_DIR_ZH}$\r$\n$\r$\n${FOOL_MSG_BLOCK_SEPARATOR}$\r$\n$\r$\n${FOOL_MSG_CLOSE_OR_REMOVE_PREVIOUS_EN}$\r$\n$\r$\n${FOOL_MSG_MAY_USE_INSTALL_DIR_EN}$\r$\n$INSTDIR$\r$\n$\r$\n${FOOL_MSG_RETRY_AFTER_CLOSING_DIR_EN}" /SD IDCANCEL IDRETRY fool_wait_for_close
          !insertmacro FOOL_WRITE_INSTALLER_LAST_FAILURE_MARKER
          !insertmacro FOOL_FAIL_REPORTABLE_BILINGUAL_DIAGNOSTICS ${FOOL_E_INSTALL_DIR_REMOVE_OR_LOCKED} "event=session-end result=fail code=${FOOL_E_INSTALL_DIR_REMOVE_OR_LOCKED} phase=app-cannot-be-closed retryCount=$FoolCloseRetries instDir=$INSTDIR" "${FOOL_MSG_CLOSE_OR_REMOVE_PREVIOUS_EN}" "${FOOL_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH}" "${FOOL_MSG_CLOSE_INSTALL_DIR_ACTION_EN}" "${FOOL_MSG_CLOSE_INSTALL_DIR_ACTION_ZH}" "app-cannot-be-closed retryCount=$FoolCloseRetries instDir=$INSTDIR" "app-cannot-be-closed retryCount=$FoolCloseRetries instDir=$INSTDIR"
        ${Else}
          !insertmacro FOOL_STOP_APP_PROCESSES
          Goto fool_wait_for_close
        ${EndIf}
      ${EndIf}
  ${EndIf}
