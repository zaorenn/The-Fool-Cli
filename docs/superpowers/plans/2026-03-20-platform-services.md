...
describe('platformRegistry', () => {
  ...
  it('getPlatformServices throws before registration', async () => {
    ...
    expect(() => getPlatformServices()).toThrow(
      '[Platform] Services not registered',
    )
  })

  it('getPlatformServices returns the registered instance', async () => {
    const { registerPlatformServices, getPlatformServices } = await import(
      '../../../src/common/platform/index'
    )
    const mock = {
      paths: {},
      worker: {},
  ...
  it('re-registering replaces the previous instance', async () => {
    ...
    const first = { paths: {}, worker: {}, power: {}, notification: {} } as Parameters<typeof registerPlatformServices>[0]
    const second = { paths: {}, worker: {}, power: {}, notification: {} } as Parameters<typeof registerPlatformServices>[0]
...
describe('NodePlatformServices.paths', () => {
  ...
  it('getDataDir falls back to tmpdir/aionui-user-data', async () => {
    const { NodePlatformServices } = await import('../../../src/common/platform/NodePlatformServices')
    expect(new NodePlatformServices().paths.getDataDir()).toBe(
      path.join(os.tmpdir(), 'aionui-user-data'),
    )
  })
...
describe('NodePlatformServices.notification', () => {
  it('send is a no-op and does not throw', async () => {
    const { NodePlatformServices } = await import('../../../src/common/platform/NodePlatformServices')
    expect(() =>
      new NodePlatformServices().notification.send({ title: 'T', body: 'B' }),
    ).not.toThrow()
  })
})
```
...
export class ElectronPlatformServices implements IPlatformServices {
  paths = {
    ...
    getSystemPath: (name: 'desktop' | 'home' | 'downloads') =>
      app.getPath(name),
    getName: () => app.getName(),
