# The Fool WebUI Mode - Startup Guide

The Fool supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts The Fool with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use The Fool from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\The Fool\The Fool.exe" --webui

# Or if The Fool is in your PATH
The Fool.exe --webui
```
...
```bash
# Using full path
/Applications/The Fool.app/Contents/MacOS/The Fool --webui

# Or using open command
open -a The Fool --args --webui
```

### Method 2: Create Shell Script

Create `start-fool-webui.sh`:
...
chmod +x The Fool-*.AppImage

# Run with --webui flag
./The Fool-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/fool-webui.desktop`:

```ini
[Desktop Entry]
Name=The Fool WebUI
Comment=Start The Fool in WebUI mode
Exec=/opt/The Fool/fool --webui
Icon=fool
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/fool-webui.desktop
```
...
Create `~/bin/start-fool-webui.sh`:
...
```bash
chmod +x ~/bin/start-fool-webui.sh
```
...
Create `/etc/systemd/system/fool-webui.service`:

```ini
[Unit]
Description=The Fool WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/The Fool/fool --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable fool-webui.service
sudo systemctl start fool-webui.service

# Check status
sudo systemctl status fool-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run The Fool in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making The Fool accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running The Fool WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/zaorenn/The-Fool-Cli/issues/217)
...
#### 4. Download and Install The Fool

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/zaorenn/The-Fool-Cli/releases
wget https://github.com/zaorenn/The-Fool-Cli/releases/download/vVERSION/TheFool_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/zaorenn/The-Fool-Cli/releases/download/vVERSION/TheFool_VERSION_arm64.deb

# Install the package
apt install -y ./Fool_*.deb

# Verify installation
which The Fool
```

#### 5. Launch The Fool WebUI

```bash
# Start The Fool in WebUI mode with no-sandbox flag
The Fool --no-sandbox --webui
```
...
To access The Fool from other devices on your local network:

```bash
# Start with --remote flag
The Fool --no-sandbox --webui --remote
...
The Fool --no-sandbox --webui --port 8080
```
...
chmod +x /opt/The Fool/fool
```

#### Out of Memory

The Fool requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if The Fool is running: look for "Server started" message
...
4. **Keep device charged** - Running The Fool consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **The Fool Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch The Fool quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-fool.sh << 'EOF'
#!/bin/bash
echo "Starting The Fool WebUI..."
The Fool --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-fool.sh
...
```bash
proot-distro login ubuntu -- bash -c "The Fool --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/zaorenn/The-Fool-Cli/issues/217)
...
```cmd
The Fool.exe --webui --remote
```

### macOS

```bash
/Applications/The Fool.app/Contents/MacOS/The Fool --webui --remote
```
...
netsh advfirewall firewall add rule name="The Fool WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add The Fool
...
```powershell
& "C:\Program Files\The Fool\The Fool.exe" --webui 2>&1 | Tee-Object -FilePath fool.log
```

**macOS/Linux:**

```bash
/path/to/fool --webui 2>&1 | tee fool.log
```
...
export FOOL_PORT=8080

# Allow remote access without passing --remote
export FOOL_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as FOOL_ALLOW_REMOTE=true)
export FOOL_HOST=0.0.0.0

# Then start the application
fool --webui

# You can also pass the port directly via CLI
fool --webui --port 8080
```
...
| Platform | Location                                                   |
| -------- | ---------------------------------------------------------- |
| Windows  | `%APPDATA%/The Fool/webui.config.json`                     |
| macOS    | `~/Library/Application Support/The Fool/webui.config.json` |
| Linux    | `~/.config/The Fool/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\The Fool\The Fool.exe" --resetpass

# Or for a specific user
"C:\Program Files\The Fool\The Fool.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/The Fool.app/Contents/MacOS/The Fool --resetpass

# Or for a specific user
/Applications/The Fool.app/Contents/MacOS/The Fool --resetpass username
```
...
# Or for a specific user
fool --resetpass username

# Or using full path
/opt/The Fool/fool --resetpass
```
...
- [GitHub Issues](https://github.com/zaorenn/The-Fool-Cli/issues)
...
2. Search [existing issues](https://github.com/zaorenn/The-Fool-Cli/issues)
3. Create a [new issue](https://github.com/zaorenn/The-Fool-Cli/issues/new) with:
   - Your OS and version
   - The Fool version
   - Steps to reproduce
...
**Happy using The Fool in WebUI mode!** 🚀
