# The Fool Headless Server Deployment Guide

Deploy The Fool WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- The Fool `.deb` package from [Releases](https://github.com/zaorenn/The-Fool-Cli/releases)
...
wget https://github.com/zaorenn/The-Fool-Cli/releases/latest/download/TheFool-linux-amd64.deb

# Install
sudo dpkg -i The Fool-linux-amd64.deb
...
The Fool is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/The Fool/start-fool.sh`:

```bash
#!/bin/bash
# The Fool WebUI headless startup script
# Usage: ./start-fool.sh [start|stop|restart|status]

PIDFILE="/var/run/fool.pid"
LOGFILE="/var/log/fool.log"
WORKDIR="$HOME"  # Change to your workspace directory

start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/The Fool --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "The Fool failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "The Fool is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping The Fool (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "The Fool --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "The Fool stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "The Fool is not running."
...
```bash
chmod +x /opt/The Fool/start-fool.sh
```

> **Tip**: `WORKDIR` determines the directory The Fool can access for file operations. Set it to your project workspace.

---

## Remote Access

The Fool WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for The Fool (Electron / Chromium Layer)
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/The Fool --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/The Fool/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: The Fool Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside The Fool WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by The Fool's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i The Fool-linux-amd64.deb`               |
| The Fool can only access `/tmp`           | Set `WORKDIR` in the startup script to your workspace path   |
...
在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 The Fool WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- The Fool `.deb` 安装包（[下载地址](https://github.com/zaorenn/The-Fool-Cli/releases)）
...
The Fool 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/The Fool/start-fool.sh`：

```bash
#!/bin/bash
# The Fool WebUI 无头启动脚本
# 用法: ./start-fool.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 The Fool WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "The Fool 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "The Fool 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "The Fool 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
The Fool WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（The Fool Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/The Fool/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/The Fool/proxy.pac"`
...
| The Fool 只能访问 /tmp | 修改启动脚本中的 `WORKDIR`            |
