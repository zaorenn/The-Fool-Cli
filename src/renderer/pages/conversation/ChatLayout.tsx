...
const ChatLayout: React.FC<{
  ...
  const headerBlock = (
    <>
      <ConversationTabs />
      <ArcoLayout.Header className={classNames('min-h-44px flex items-center justify-between px-16px pt-8px pb-10px gap-16px !bg-1 chat-layout-header chat-layout-header--glass overflow-hidden', layout?.isMobile && 'chat-layout-header--mobile-unified')}>
        ...
        <FlexFullContainer className='h-full min-w-0' containerClassName='flex items-center gap-16px'>
          {!layout?.isMobile && !hasTabs && (
            <div className={classNames('group flex min-w-0 max-w-full items-center rounded-12px border border-solid border-transparent transition-all duration-180', editingTitle ? 'bg-fill-2 border-[var(--color-fill-3)] shadow-[0_1px_2px_rgba(15,23,42,0.06)]' : 'hover:bg-fill-2 hover:border-[var(--color-fill-3)] hover:shadow-[0_1px_2px_rgba(15,23,42,0.06)] focus-within:bg-fill-2 focus-within:border-[var(--color-fill-3)] focus-within:shadow-[0_1px_2px_rgba(15,23,42,0.06)]')} style={{ width: '100%', maxWidth: `${titleAreaMaxWidth}px` }}>
              <div className='min-w-0 flex-1 px-10px py-5px'>
                {editingTitle && canRenameTitle ? (
                  ...
                  <span
                    ...
                    className={classNames('block min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-16px font-bold text-t-primary transition-colors duration-150', canRenameTitle && 'cursor-text group-hover:text-[rgb(var(--primary-6))] group-focus-within:text-[rgb(var(--primary-6))] focus:outline-none')}
                    onClick={() => {
                      if (!canRenameTitle) return;
