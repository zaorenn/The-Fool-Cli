      ...
      <div className='w-full min-h-full box-border overflow-y-auto px-14px pt-28px pb-24px md:px-40px md:pt-52px md:pb-42px'>
        <div className='mx-auto flex w-full max-w-760px flex-col gap-28px box-border'>
          <Button
            type='text'
            size='small'
            className='w-fit !px-0 !text-14px !text-text-3 dark:!text-t-secondary md:!text-15px hover:!text-text-1 dark:hover:!text-t-primary'
      ...
      <div className='mx-auto flex w-full max-w-760px flex-col gap-28px box-border'>
        ...
        <div className='flex flex-col gap-20px pb-8px'>
          <div className='flex flex-wrap items-start justify-between gap-14px'>
            <h1 className='m-0 min-w-0 flex-1 break-words text-30px font-bold leading-38px text-text-1 dark:text-t-primary md:text-34px md:leading-42px'>
              {job.name}
            </h1>
            <div className='flex shrink-0 items-center gap-8px'>
              <Button
                size='mini'
                type='text'
                className='!h-20px !min-w-20px !w-20px !rounded-0 !border-none !bg-transparent !p-0 !text-text-3 dark:!text-t-secondary hover:!bg-transparent hover:!text-text-2 dark:hover:!text-t-primary translate-y-1px'
          ...
          <div className='flex flex-wrap items-center gap-10px md:gap-12px'>
            ...
            {job.state.nextRunAtMs && (
              <span className='text-14px text-text-3 dark:text-t-secondary'>
        ...
        <div className='grid w-full min-w-0 grid-cols-1 gap-28px md:grid-cols-[280px_minmax(0,1fr)] md:items-start md:gap-40px'>
          <div className='flex min-w-0 flex-col gap-28px'>
            <section className='flex flex-col gap-10px'>
              <h2 className='m-0 text-13px font-medium text-text-3 dark:text-t-secondary'>
                {t('cron.detail.description')}
              </h2>
              <p className='m-0 text-14px leading-22px text-text-1 dark:text-t-primary'>{descriptionPreview}</p>
            </section>
            ...
            {job.metadata.agentConfig && (
              <section className='flex flex-col gap-10px'>
                ...
                <div className='flex items-center gap-10px'>
                  ...
                  />
                  <span className='min-w-0 text-14px font-medium text-text-1 dark:text-t-primary'>
            ...
            <section className='flex flex-col gap-10px'>
              ...
              <div className='flex flex-wrap items-center gap-10px'>
                ...
                <span className='text-14px text-text-1 dark:text-t-primary'>{formatSchedule(job, t)}</span>
              </div>
            </section>

            <section className='flex flex-col gap-10px'>
              <h2 className='m-0 text-13px font-medium text-text-3 dark:text-t-secondary'>
                {t('cron.page.form.executionMode')}
              </h2>
              <div className='inline-flex items-center gap-4px'>
                <span className='text-14px leading-22px text-text-1 dark:text-t-primary'>
                  {currentExecutionModeLabel}
                </span>
                <Attention
                  theme='outline'
                  size={12}
                  className='line-height-0 shrink-0 text-text-3 dark:text-t-secondary'
                />
              </div>
              <div className='box-border rounded-12px border border-solid border-[var(--color-border-2)] bg-fill-2 px-16px py-14px'>
                <div className='flex flex-col gap-10px'>
                  <p className='m-0 text-13px leading-20px text-text-2 dark:text-t-primary'>
                    {executionModeExplanation}
                  </p>
                  <div className='h-1px w-full bg-[var(--color-border-2)]' />
                  <p className='m-0 text-12px leading-18px text-text-3 dark:text-t-secondary'>
          ...
          <div className='flex min-w-0 flex-col gap-28px'>
            <section className='flex flex-col gap-12px'>
              <h2 className='m-0 text-13px font-medium text-text-3 dark:text-t-secondary'>
                {t('cron.detail.instructions')}
              </h2>
              <div className='box-border rounded-12px border border-solid border-[var(--color-border-2)] bg-fill-2 px-16px py-14px'>
                <div className='whitespace-pre-wrap break-words text-14px leading-22px text-text-1 dark:text-t-primary'>
            ...
            <section className='flex flex-col gap-12px'>
              ...
              {conversations.length > 0 ? (
                <div className='flex flex-col'>
                  ...
                  {conversations.map((conv, index) => (
                    <React.Fragment key={conv.id}>
                      <div
                        className='flex cursor-pointer items-center justify-between gap-14px py-15px transition-colors hover:text-text-1 dark:hover:text-t-primary'
                        onClick={() => navigate(`/conversation/${conv.id}`)}
                      >
                        <span className='min-w-0 flex-1 truncate text-14px text-text-1 dark:text-t-primary'>
                          {conv.name || conv.id}
                        </span>
                        <span className='shrink-0 text-13px text-text-3 dark:text-t-secondary'>
                ...
                <div className='text-14px text-text-3 dark:text-t-secondary'>
