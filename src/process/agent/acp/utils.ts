...
export async function readTextFile(file_path: string): Promise<{ content: string }> {
  try {
    const content = await fsAsync.readFile(file_path, 'utf-8');
    return { content };
  } catch (error) {
...
export async function writeTextFile(file_path: string, content: string): Promise<null> {
  try {
    await fsAsync.mkdir(path.dirname(file_path), { recursive: true });
    await fsAsync.writeFile(file_path, content, 'utf-8');

    // Send streaming content update to preview panel (for real-time updates)
    try {
      const { ipcBridge } = await import('@/common');
      const pathSegments = file_path.split(path.sep);
...
export function writeJsonRpcMessage(child: ChildProcess, message: object): void {
  if (child.stdin) {
    ...
    child.stdin.write(JSON.stringify(message) + line_ending);
  }
}
