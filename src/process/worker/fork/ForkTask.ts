...
export class ForkTask<Data> extends Pipe {
  ...
  protected init() {
    ...
    const workerCwd = platform.paths.isPackaged()
      ? (platform.paths.getAppPath() ?? process.cwd()).replace(
          "app.asar",
          "app.asar.unpacked",
        )
      : process.cwd();
    ...
    fcp.on("message", (...args: unknown[]) => {
      ...
      if (e.type === "complete") {
        ...
        if (e.pipeId) {
          // 如果存在回调，则将回调信息发送到子进程
          Promise.resolve(deferred.pipe(this.postMessage.bind(this))).catch(
            (error) => {
  ...
  // 向子进程发送回调
  postMessage(
    type: MainToWorkerMessage["type"] | string,
    data: unknown,
    extPrams: Record<string, unknown> = {},
  ) {
