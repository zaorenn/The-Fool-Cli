...
export function initDatabaseBridge(repo: IConversationRepository): void {
  ...
  ipcBridge.database.getConversationMessages.provider(
    async ({ conversation_id, page = 0, pageSize = 10000 }) => {
      try {
        const result = await repo.getMessages(conversation_id, page, pageSize);
        return result.data;
      } catch (error) {
        console.error(
          "[DatabaseBridge] Error getting conversation messages:",
          error,
        );
        return [];
      }
  ...
  ipcBridge.database.getUserConversations.provider(
    async ({ page = 0, pageSize = 10000 }) => {
      try {
        ...
        try {
          ...
          console.warn(
            "[DatabaseBridge] No file-based conversations found:",
            error,
          );
        }
        ...
        const dbConversationMap = new Map(
          dbConversations.map((conv) => [conv.id, conv] as const),
        );
        ...
        if (fileOnlyConversations.length > 0) {
          void Promise.all(
            fileOnlyConversations.map((conv) =>
              migrateConversationToDatabase(conv),
            ),
          );
        }
        ...
        allConversations.sort(
          (a, b) =>
            (b.modifyTime || b.createTime || 0) -
            (a.modifyTime || a.createTime || 0),
        );
        return allConversations;
      } catch (error) {
        console.error(
          "[DatabaseBridge] Error getting user conversations:",
          error,
        );
        return [];
      }
  ...
  ipcBridge.database.searchConversationMessages.provider(
    async ({ keyword, page = 0, pageSize = 20 }) => {
      try {
        const result = await repo.searchMessages(keyword, page, pageSize);
        return result;
      } catch (error) {
        console.error(
          "[DatabaseBridge] Error searching conversation messages:",
          error,
        );
        return {
          items: [],
