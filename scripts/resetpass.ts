 ...
 *   2. No webui running → spawn a short-lived foolcore against the same
 ...
 *   FOOL_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@fool/web-host';
 ...
 * See the comment there for why the default is `~/.fool-web*` (not `~/.fool*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `FOOL_PORT` to match
 ...
 * pass --port / FOOL_PORT if they launched webui on a non-default one.
 */
