...
export INSTALL_DIR="/tmp/fool-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/fool-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/fool-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/fool-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/fool-web"
    exit 1
fi
...
VERSION_OUTPUT=$(fool-web version 2>&1 || echo "")
