#!/usr/bin/env bash
# ============================================================================
# The Fool WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/zaorenn/The-Fool-Cli/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/fool-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/fool-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/zaorenn/The-Fool-Cli/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     The Fool WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/fool-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="fool-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/zaorenn/The-Fool-Cli/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is fool-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/fool-web" ]]; then
        mv "${extract_temp}/fool-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing fool-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/fool-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/fool-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/fool-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/fool-web"
    local target_path="${INSTALL_DIR}/fool-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by fool-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 The Fool WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/fool-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start The Fool WebUI"
        echo "    fool-web start"
        echo ""
        echo "    # Check version"
        echo "    fool-web version"
    else
        echo "    # Start The Fool WebUI (using full path)"
        echo "    ${INSTALL_DIR}/fool-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/fool-web ~/.local/bin/fool-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/zaorenn/The-Fool-Cli"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/zaorenn/The-Fool-Cli/issues"
    echo ""
