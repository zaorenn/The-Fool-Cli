...
//! `.foolrs/skills/` and `.foolrs/commands/` layout, then exercises the full
...
use foolrs::context::{SystemPromptCache, build_system_prompt};
use foolrs::skills::loader::load_all_skills;
use foolrs::skills::permissions::SkillPermissionChecker;
use foolrs::skills::types::SkillMetadata;
use foolrs::tools::skill::SkillTool;
...
/// Create a project-like temp directory with `.git` marker and `.foolrs/skills/` + `.foolrs/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    let commands_dir = root.join(".foolrs").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .foolrs/commands/");
}
