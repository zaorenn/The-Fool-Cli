...
impl SkillWatcher {
    ...
    pub fn watch_directory(&mut self, dir: &Path) -> notify::Result<()> {
        if !dir.is_dir() {
            tracing::debug!(target: "foolrs_skills", path = %dir.display(), "skipped non-existent watcher directory");
            return Ok(());
        }
        ...
        if let Some(ref mut w) = self.watcher {
            ...
            tracing::debug!(target: "foolrs_skills", path = %dir.display(), "watching skill directory");
        }
