...
/// 5. Skill directory: `${FOOLRS_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${FOOLRS_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    if let Some(root) = skill_root {
        result = result.replace("${FOOLRS_SKILL_DIR}", root);
    }

    // 6. ${FOOLRS_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${FOOLRS_SESSION_ID}", sid);
    }
