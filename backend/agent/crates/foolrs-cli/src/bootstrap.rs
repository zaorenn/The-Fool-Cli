//! Shared initialization steps for foolrs-cli entry points.
//!
...
//!
//! Note: `foolrs_agent` also exposes a module named `bootstrap`
//! (`foolrs_agent::bootstrap::AgentBootstrap`). Call sites should keep using
//! that fully-qualified path for the engine builder itself; this module is
//! `crate::bootstrap` and only wraps it.

use std::sync::Arc;

use foolrs_agent::bootstrap::{AgentBootstrap, BootstrapResult};
use foolrs_agent::output::OutputSink;
use foolrs_agent::session::{Session, SessionManager};
use foolrs_compact::CompactLevel;
use foolrs_config::config::{CliArgs, Config};
use foolrs_config::logging::{LoggingGuard, create_file_layer};
