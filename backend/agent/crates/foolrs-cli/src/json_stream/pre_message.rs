...
use foolrs_mcp::tool_proxy::register_single_server_tools;
use foolrs_protocol::commands::ProtocolCommand;
use foolrs_protocol::events::ProtocolEvent;
use foolrs_protocol::writer::{ProtocolEmitter, ProtocolWriter};
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "foolrs_mcp", %name, %transport, ?command, "AddMcpServer received");
                ...
                tracing::info!(target: "foolrs_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        tracing::info!(target: "foolrs_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "foolrs_mcp", %name, error = %e, "mcp server connection failed");
