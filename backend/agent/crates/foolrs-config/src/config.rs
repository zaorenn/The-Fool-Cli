...
use foolrs_types::llm::ThinkingConfig;
...
fn resolve_api_key(cli_key: Option<&str>, config_key: Option<&str>, provider: ProviderType) -> anyhow::Result<String> {
    ...
    anyhow::bail!(
        "No API key found. Provide via --api-key, config file, environment variable \
         (API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY), or run 'foolrs auth login'."
    )
}
...
///
/// - Linux:   `~/.config/foolrs`
/// - macOS:   `~/Library/Application Support/foolrs`
/// - Windows: `%APPDATA%\foolrs`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("foolrs"))
}
...
fn project_config_path() -> PathBuf {
    PathBuf::from(".foolrs.toml")
}

fn load_config_file(path: &Path) -> ConfigFile {
    match std::fs::read_to_string(path) {
        Ok(content) => toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "foolrs_config", path = %path.display(), error = %e, "failed to parse config file");
            ConfigFile::default()
        }),
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "foolrs_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "foolrs_config", path = %path.display(), "config file created");
    Ok(())
}

const DEFAULT_CONFIG_TEMPLATE: &str = r#"# foolrs configuration
...
# OAuth settings (for `foolrs auth login` with Claude.ai account)
# [auth]
...
[session]
enabled = true
directory = ".foolrs/sessions"  # relative to project root
...
# dir = "~/Library/Logs/foolrs"    # log directory (default: platform-specific)
