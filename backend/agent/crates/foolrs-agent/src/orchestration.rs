...
use foolrs_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
...
use foolrs_types::skill_types::ContextModifier;
...
pub async fn execute_tool_calls(
    ...
    compaction_level: foolrs_compact::CompactLevel,
    toon_enabled: bool,
...
async fn execute_single(
    ...
    tracing::info!(target: "foolrs_agent", tool = %name, call_id = %id, "tool execution started");
    ...
    let (result, modifier, follow_up_blocks) = match registry.get(name) {
        Some(tool) => {
            ...
            let content = foolrs_compact::compact_output(&content, compaction_level);
            let content = if toon_enabled {
                foolrs_compact::compact_output_toon(&content)
            } else {
                content
            };
            (
    ...
    if let Some(hook_engine) = hooks {
        ...
        for msg in messages {
            tracing::info!(target: "foolrs_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "foolrs_agent", duration_ms, success = !result.is_error, "tool execution completed");
