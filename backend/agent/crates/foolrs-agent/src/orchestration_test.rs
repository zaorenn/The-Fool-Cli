...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> foolrs_types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return foolrs_types::tool::ToolResult {
        ...
        fn category(&self) -> foolrs_protocol::events::ToolCategory {
            foolrs_protocol::events::ToolCategory::Exec
        }
    }
    ...
    async fn execute_single_deferred_tool_error_missing_required_appends_hint() {
        ...
        let (result, _, follow_up_blocks) =
            execute_single(&registry, &call, None, foolrs_compact::CompactLevel::Off, false).await;
