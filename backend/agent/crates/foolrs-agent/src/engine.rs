...
impl AgentEngine {
    ...
    async fn run_inner(&mut self, content_blocks: Vec<ContentBlock>, msg_id: &str) -> Result<AgentResult, AgentError> {
        ...
        loop {
            if let Some(limit) = guards.turn_budget_reached() {
                ...
                warn!(target: "foolrs_agent", limit, "stopping agent run at turn budget");
    ...
    fn record_turn_usage(&mut self, turn_usage: &TokenUsage) -> bool {
        ...
        if context_usage == 0 {
            debug!(target: "foolrs_agent", "provider omitted turn usage; retaining local context projection");
            return false;
        }
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            info!(target: "foolrs_agent", context_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
...
impl AgentEngine {
    ...
    pub fn init_session(&mut self, provider_name: &str, cwd: &str, session_id: Option<&str>) -> AnyhowResult<()> {
        if let Some(mgr) = &self.session_manager {
            ...
            mgr.save(&session)?;
            info!(target: "foolrs_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                info!(target: "foolrs_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
}
