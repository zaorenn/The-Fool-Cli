...
mod tests_set_config {
    ...
    use foolrs_config::compat::ReasoningCompat;
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            compact_config: foolrs_config::compact::CompactConfig::default(),
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(foolrs_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(foolrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 8000 });
        let changes = engine.apply_config_update(None, None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(foolrs_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_budget_only_updates_existing_enabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(foolrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 5000 });
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(foolrs_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(foolrs_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_applies_when_capability_is_false() {
        ...
        assert!(matches!(
            engine.thinking,
            Some(foolrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 10000 })
        ));
    }
...
mod tests_phase6 {
    ...
    use foolrs_tools::registry::ToolRegistry;
    use foolrs_types::llm::{LlmEvent, LlmRequest};
    use foolrs_types::skill_types::{ContextModifier, EffortLevel};
...
mod tests_plan_mode {
    ...
    use foolrs_tools::registry::ToolRegistry;
    use foolrs_types::llm::{LlmEvent, LlmRequest};
    use foolrs_types::skill_types::{ContextModifier, PlanModeTransition};
...
mod tests_handle_command {
    ...
    #[tokio::test]
    async fn run_treats_fool_files_marker_as_plain_text() {
        let mut engine = make_engine_with_provider(Arc::new(SingleResponseProvider));
        let input = "discuss [[FOOL_FILES]] and file.txt";
...
mod tests_loop_helpers {
    use foolrs_types::message::{ContentBlock, StopReason, TokenUsage};
    use serde_json::json;
