...
use foolrs_agent::confirm::ToolConfirmer;
...
use foolrs_config::config::{Config, ProviderType, SessionConfig, ToolsConfig};
use foolrs_config::hooks::HooksConfig;
use foolrs_mcp::config::McpConfig;
use foolrs_providers::create_provider;
...
fn openai_config(api_key: &str) -> Config {
    Config {
        ...
        tools: ToolsConfig {
            auto_approve: true,
            allow_list: vec![],
            skills: foolrs_config::config::SkillsPermissionConfig::default(),
        },
        ...
        compact: foolrs_config::compact::CompactConfig::default(),
        plan: foolrs_config::plan::PlanConfig::default(),
        shell: foolrs_config::shell::ShellConfig::default(),
        file_cache: foolrs_config::file_cache::FileCacheConfig::default(),
        hooks: HooksConfig::default(),
        bedrock: None,
        vertex: None,
        mcp: McpConfig::default(),
        logging: foolrs_config::logging::LoggingConfig::default(),
    }
}
...
impl foolrs_tools::Tool for FixedOutputTool {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn category(&self) -> foolrs_protocol::events::ToolCategory {
        foolrs_protocol::events::ToolCategory::Info
    }
    ...
    async fn execute(&self, _input: serde_json::Value) -> foolrs_types::tool::ToolResult {
        foolrs_types::tool::ToolResult {
