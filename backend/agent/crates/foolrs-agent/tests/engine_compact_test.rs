...
async fn tc_2_6_05_session_save_after_compact() {
    ...
    let has_boundary = session.messages.iter().any(|m| {
        m.content.iter().any(|b| {
            matches!(b, foolrs_types::message::ContentBlock::Text { text } if text.contains("[Conversation compacted]"))
        })
    });
...
async fn tc_2_6_02_micro_before_auto_execution_order() {
    ...
    let captured: Arc<Mutex<Option<Vec<foolrs_types::message::Message>>>> = Arc::new(Mutex::new(None));
    let capture_ref = captured.clone();

    struct OrderProvider {
        regular_count: Mutex<usize>,
        captured: Arc<Mutex<Option<Vec<foolrs_types::message::Message>>>>,
    }
    ...
    let cleared_count = msgs
        ...
        .filter(|b| {
            matches!(
                b,
                foolrs_types::message::ContentBlock::ToolResult { content, .. }
                    if content == foolrs_agent::compact::micro::CLEARED_TOOL_RESULT
            )
        })
        .count();
