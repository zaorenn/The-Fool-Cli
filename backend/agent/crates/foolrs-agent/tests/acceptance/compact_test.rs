...
use foolrs_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use foolrs_agent::compact::emergency::is_at_emergency_limit;
use foolrs_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
use foolrs_agent::compact::state::CompactState;
...
async fn autocompact_triggers_llm_summary() {
    ...
    let provider = foolrs_providers::create_provider(&config);
