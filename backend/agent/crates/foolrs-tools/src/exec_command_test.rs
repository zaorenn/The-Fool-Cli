...
mod tests {
    ...
    async fn execute_injects_runtime_env() {
        let tool = ExecCommandTool::new_with_env(
            std::env::temp_dir(),
            vec![("FOOL_RUNTIME_ENV_TEST".to_string(), "exec-env-value".to_string())],
        );
        #[cfg(windows)]
        let input = json!({"cmd": "Write-Output $env:FOOL_RUNTIME_ENV_TEST", "shell": "powershell"});
        #[cfg(not(windows))]
        let input = json!({"cmd": "printf '%s' \"$FOOL_RUNTIME_ENV_TEST\"", "shell": "sh"});
    ...
    async fn execute_timeout_preserves_stdout_emitted_before_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "Write-Output fool_stdout_before_timeout; Start-Sleep -Seconds 5";
        #[cfg(not(windows))]
        let cmd = "printf 'fool_stdout_before_timeout\\n'; sleep 5";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("fool_stdout_before_timeout"),
    ...
    async fn execute_timeout_preserves_stderr_emitted_before_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "[Console]::Error.WriteLine('fool_stderr_before_timeout'); Start-Sleep -Seconds 5";
        #[cfg(not(windows))]
        let cmd = "printf 'fool_stderr_before_timeout\\n' >&2; sleep 5";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("STDERR:\n") && result.content.contains("fool_stderr_before_timeout"),
    ...
    async fn execute_timeout_omits_output_after_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "Write-Output fool_before_timeout; Start-Sleep -Seconds 5; Write-Output fool_after_timeout";
        #[cfg(not(windows))]
        let cmd = "printf 'fool_before_timeout\\n'; sleep 5; printf 'fool_after_timeout\\n'";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("fool_before_timeout"),
            "output emitted before timeout should be preserved, got: {}",
            result.content
        );
        assert!(
            !result.content.contains("fool_after_timeout"),
    ...
    async fn execute_powershell_write_output_returns_stdout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        let input = json!({
            "cmd": "Write-Output fool_powershell_stdout_probe",
            "shell": "powershell"
        });
        ...
        assert!(!result.is_error, "unexpected error: {}", result.content);
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("fool_powershell_stdout_probe"),
    ...
    async fn execute_cmd_echo_returns_stdout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        let input = json!({
            "cmd": "echo fool_cmd_stdout_probe",
            "shell": "cmd"
        });
        ...
        assert!(!result.is_error, "unexpected error: {}", result.content);
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("fool_cmd_stdout_probe"),
