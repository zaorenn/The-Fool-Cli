...
use foolrs_types::message::{
    ContentBlock, ImageUrl, MAX_IMAGE_INPUT_BYTES, extension_to_image_media_type, sniff_image_media_type,
};
...
impl ViewImageTool {
    ...
    async fn load_image(&self, input: &Value) -> Result<ImageUrl, String> {
        ...
        if metadata.len() > MAX_IMAGE_INPUT_BYTES {
            return Err(format!("Image exceeds the {} byte size limit", MAX_IMAGE_INPUT_BYTES));
        }
        ...
        let detected_mime_type = sniff_image_media_type(&bytes)
        ...
        let image_url = ImageUrl::from_image_bytes(detected_mime_type, &bytes);
        image_url
            .validate()
