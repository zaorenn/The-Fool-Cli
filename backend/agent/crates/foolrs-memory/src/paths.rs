...
const MEMORY_DIR_ENV: &str = "FOOLRS_MEMORY_DIR";
...
///
/// Resolution order:
///   1. `FOOLRS_MEMORY_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `foolrs-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    foolrs_config::config::app_config_dir()
}
