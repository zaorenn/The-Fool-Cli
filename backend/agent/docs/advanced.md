...
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .foolrs/audit.log"
...
```bash
# CLI — logs to /tmp/foolrs-logs at debug level
foolrs --log-dir /tmp/foolrs-logs --log-level debug "Read Cargo.toml"
```
...
| Platform | Path                                                            |
| -------- | --------------------------------------------------------------- |
| macOS    | `~/Library/Logs/foolrs/`                                        |
| Linux    | `$XDG_STATE_HOME/foolrs/logs/` or `~/.local/state/foolrs/logs/` |
| Windows  | `{data_local_dir}/foolrs/logs/`                                 |
...
grep '"session_id":"abc-123"' 2026-05-13.foolrs.log | jq .
```
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // foolrs logs → separate foolrs.log file
    .init();
```

The host application owns the global subscriber; foolrs library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/foolrs/AGENTS.md` — user-level instructions for all projects
...
```
<config_dir>/foolrs/projects/<sanitized-project-path>/memory/
...
plan_directory = ".foolrs/plans"  # Where plan files are saved
```
