...
use fool_common::constants::UPLOAD_MAX_SIZE;
...
pub struct FileRouterState {
    ...
    pub project: Arc<fool_project::ProjectService>,
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `fool-app`
...
fn to_zip_entry(e: fool_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => fool_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => fool_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = fool_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, fool_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, fool_api_types::SnapshotMode::Snapshot);
