...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "fool-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "fool";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@fool.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/fool-snapshot-{hash}` (bare-style git dir).
