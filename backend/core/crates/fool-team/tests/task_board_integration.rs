...
use fool_db::{ITeamRepository, SqliteTeamRepository, init_database_memory};
use fool_team::{TaskBoard, TaskStatus, TaskUpdate};

async fn setup() -> (TaskBoard, fool_db::Database) {
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteTeamRepository::new(db.pool().clone()));
    repo.create_team(&fool_db::models::TeamRow {
        id: "t1".to_owned(),
