...
use fool_team::events::TeamEventEmitter;
use fool_team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use fool_team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use fool_team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use fool_team::visibility::TeamVisibilityPolicy;
use fool_team::{Mailbox, TaskBoard, TeammateManager};
...
fn lp2_lead_prompt_contains_tool_descriptions() {
    ...
    // The Fool lead prompt references the `team_*` coordination tools that the
