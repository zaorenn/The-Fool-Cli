...
use fool_ai_agent::shared_kernel::approval_key;
use fool_ai_agent::types::{BuildTaskOptions, SendMessageData};
use fool_api_types::AgentModeResponse;
...
use fool_team::event_loop::AgentLoopContext;
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<fool_db::models::MessageRow>, fool_team::TeamError> {
        Ok(None)
    }
...
impl IAgentTask for RecordingAgent {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), fool_ai_agent::AgentSendError> {
        ...
        match &self.fail_with {
            Some(msg) => Err(fool_ai_agent::AgentSendError::from_agent_error(AgentError::internal(
                msg.clone(),
            ))),
            None => Ok(()),
        }
...
#[async_trait]
impl fool_ai_agent::IWorkerTaskManager for StubTaskManager {
...
fn backend_path() -> Arc<PathBuf> {
    Arc::new(PathBuf::from("/tmp/foolcore-e2e-test"))
}
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn fool_ai_agent::IWorkerTaskManager> = task_manager.clone();
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn fool_ai_agent::IWorkerTaskManager> = task_manager;
    let team = fool_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", fool_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", fool_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            fool_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn one_notify_drains_five_intents_after_busy_turn() {
    ...
    for index in 1..=5 {
        ...
        assert_eq!(ack.enqueue_status, fool_api_types::TeamMessageEnqueueStatus::Queued);
...
async fn s10_error_finish_sets_agent_status_to_error() {
    ...
    assert_eq!(
        status,
        fool_team::TeammateStatus::Error,
