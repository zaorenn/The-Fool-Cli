...
pub(crate) mod workspace_harness {
    ...
    use fool_common::{AgentKillReason, AgentType, PaginatedResult, now_ms};
    use fool_db::models::{
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(
            &self,
            _user_id: &str,
            _row: &fool_db::models::MailboxMessageRow,
        ) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(
            ...
            _team_id: &str,
            _task_id: &str,
            _params: &fool_db::UpdateTaskParams,
        ) -> Result<(), DbError> {
            Ok(())
        }
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<fool_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: fool_ai_agent::types::BuildTaskOptions,
    ...
    pub(crate) fn setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(std::path::PathBuf::from("/tmp/foolcore-test")),
        );
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![fool_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
