...
use fool_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_id, now_ms};
...
impl TeamSessionService {
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                fool_api_types::TeamRunStatus::Accepted
                    | fool_api_types::TeamRunStatus::Running
                    | fool_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = fool_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
    ...
    /// the content (`[[FOOL_FILES]]` form) at the team send boundary. Atomic;
...
async fn set_active_agent_session_mode(instance: &AgentInstance, mode: &str) -> Result<(), AgentError> {
    ...
    match instance {
        ...
        AgentInstance::Foolrs(manager) => manager.set_mode(mode).await,
...
mod tests {
    ...
    use fool_api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use fool_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use fool_db::{IConversationRepository, ITeamRepository};
    ...
    fn two_agent_team_request(name: &str) -> fool_api_types::CreateTeamRequest {
        fool_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                fool_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_foolrs_worker_request(name: &str) -> fool_api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(fool_api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
    ...
    async fn idle_cleanup_stops_team_session_when_foolrs_member_is_idle_pending() {
        ...
        let (svc, _repo, _task_manager, _conv_repo, _broadcaster) =
            setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(task_manager.clone());
        let created = svc
            .create_team("user-test", team_with_foolrs_worker_request("Idle Foolrs Cleanup"))
            .await
            .unwrap();
        let lead = created.assistants.iter().find(|agent| agent.role == "lead").unwrap();
