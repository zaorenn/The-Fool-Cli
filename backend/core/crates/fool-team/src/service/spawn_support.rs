...
use fool_db::models::AgentMetadataRow;
use fool_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use std::sync::Arc;
...
impl TeamSessionService {
    ...
    /// Returns a flat list of model IDs for use by internal agents (foolrs).
...
mod tests {
    ...
    use fool_db::models::{AgentMetadataRow, AssistantDefinitionRow, AssistantOverlayRow, Provider};
    use fool_db::{
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: fool_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
    ...
    async fn create_team_rejects_assistant_missing_from_team_selectable_catalog() {
        let svc = service_with_selectable_catalog(vec![], vec![assistant_definition("word-creator", "foolrs")]);
    ...
    async fn add_agent_rejects_assistant_missing_from_team_selectable_catalog() {
        ...
        let err = svc
            .add_agent(
                "user1",
                &created.id,
                fool_api_types::AddAgentRequest {
                    name: "Worker".into(),
                    role: "teammate".into(),
                    backend: None,
    ...
    async fn resolve_spawn_backend_and_model_prefers_assistant_identity_over_caller_backend() {
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(RowsTeamAssistantCatalog {
                rows: vec![team_assistant_entry("word-creator", "Word Creator", "foolrs")],
            }),
