...
use fool_api_types::TeamToolTransport;

mod wake_summary;

pub use fool_team_prompts::AvailableAssistant;

use crate::types::{MailboxMessage, MailboxMessageType, TeamAgent, TeamTask};

fn to_prompt_role(role: crate::types::TeammateRole) -> fool_team_prompts::TeamPromptRole {
    match role {
        crate::types::TeammateRole::Lead => fool_team_prompts::TeamPromptRole::Lead,
        crate::types::TeammateRole::Teammate => fool_team_prompts::TeamPromptRole::Teammate,
    }
}

fn to_prompt_agent(agent: &TeamAgent) -> fool_team_prompts::TeamPromptAgent {
    fool_team_prompts::TeamPromptAgent {
...
///
/// Delegates to `fool-team-prompts`, the canonical Team role prompt crate.
...
pub fn build_lead_prompt_for_transport(
    ...
    let body = fool_team_prompts::build_lead_prompt(&fool_team_prompts::LeadPromptParams {
        agent: &prompt_agent,
        team_name,
...
pub fn build_teammate_prompt_for_transport(
    ...
    let leader = prompt_members
        .iter()
        .find(|candidate| candidate.role == fool_team_prompts::TeamPromptRole::Lead)
        .cloned()
        .unwrap_or_else(|| fool_team_prompts::TeamPromptAgent {
            slot_id: "lead".to_owned(),
            name: "Lead".to_owned(),
            role: fool_team_prompts::TeamPromptRole::Lead,
            backend: agent.backend.clone(),
            model: agent.model.clone(),
            status: None,
        });
    ...
    fool_team_prompts::build_teammate_prompt(&fool_team_prompts::TeammatePromptParams {
        agent: &prompt_agent,
        team_name,
        leader: &leader,
        teammates: &teammates,
