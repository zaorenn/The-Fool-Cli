# Team 提示词体系（The Fool 参考实现）

> 本文档完整记录 The Fool main 分支中 team 模块的三层提示词设计。
> 源码路径前缀：`/Volumes/Macintosh HD/Users/zhuqingyu/project/The Fool/src/process/team/prompts/`
...
- [The Fool 完整调研](./fool-team-complete.md) — team 模块全貌（含 prompt 注入路径）
...
以下是 The Fool leader prompt 中关于 shutdown 的**原文**，后端实现时应直接复用：
...
以下是 The Fool 每个 MCP 工具的 description + schema 原文。后端在 `tools/list` 返回的 `ToolDescriptor` 里必须使用这些文本，不要改写。
...
- `crates/fool-team/src/prompts.rs` — leader + teammate prompt 的基础版本
- `crates/fool-team/src/mcp/tools.rs` — `team_spawn_agent` 工具描述
...
### 后端已有 prompt vs The Fool prompt 对比

后端的 `prompts.rs` 需要与 The Fool 对齐的点：
...
（需要读后端 prompts.rs 做逐项比对，本文档先记录 The Fool 侧事实）
