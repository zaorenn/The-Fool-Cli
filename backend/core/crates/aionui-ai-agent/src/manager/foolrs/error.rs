...
pub(super) fn foolrs_engine_error_to_send_error(error: &AionrsAgentError) -> AgentSendError {
    let detail = format!("Aionrs agent error: {error}");
    match error {
        AionrsAgentError::Provider(provider_error) => foolrs_provider_error_to_send_error(provider_error, detail),
...
pub(super) fn foolrs_runtime_error_summary(error: &AionrsAgentError) -> AionrsRuntimeErrorSummary {
    match error {
