...
use aionui_ai_agent::manager::foolrs::AionrsAgentManager;
...
fn make_foolrs_config() -> AionrsResolvedConfig {
...
async fn foolrs_agent_kill_succeeds() {
    let agent = AionrsAgentManager::new("conv-1".into(), "/proj".into(), make_foolrs_config(), None)
        .await
        .unwrap();
...
async fn foolrs_agent_metadata() {
    let agent = AionrsAgentManager::new("conv-abc".into(), "/work".into(), make_foolrs_config(), None)
        .await
        .unwrap();
...
async fn collect_idle_ignores_foolrs_agent_type() {
    ...
    mgr.get_or_build_task("acp-1", make_opts(AgentType::Acp, "acp-1"))
        .await
        .unwrap();
    mgr.get_or_build_task("foolrs-1", make_opts(AgentType::Aionrs, "foolrs-1"))
        .await
        .unwrap();
