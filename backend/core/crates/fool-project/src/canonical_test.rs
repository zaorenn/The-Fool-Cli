...
fn casing_folds_per_platform() {
    let mixed = canonicalize("file:///Users/Me/Fool").unwrap();
    let lower = canonicalize("file:///users/me/fool").unwrap();
    if IGNORE_PATH_CASING {
        // macOS / Windows: same folder.
        assert_eq!(mixed, lower);
        assert_eq!(mixed.as_str(), "file:///users/me/fool");
    } else {
...
fn basename_is_final_segment() {
    let c = canonicalize("file:///Users/me/fool").unwrap();
...
fn to_file_uri_does_not_fold_casing() {
    ...
    let uri = to_file_uri(std::path::Path::new("/Users/Me/Fool")).unwrap();
    assert_eq!(uri, "file:///Users/Me/Fool");
}
