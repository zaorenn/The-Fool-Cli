...
use fool_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
use fool_db::models::{AssistantDefinitionRow, ConversationAssistantSnapshotRow, ConversationRow, MessageRow};
use fool_db::{
...
use fool_extension::AssistantRuleDispatcher;
...
use crate::session_context::{FoolrsRuntimePermissionSeed, SessionContextBuilder};
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a fool_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    /// into the message content (`[[FOOL_FILES]]` form) at the send boundary.
    ...
    async fn resolve_message_attachments(
        ...
        let upload_root = std::env::temp_dir().join("fool");
        project
    ...
    /// `delete()` removes the conversation row. Used by `fool-app` to wire
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<fool_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(fool_api_types::ConversationAssistantIdentityResponse {
    ...
    /// to `fool`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::Fool);
        ...
        // Type-aware rule: top-level `model` is foolrs-only. Other agent types
        ...
        if effective_type != AgentType::Foolrs && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for foolrs conversations; pass model via `extra` for {}",
                    effective_type.serde_name()
                ),
            });
        }

        // foolrs source-of-truth rule: top-level `model` wins. If an older client
        // still packs `extra.model`, strip it before persist so the stored row
        // has a single canonical model representation.
        if effective_type == AgentType::Foolrs
            && let Some(obj) = extra.as_object_mut()
            && obj.remove("model").is_some()
        {
            warn!("foolrs create: stripped legacy `extra.model`; top-level `model` is canonical");
        }
        ...
        let row = fool_db::models::ConversationRow {
            id: id.clone(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(fool_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert_for_user(
                user_id,
                &fool_db::UpsertAssistantPreferenceParams {
                    assistant_definition_id: &snapshot.assistant_definition_id,
    ...
    pub async fn update(
        ...
        // Type-aware rule: top-level `model` is foolrs-only. For non-foolrs
        // conversations, model/mode must be updated via `extra` (see spec
        // 2026-05-12).
        if existing_type != AgentType::Foolrs && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for foolrs conversations; pass model via `extra` for {}",
                    existing.r#type
        ...
        // Merge extra if provided. For foolrs, strip `extra.model` post-merge
        ...
        let merged_extra = if let Some(new_extra) = &req.extra {
            ...
            if existing_type == AgentType::Foolrs
                && let Some(obj) = existing_extra.as_object_mut()
                && obj.remove("model").is_some()
            {
                warn!("foolrs update: stripped legacy `extra.model` from merged extra");
            }
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        // `[[FOOL_FILES]]` content used for persistence, broadcast, and the turn.
        let resolved = self
        ...
        let user_msg = fool_db::models::MessageRow {
...
impl ConversationService {
    /// Loads the persisted runtime permission gate inputs for an foolrs
    /// rebuild. Returns `None` for non-foolrs conversations and for foolrs
    ...
    async fn load_foolrs_permission_seed(
        &self,
        row: &fool_db::models::ConversationRow,
    ) -> Result<Option<FoolrsRuntimePermissionSeed>, ConversationError> {
        if row.r#type != AgentType::Foolrs.serde_name() {
            return Ok(None);
        }
        let snapshot = self
            .conversation_repo
            ...
            .map_err(|e| {
                ConversationError::internal(format!(
                    "Failed to load assistant snapshot for foolrs permission seed: {e}"
                ))
            })?;
        Ok(snapshot.map(|snapshot| FoolrsRuntimePermissionSeed {
            default_permission_mode: snapshot.default_permission_mode,
    ...
    pub(crate) async fn build_task_options(
        ...
        reject_deprecated_runtime_row(row)?;
        let seed = self.load_foolrs_permission_seed(row).await?;
    ...
    fn runtime_token_for_build(
        ...
        // CLI (`foolcore config` / `diagnose`) can authenticate in Pro
    ...
    /// workspaces are constrained to The Fool's generated path; custom
...
/// name (e.g. `"foolrs"`). Falls back to the agent type's serde name
...
impl ConversationService {
    async fn resolve_mcp_support_policy(
        ...
        match agent_type {
            ...
            AgentType::Foolrs => Ok(McpSupportPolicy::FOOLRS),
            _ => Ok(McpSupportPolicy::FOOLRS),
        }
    }
}
...
fn classify_repo_mcp_status(row: &fool_db::models::McpServerRow, support: McpSupportPolicy) -> ConversationMcpStatus {
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::Fool).unwrap(), "fool");
    ...
    fn response_with_type(agent_type: fool_common::AgentType) -> ConversationResponse {
    ...
    #[test]
    fn assistant_lineage_extracts_foolrs_preset_id() {
        use fool_common::AgentType;
        let response = response_with_type(AgentType::Foolrs);
        let extra = json!({ "preset_assistant_id": "preset-xyz" });
        let lineage = AssistantLineage::from_response_and_extra(&response, &extra);
        assert_eq!(lineage.agent_type, "foolrs");
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_fool_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
            McpSupportPolicy::FOOLRS,
        );
