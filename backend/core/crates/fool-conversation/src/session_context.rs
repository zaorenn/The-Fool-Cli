...
use fool_api_types::{AcpBuildExtra, FoolrsBuildExtra, TeamSessionBinding};
...
impl<'a> SessionContextBuilder<'a> {
    ...
    pub(crate) async fn build_options(
        &self,
        row: &ConversationRow,
        seed: Option<FoolrsRuntimePermissionSeed>,
    ...
    async fn build_kind(
        ...
        match agent_type {
            ...
            AgentType::Foolrs => Ok(AgentSessionKind::Foolrs(Box::new(build_foolrs_context(
                row, extra, team, seed,
            )))),
            AgentType::Gemini
            | AgentType::Codex
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&fool_db::models::AcpSessionRow>,
...
///   sessions.
#[derive(Debug, Clone)]
pub(crate) struct FoolrsRuntimePermissionSeed {
...
fn build_foolrs_context(
    ...
    extra: serde_json::Value,
    team: Option<TeamSessionBinding>,
    permission_seed: Option<FoolrsRuntimePermissionSeed>,
) -> FoolrsSessionBuildContext {
    let mut config: FoolrsBuildExtra = match serde_json::from_value(extra.clone()) {
        Ok(config) => config,
        Err(err) => {
            warn!(
                conversation_id = %row.id,
...
fn apply_runtime_permission_seed(
    seed: FoolrsRuntimePermissionSeed,
    row: &ConversationRow,
    config: &mut FoolrsBuildExtra,
) {
...
fn apply_team_seed_to_foolrs_config(team: &Option<TeamSessionBinding>, config: &mut FoolrsBuildExtra) {
    let Some(team) = team else {
        return;
    };
    if config.backend.as_deref().is_none_or(str::is_empty) {
...
fn decode_persisted_session_state(state: fool_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    fn foolrs_context(context: AgentSessionContext) -> FoolrsSessionBuildContext {
        match context.kind {
            AgentSessionKind::Foolrs(foolrs) => *foolrs,
            other => panic!("expected Foolrs context, got {other:?}"),
        }
    }
    ...
    fn workspace_validation_failure_logs_redacted_runtime_check() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let raw_path = "/tmp/fool-secret-workspace-token-12345";
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            ("codex", serde_json::json!({ "workspace": "/tmp/fool-codex-history" })),
            (
                "openclaw-gateway",
    ...
    fn foolrs_seed(mode: &str, resolved: Option<&str>) -> FoolrsRuntimePermissionSeed {
        FoolrsRuntimePermissionSeed {
