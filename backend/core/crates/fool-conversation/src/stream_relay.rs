...
use fool_ai_agent::protocol::events::{ErrorEventData, TipType};
use fool_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use fool_api_types::{AgentErrorCode, WebSocketMessage};
use fool_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
use fool_db::IConversationRepository;
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(fool_ai_agent::protocol::events::FinishEventData::default()),
...
mod tests {
    ...
    use fool_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    use fool_db::DbError;
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use fool_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn acp_dialect_signal_is_not_forwarded_or_persisted() {
        use fool_ai_agent::protocol::events::{AcpDialectSignalData, AcpDialectSignalKind};
    ...
    fn stream_relay_emits_feedback_runtime_terminal_log() {
        ...
        assert!(captured.contains("fool_feedback_diagnostics"), "{captured}");
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(fool_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use fool_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use fool_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(fool_realtime::BroadcastEventBus::new(64));
        let adapter =
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, fool_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, fool_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _user_id: &str, _id: &str) -> Result<Option<fool_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn owner_user_id(&self, _id: &str) -> Result<Option<String>, DbError> {
            Ok(Some("user-1".into()))
        }
        async fn create(&self, _row: &fool_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(
            &self,
            _user_id: &str,
            _id: &str,
            _updates: &fool_db::ConversationRowUpdate,
        ) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &fool_db::ConversationFilters,
        ) -> Result<fool_common::PaginatedResult<fool_db::models::ConversationRow>, DbError> {
            Ok(fool_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<fool_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<fool_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            ...
            _user_id: &str,
            _conv_id: &str,
            _params: &fool_db::MessagePageParams,
        ) -> Result<fool_db::MessagePageResult, DbError> {
            Ok(fool_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(
            ...
            _conversation_id: &str,
            id: &str,
            updates: &fool_db::MessageRowUpdate,
        ) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<fool_common::PaginatedResult<fool_db::MessageSearchRow>, DbError> {
            Ok(fool_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
