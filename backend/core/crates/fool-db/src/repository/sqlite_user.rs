...
impl IUserRepository for SqliteUserRepository {
    ...
    async fn create_user(&self, username: &str, password_hash: &str) -> Result<User, DbError> {
        let id = fool_common::generate_prefixed_id("user");
        let now = fool_common::now_ms();
    ...
    async fn adopt_system_default_data(&self, owner_id: &str) -> Result<u64, DbError> {
        ...
        // adoption-coverage classification test in fool-db/tests.
        let mut moved: u64 = 0;
...
mod tests {
    ...
    async fn ensure_external_user_is_idempotent_and_has_no_password() {
        ...
        let first = repo
            .ensure_external_user(UserType::Pro, "external-123", projection)
            .await
            .unwrap();
        let second = repo
        ...
        assert!(repo.find_by_username("Pro User").await.unwrap().is_none());
    }
    ...
    async fn adopt_moves_legacy_data_to_sole_external_user_once() {
        ...
        let user = repo
            .ensure_external_user(UserType::Pro, "ext-1", ExternalUserProjection::default())
            .await
            .unwrap();
    ...
    async fn adopt_is_refused_once_a_second_external_user_exists() {
        ...
        let first = repo
            .ensure_external_user(UserType::Pro, "ext-a", ExternalUserProjection::default())
            .await
            .unwrap();
        let second = repo
            .ensure_external_user(UserType::Pro, "ext-b", ExternalUserProjection::default())
            .await
            .unwrap();
    ...
    async fn adopt_is_refused_for_local_or_unknown_owner() {
        ...
        repo.ensure_external_user(UserType::Pro, "ext-c", ExternalUserProjection::default())
            .await
            .unwrap();
    ...
    async fn adopt_skips_rows_colliding_with_the_new_owner() {
        ...
        let now = fool_common::now_ms();
        let user = repo
            .ensure_external_user(UserType::Pro, "ext-d", ExternalUserProjection::default())
            .await
            .unwrap();
