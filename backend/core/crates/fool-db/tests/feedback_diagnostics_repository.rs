...
async fn insert_feedback_fixture(db: &fool_db::Database) {
    let pool = db.pool();
    sqlx::query(
...
async fn insert_mcp_feedback_fixture(db: &fool_db::Database) {
...
async fn insert_foreign_agent_metadata(db: &fool_db::Database) {
    sqlx::query(
        "INSERT INTO users \
         ...
         VALUES (?, 'pro', ?, ?, ?, NULL, NULL, NULL, 'active', 0, ?, ?, NULL)",
    )
    .bind("user-b")
