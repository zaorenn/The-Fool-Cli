...
async fn system_user_has_valid_timestamps() {
    let before = fool_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = fool_common::now_ms();
...
fn copy_legacy_noop_when_no_legacy_db() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("foolcore.db");
...
fn copy_legacy_noop_when_target_exists() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("foolcore.db");
    let legacy = dir.path().join("fool.db");
...
// `foolcore doctor` racing the server) both decide to apply the same
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("foolcore.db");
    ...
    let lock = path.with_file_name("foolcore.db.migrate.lock");
