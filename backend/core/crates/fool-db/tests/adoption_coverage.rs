//! Pro adoption coverage gate.
//!
//! `adopt_system_default_data` re-owns the local default user's data to the
//! machine's first external (Pro) account by convention: any table with a
...
#[tokio::test]
async fn every_table_is_classified_for_pro_adoption() {
    ...
    assert!(
        ...
        "NEW TABLE(S) WITHOUT AN ADOPTION DECISION: {unclassified:?}\n\
         ...
         See crates/fool-db/tests/adoption_coverage.rs."
    );
