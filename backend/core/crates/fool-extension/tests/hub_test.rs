...
use fool_extension::hub::{HubIndexManager, HubInstaller};
use fool_extension::registry::ExtensionRegistry;
use fool_extension::state::ExtensionStateStore;
use fool_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![fool_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: fool_extension::types::ExtensionSource::Env,
    }];
...
async fn verify_installation_catches_invalid_manifest() {
    ...
    std::fs::create_dir_all(&ext_dir).unwrap();
    std::fs::write(ext_dir.join("fool-extension.json"), b"not valid json").unwrap();
...
async fn verify_installation_catches_reserved_name() {
    let h = setup();
    let ext_dir = h.hub_dir.path().join("fool-bad");
    write_extension_manifest(&ext_dir, "fool-bad", "1.0.0");
