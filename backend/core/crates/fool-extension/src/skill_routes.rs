...
/// `POST /api/skills/market/enable` — enable the fool skills market.
...
/// `POST /api/skills/market/disable` — disable the fool skills market.
...
mod tests {
    ...
    async fn make_state() -> SkillRouterState {
        ...
        let skill_repo = Arc::new(fool_db::SqliteSkillRepository::new(db.pool().clone()));
        std::mem::forget(tmp);
        SkillRouterState {
            skill_paths: paths,
            skill_repo,
