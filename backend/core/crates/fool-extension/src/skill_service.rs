...
use fool_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `fool-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`FOOL_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../fool-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "FOOL_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    /// Root data directory (~/.fool/).
    pub data_dir: PathBuf,
    /// User-created skills directory (~/.fool/skills/).
    pub user_skills_dir: PathBuf,
    /// Per-job cron skills directory (~/.fool/cron/skills/).
    ...
    /// Assistant-level rules directory (~/.fool/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.fool/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.fool/`) and
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = fool_common::generate_prefixed_id("skill_import_op");
...
/// Startup sync for Pro machines: built-in skills only.
///
...
async fn sync_builtin_skills_into_repo(paths: &SkillPaths, repo: &dyn ISkillRepository) -> Result<(), ExtensionError> {
    ...
    // owned by the job owner and maintained by fool-cron, never synced
    // here as global rows.
...
mod tests {
    use super::*;
    use fool_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    use std::io::Write;
