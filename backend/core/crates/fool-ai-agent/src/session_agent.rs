//! `SessionAgentTask` — adapts the clean-slate `fool_session::SessionBackend`
...
use fool_common::{AgentKillReason, ConversationStatus, TimestampMs, now_ms};
use fool_session::{
...
use fool_db::{IAcpSessionRepository, IMcpServerRepository, SaveRuntimeStateParams};
...
fn resolve_current_model_efforts(models: &[fool_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `fool_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<fool_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<fool_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &fool_api_types::AgentHandshake) -> Self {
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &fool_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ) -> Arc<Self> {
        Self::build(
            agent_type,
    ...
    /// generic allow/deny.
    pub fn get_confirmations(&self) -> Vec<fool_common::Confirmation> {
        self.backend
            ...
            .map(|p| {
                ...
                fool_common::Confirmation {
                    ...
                    options: options
                        .into_iter()
                        .map(|o| fool_common::ConfirmationOption {
                            label: o.name,
    ...
    pub fn confirm(
        ...
        use fool_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<fool_session::ModelInfo>,
        Option<String>,
        Vec<fool_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<fool_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(fool_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<fool_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(fool_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<fool_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| fool_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(fool_api_types::GetModelInfoResponse {
            model_info: Some(fool_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<fool_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(fool_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| fool_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        // path's `thought_level` config option (category-keyed so The Fool's
        ...
        if !efforts.is_empty() {
            config_options.push(fool_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| fool_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(fool_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<fool_api_types::SetConfigOptionResponse, AgentError> {
        ...
        match option_id {
            ...
            _ => {
                return Ok(fool_api_types::SetConfigOptionResponse {
                    confirmation: fool_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(fool_api_types::SetConfigOptionResponse {
            confirmation: if observed {
                fool_api_types::ConfigOptionConfirmation::Observed
            } else {
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<fool_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| fool_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: fool_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    pub metadata: &'a fool_api_types::AgentMetadata,
    ...
    /// The conversation runtime context env (`FOOL_USER_ID` /
    /// `FOOL_CONVERSATION_ID` / `FOOL_HELPER_BIN` / `FOOL_BASE_URL` /
    /// `FOOL_RUNTIME_TOKEN`, filled by `apply_conversation_runtime_context`).
...
fn spec_mode_model(
    ...
    metadata: &fool_api_types::AgentMetadata,
) -> (fool_session::SessionSpec, Option<String>, Option<String>) {
    use fool_session::SessionSpec;
    ...
    // transform the ACP path applies in `initial_mode_from_params`. The Fool persists
...
/// assembly (`crates/fool-app/src/session_runtime/mod.rs`): it resolves the
...
pub async fn build_session_instance(
    ...
    spawner: Arc<dyn fool_process::Spawner>,
    ...
    let mut session_config = SessionConfig {
        ...
        cli_program: fool_runtime::resolve_bundled_cli(backend_label)
            .or_else(|| fool_runtime::resolve_command_path(backend_label)),
        ..Default::default()
    };
    ...
    if backend_label == "claude" {
        ...
        if !provider_env.is_empty() {
            ...
            session_config.spawn_env.extend(
                provider_env
                    .into_iter()
                    .map(|(name, value)| fool_common::EnvVar { name, value }),
            );
    ...
    let backend = connection
        ...
        .map_err(|e| match e {
            ...
            fool_session::BackendError::WorkspaceUnavailable(path) => {
...
///     path injected these via `resolve_agent_command_spec`; `FOOL_*`/`PATH`/…
///     keys are already filtered at the registry (`is_blocked_override_env_key`),
///     so they cannot shadow the runtime context below.
///  2. the `FOOL_*` conversation runtime context (`FOOL_USER_ID` /
///     `FOOL_CONVERSATION_ID` / `FOOL_HELPER_BIN` / `FOOL_BASE_URL` /
///     `FOOL_RUNTIME_TOKEN`) — the legacy path appended these via
///     `apply_acp_launch_policy` for every agent spawn.
fn assemble_spawn_env(
    agent_env: &[fool_api_types::AgentEnvEntry],
    runtime_env: &[(String, String)],
) -> Vec<fool_common::EnvVar> {
    let mut env: Vec<fool_common::EnvVar> = agent_env
        .iter()
        ...
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| fool_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &fool_session::SessionConfig) -> serde_json::Value {
    use fool_session::McpTransport;
...
fn session_server_to_spec(server: &fool_api_types::SessionMcpServer) -> fool_session::McpServerSpec {
    use fool_api_types::SessionMcpTransport as T;
    use fool_session::{McpServerSpec, McpTransport};
...
fn team_mcp_server_spec(cfg: &fool_api_types::TeamMcpStdioConfig) -> fool_session::McpServerSpec {
    use fool_api_types::TeamMcpStdioConfig as C;
    fool_session::McpServerSpec {
        name: fool_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
...
pub fn spawn_catalog_writeback(
    agent_id: String,
    user_id: String,
    backend: Arc<dyn fool_session::SessionBackend>,
...
/// going empty).
fn catalog_partial_from_caps(caps: &fool_session::Capabilities) -> Option<fool_api_types::AgentHandshake> {
    let mut config_options = Vec::new();
    ...
    Some(fool_api_types::AgentHandshake {
        config_options,
        available_modes,
        available_models,
        available_commands,
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        while let Some(env) = events.next().await {
            ...
            {
                let mut config_options: Vec<fool_api_types::AcpConfigOptionDto> = Vec::new();
                if !modes.is_empty() {
                    config_options.push(fool_api_types::AcpConfigOptionDto {
                        id: "mode".into(),
                        name: Some("Mode".into()),
            ...
            if let SessionEvent::SubagentUpdate { r#ref, status, .. } = &env.event {
                use fool_session::SubagentStatus;
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, fool_session::TurnOutcome::Cancelled { .. }));
...
/// `Orchestrator` uses (`fool_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use fool_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = fool_session::ErrorReason::Backend {
...
fn empty_turn_tip(outcome: &fool_session::TurnOutcome) -> Option<TipsEventData> {
    use fool_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, fool_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match fool_session::crash_outcome(terminal_result_seen, exit) {
                fool_session::Outcome::Crashed => {
                ...
                fool_session::Outcome::CleanNoResult | fool_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        fool_session::PlanStatus::Pending => "pending",
                        fool_session::PlanStatus::InProgress => "in_progress",
                        fool_session::PlanStatus::Completed => "completed",
                    };
        ...
        SessionEvent::Notice { level, message } => {
            let tip_type = match level {
                fool_session::NoticeLevel::Info => TipType::Info,
                fool_session::NoticeLevel::Warning => TipType::Warning,
            };
...
mod build_mapping_tests {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        ...
        let runtime_env = vec![
            ("FOOL_USER_ID".to_owned(), "user-1".to_owned()),
            ("FOOL_RUNTIME_TOKEN".to_owned(), "tok".to_owned()),
        ];
        ...
        assert_eq!(
            names,
            ["AWS_PROFILE", "HTTPS_PROXY", "FOOL_USER_ID", "FOOL_RUNTIME_TOKEN"]
        );
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use fool_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> fool_api_types::AgentMetadata {
        use fool_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use fool_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use fool_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = fool_session::Capabilities {
        ...
        let empty = fool_session::Capabilities::default();
        assert!(
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use fool_session::PermissionKind;
    ...
    fn plan_surfaces_with_snake_case_status() {
        use fool_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use fool_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use fool_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use fool_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: fool_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    use super::*;
    use fool_db::{CreateAcpSessionParams, IAcpSessionRepository, SqliteAcpSessionRepository, init_database_memory};
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, fool_db::Database) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(fool_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    async fn set_config_option_effort_returns_observed_via_override() {
        ...
        assert!(
            matches!(resp.confirmation, fool_api_types::ConfigOptionConfirmation::Observed),
    ...
    fn is_dead_resume_anchor_matrix() {
        use fool_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    async fn config_change_emits_no_frame_and_bound_id_reaches_finish() {
        let script = vec![
            ...
            env(SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: fool_session::TurnOutcome::EndTurn,
            }),
        ];
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use fool_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use fool_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use fool_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        ...
        let script = vec![
            env(SessionEvent::ToolCall {
                ...
                name: "Task".into(),
                subagent: fool_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
    ...
    struct PendingPermBackend(fool_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<fool_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    async fn get_confirmations_recovers_pending_ask_user_question() {
        let backend: Arc<dyn SessionBackend> = Arc::new(PendingPermBackend(fool_session::PendingPermissionView {
    ...
    fn handshake_with_catalog() -> fool_api_types::AgentHandshake {
        fool_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = fool_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
...
mod force_kill_tests {
    ...
    use crate::types::SendMessageData;
    use fool_common::{AgentKillReason, ConversationStatus};
    use fool_session::{
        Admission, BackendError, Capabilities, Command, CommandReceipt, SessionBackend, SessionEnvelope,
    };
    use futures_util::stream::BoxStream;
