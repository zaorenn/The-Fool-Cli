    ...
    pub fn apply_conversation_runtime_context(
        ...
        self.context.runtime_env.retain(|(key, _)| {
            !matches!(
                key.as_str(),
                FOOL_USER_ID_ENV
                    | FOOL_CONVERSATION_ID_ENV
                    | FOOL_HELPER_BIN_ENV
                    | FOOL_BASE_URL_ENV
                    | FOOL_RUNTIME_TOKEN_ENV
            )
        });
        self.context
            .runtime_env
            .push((FOOL_USER_ID_ENV.to_owned(), user_id.to_owned()));
        self.context
            .runtime_env
            .push((FOOL_CONVERSATION_ID_ENV.to_owned(), conversation_id.to_owned()));
        if let Some(helper_bin) = helper_bin {
            self.context
                .runtime_env
                .push((FOOL_HELPER_BIN_ENV.to_owned(), helper_bin.to_owned()));
        }
        if let Some(base_url) = base_url {
            self.context
                .runtime_env
                .push((FOOL_BASE_URL_ENV.to_owned(), base_url.to_owned()));
        }
        if let Some(runtime_token) = runtime_token {
            self.context
                .runtime_env
                .push((FOOL_RUNTIME_TOKEN_ENV.to_owned(), runtime_token.to_owned()));
        }
        self.runtime_capabilities.conversation_runtime_context_version = Some(CONVERSATION_RUNTIME_CONTEXT_VERSION);
    }
}

pub const FOOL_USER_ID_ENV: &str = "FOOL_USER_ID";
pub const FOOL_CONVERSATION_ID_ENV: &str = "FOOL_CONVERSATION_ID";
pub const FOOL_HELPER_BIN_ENV: &str = "FOOL_HELPER_BIN";
pub const FOOL_BASE_URL_ENV: &str = "FOOL_BASE_URL";
pub const FOOL_RUNTIME_TOKEN_ENV: &str = "FOOL_RUNTIME_TOKEN";
...
pub struct FoolrsCompatOverrides {
...
/// Fully resolved Foolrs configuration passed to the agent manager.
#[derive(Debug, Clone)]
pub struct FoolrsResolvedConfig {
    ...
    /// Normal The Fool conversations leave this unset.
    ...
    pub compat_overrides: FoolrsCompatOverrides,
    /// Directory for foolrs session persistence files.
    ...
    pub bedrock_config: Option<foolrs_config::config::BedrockConfig>,
...
mod tests {
    use super::*;
    use fool_api_types::{AcpBuildExtra, AcpModelInfo, FoolrsBuildExtra, SlashCommandItem};
    use serde_json::json;
    ...
    fn build_task_options_applies_conversation_runtime_context_once() {
        ...
        use fool_common::{AgentType, ProviderWithModel};
        ...
        let context = AgentSessionContext {
            ...
            skills: vec![],
            runtime_env: vec![
                (FOOL_USER_ID_ENV.into(), "old-user".into()),
                (FOOL_CONVERSATION_ID_ENV.into(), "old-conv".into()),
                (FOOL_RUNTIME_TOKEN_ENV.into(), "old-token".into()),
        ...
        options.apply_conversation_runtime_context(
            "user-1",
            "conv-1",
            Some("/Applications/The Fool/foolcore"),
        ...
        assert_eq!(
            options
                ...
                .runtime_env
                .iter()
                .filter(|(key, _)| key == FOOL_USER_ID_ENV)
                .count(),
            1
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(FOOL_USER_ID_ENV.to_owned(), "user-1".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(FOOL_CONVERSATION_ID_ENV.to_owned(), "conv-1".to_owned()))
        );
        ...
        assert!(
            options
                .context
                .runtime_env
                .contains(&(FOOL_BASE_URL_ENV.to_owned(), "http://127.0.0.1:25808".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(FOOL_RUNTIME_TOKEN_ENV.to_owned(), "runtime-token-1".to_owned()))
        );
        assert_eq!(
            options
                .context
                .runtime_env
                .iter()
                .filter(|(key, _)| key == FOOL_RUNTIME_TOKEN_ENV)
                .count(),
            1
        );
    ...
    #[test]
    fn foolrs_build_extra_serde_defaults() {
        let json = json!({});
        let extra: FoolrsBuildExtra = serde_json::from_value(json).unwrap();
    ...
    #[test]
    fn foolrs_build_extra_serde_with_overrides() {
        let json = json!({
    ...
    #[test]
    fn foolrs_build_extra_serde_with_preset_rules() {
        let json = json!({
    ...
    #[test]
    fn foolrs_build_extra_accepts_frozen_skills_snapshot() {
        let json = json!({
            "preset_rules": "Rules",
            "skills": ["pdf", "cron"]
        });
