...
use fool_common::{AgentType, now_ms};
use fool_db::{AgentMetadataRow, IAgentMetadataRepository, UpdateAgentHandshakeParams};
...
impl AgentRegistry {
    ...
    pub async fn run_slow_probe_recheck(&self) {
        ...
        for id in pending {
            ...
            let params = fool_db::UpdateAgentAvailabilitySnapshotParams {
    ...
    /// `false`. Internal rows (e.g. the foolrs row) intentionally
...
fn decode_row(
    ...
    let inferred_team_capable = behavior_policy.supports_team
        || fool_common::constants::is_team_capable(backend_str, handshake.agent_capabilities.as_ref());
    ...
    let is_internal_foolrs_cli = is_internal_foolrs_cli(&meta);
    if is_internal_foolrs_cli && command_override.is_some() {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring command override for internal Fool CLI agent"
        );
    }
    let env_override = parse_env_override(&env_override_raw);
    if is_internal_foolrs_cli && env_override.as_ref().is_some_and(|entries| !entries.is_empty()) {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring environment overrides for internal Fool CLI agent"
        );
    }

    meta.has_command_override = command_override.is_some() && !is_internal_foolrs_cli;
    meta.env_override_key_count = env_override
        .as_ref()
        .filter(|_| !is_internal_foolrs_cli)
        .map(|v| v.iter().filter(|e| !is_blocked_override_env_key(&e.name)).count())
        .unwrap_or(0);

    if is_internal_foolrs_cli {
        meta.command = None;
    } else if let Some(path) = command_override {
        meta.command = Some(path);
    }
    if !is_internal_foolrs_cli && let Some(extra) = env_override {
        for entry in extra {
...
fn is_internal_foolrs_cli(meta: &AgentMetadata) -> bool {
    meta.agent_type == AgentType::Foolrs && meta.agent_source == AgentSource::Internal
}
...
/// unavailable rows go to `info!` so the default foolcore.log surfaces
...
/// agent's runtime environment or The Fool-internal wiring. Case-insensitive.
pub(crate) fn is_blocked_override_env_key(key: &str) -> bool {
    let upper = key.to_ascii_uppercase();
    if upper.starts_with("FOOL_") {
        return true;
    }
    matches!(
        upper.as_str(),
...
mod tests {
    use super::*;
    use fool_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database_memory};
    ...
    async fn codex_yolo_id_maps_to_agent_full_access() {
        ...
        // Legacy The Fool yolo aliases resolve to Codex's native
    ...
    /// (Fool CLI is `agent_source = internal` with no `command`).
    ...
    async fn list_all_filters_out_unavailable_rows() {
        ...
        // Fool CLI (internal, no spawn command) is always available.
        assert!(
            visible.iter().any(|m| m.agent_type == AgentType::Foolrs),
            "internal foolrs row should survive the filter"
        );
    }
    ...
    async fn list_by_agent_type_counts_seed_rows() {
        ...
        assert_eq!(count(AgentType::Foolrs), 1);
    }

    #[tokio::test]
    async fn foolrs_internal_row_is_available_without_command() {
        let reg = registry().await;
        let foolrs = reg
            .list_by_agent_type(AgentType::Foolrs)
            .await
            .into_iter()
            .next()
            .unwrap();
        assert_eq!(foolrs.agent_source, AgentSource::Internal);
        assert!(foolrs.command.is_none());
        assert!(foolrs.available);
    }
    ...
    async fn diagnostic_snapshot_pairs_rows_with_reasons() {
        ...
        // The internal foolrs row is always available — its reason
        // slot must be None (sanity check that "available" doesn't
        // accidentally co-occur with a reason).
        let foolrs = snapshot
            .iter()
            .find(|(m, _)| m.agent_type == AgentType::Foolrs)
            .expect("foolrs seed row");
        assert!(foolrs.0.available);
        assert!(foolrs.1.is_none());
    }
    ...
    #[test]
    fn decode_row_ignores_internal_foolrs_cli_overrides() {
        use fool_db::AgentMetadataRow;
        let row = AgentMetadataRow {
            id: "632f31d2".to_string(),
            user_id: None,
            icon: None,
            name: "Fool CLI".to_string(),
            name_i18n: None,
            description: None,
            description_i18n: None,
            backend: None,
            agent_type: "foolrs".to_string(),
        ...
        assert_eq!(meta.agent_type, AgentType::Foolrs);
