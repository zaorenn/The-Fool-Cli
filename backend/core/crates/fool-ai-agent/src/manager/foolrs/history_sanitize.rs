//! Sanitize a resumed foolrs session's message history before it is replayed
//! to a provider.
//!
//! Background: when the user clicks "Stop" on a tool-call mid-stream, foolrs
...
//! `FoolrsAgentManager`) so it can be unit-tested in isolation and so we do
