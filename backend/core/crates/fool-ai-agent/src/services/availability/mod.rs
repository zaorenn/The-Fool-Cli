...
use fool_db::{IProviderRepository, UpdateAgentAvailabilitySnapshotParams};
...
async fn run_probe(
    ...
    } else if meta.agent_type == AgentType::Foolrs {
...
fn explicit_probe_args(meta: &AgentMetadata) -> Result<Vec<String>, String> {
    if meta.agent_source == AgentSource::Builtin && meta.agent_source_info.bridge_binary.as_deref() == Some("npx") {
        ...
        return fool_runtime::pin_registry_npx_args(backend, &meta.args).map_err(|error| error.to_string());
    }
    Ok(meta.args.clone())
}
