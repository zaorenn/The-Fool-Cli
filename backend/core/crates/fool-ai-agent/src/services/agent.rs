...
//! `fool-conversation::ConversationService`, which dispatches through
...
use fool_api_types::{AgentLogoEntry, AgentManagementRow, ProviderHealthCheckRequest, ProviderHealthCheckResponse};
use fool_db::IProviderRepository;
...
impl AgentService {
    ...
    pub async fn list_agent_logos(&self) -> Result<Vec<AgentLogoEntry>, AgentError> {
        ...
        for agent in self.registry.list_all_including_hidden().await {
            ...
            // for backends without a vendor label (e.g. foolrs, where `backend`
    ...
    pub async fn set_agent_overrides(
        ...
        user_id: &str,
        id: &str,
        req: fool_api_types::SetAgentOverridesRequest,
        ...
        if (command_override.is_some() || has_env_override) && is_internal_foolrs_cli_row(&row) {
            return Err(AgentError::bad_request("Internal Fool CLI does not support overrides"));
        }
    ...
    pub async fn get_agent_overrides(
        ...
        user_id: &str,
        id: &str,
    ) -> Result<fool_api_types::AgentOverridesResponse, AgentError> {
        let row = self
            .registry
            .repo_handle()
            .get_for_user(user_id, id)
        ...
        let env_override = row
            .env_override
            .as_deref()
            .and_then(|s| serde_json::from_str::<Vec<fool_api_types::AgentEnvEntry>>(s).ok())
            .unwrap_or_default();

        Ok(fool_api_types::AgentOverridesResponse {
            command_override: if is_internal_foolrs_cli_row(&row) {
                None
            } else {
...
fn is_bridge_launched_row(row: &fool_db::AgentMetadataRow) -> bool {
    let Some(raw) = row.agent_source_info.as_deref() else {
        return false;
    };
    let Ok(info) = serde_json::from_str::<fool_api_types::AgentSourceInfo>(raw) else {
        return false;
    };
...
fn is_internal_foolrs_cli_row(row: &fool_db::AgentMetadataRow) -> bool {
    row.agent_type.eq_ignore_ascii_case("foolrs") && row.agent_source.eq_ignore_ascii_case("internal")
}
