...
impl ProviderHealthCheckService {
    ...
    fn resolve_probe_config(&self, row: &Provider, model_id: &str) -> Result<FoolrsResolvedConfig, AgentError> {
        let api_key = fool_common::decrypt_string(&row.api_key_encrypted, &self.encryption_key)
...
async fn run_probe(
    ...
    let mut engine = match build_probe_engine(config_extra).await {
        Ok(engine) => engine,
        Err(error) => {
            let message = format!("Foolrs probe bootstrap failed: {error}");
...
async fn build_probe_engine(config_extra: FoolrsResolvedConfig) -> Result<AgentEngine, AgentError> {
...
mod tests {
    ...
    fn test_service() -> ProviderHealthCheckService {
        ProviderHealthCheckService {
            ...
            data_dir: PathBuf::from("/tmp/foolcore-provider-health-test"),
        }
    }
