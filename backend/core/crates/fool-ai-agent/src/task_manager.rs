...
impl WorkerTaskManagerImpl {
    ...
    fn refresh_runtime_token_for_new_task(&self, options: &mut BuildTaskOptions) {
        ...
        options
            .context
            .runtime_env
            .retain(|(key, _)| key != FOOL_RUNTIME_TOKEN_ENV);
        options
            .context
            .runtime_env
            .push((FOOL_RUNTIME_TOKEN_ENV.to_owned(), issue.token));
    }
}
...
/// Wired up by `fool-app` so deleting a conversation tears down its
/// agent process. Without this hook, ACP/foolrs subprocesses keep
...
mod tests {
    ...
    use fool_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel};
    ...
    fn make_team_options_with_runtime_token(conversation_id: &str, runtime_token: &str) -> BuildTaskOptions {
        ...
        let team = fool_api_types::TeamSessionBinding {
            team_id: "team-1".into(),
            ...
            move |opts: BuildTaskOptions| {
                ...
                async move {
                    if let Some((_, token)) = opts
                        ...
                        .runtime_env
                        .iter()
                        .find(|(key, _)| key == FOOL_RUNTIME_TOKEN_ENV)
                    {
    ...
    fn collect_idle_finds_finished_and_warmup_only_stale_acp_tasks() {
        ...
        // Non-ACP (Foolrs) + Finished + old activity → should NOT be collected
        insert(
            "conv-foolrs",
            mock_instance(
                MockAgent::new("conv-foolrs", Some(ConversationStatus::Finished))
