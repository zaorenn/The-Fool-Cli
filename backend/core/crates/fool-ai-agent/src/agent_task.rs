...
use crate::manager::foolrs::FoolrsAgentManager;
...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<fool_api_types::AgentModeResponse, AgentError> {
...
pub enum AgentInstance {
    ...
    Foolrs(Arc<FoolrsAgentManager>),
    /// clean-slate direct-CLI session model (claude/codex only). Wraps an
    /// `fool_session::SessionBackend` via [`SessionAgentTask`]. Every other
...
impl AgentInstance {
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::Foolrs(m) => m.kill_and_wait(reason),
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::Foolrs(m) => m.confirm(msg_id, call_id, data, always_allow),
    ...
    pub fn check_approval(&self, action: &str, command_type: Option<&str>) -> bool {
        match self {
            Self::Acp(_) => false,
            Self::Foolrs(m) => m.check_approval(action, command_type),
            // Session (claude/codex) has no foolrs-style auto-approve list.
    ...
    pub fn get_session_key(&self) -> Option<String> {
        match self {
            Self::Acp(_) | Self::Foolrs(_) | Self::Session(_) => None,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_mode(&self) -> Result<fool_api_types::AgentModeResponse, AgentError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::Foolrs(m) => m.mode().await,
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AgentError> {
        match self {
            ...
            Self::Foolrs(_) => Ok(GetModelInfoResponse { model_info: None }),
    ...
    pub async fn get_config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        match self {
            ...
            Self::Foolrs(m) => m.config_options().await,
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Foolrs(m) => m.set_config_option(option_id, value).await,
    ...
    /// non-ACP variants (claude/codex `Session`, `Foolrs`, `Mock`) — the
    ...
    /// [`fool_common::normalize_keys_to_snake_case`] so keys land as
    /// `used` / `size` / `cost` to match the The Fool wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                fool_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
            Self::Foolrs(_) => Ok(None),
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AgentError> {
        match self {
            ...
            Self::Foolrs(m) => m.get_slash_commands().await,
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Foolrs(_) => Ok(SideQuestionResponse {
                status: "unsupported".into(),
                answer: None,
            }),
...
#[cfg(test)]
mod foolrs_config_option_tests {
    use std::sync::Arc;

    use fool_api_types::ConfigOptionConfirmation;
    ...
    async fn foolrs_instance() -> AgentInstance {
        let manager = FoolrsAgentManager::new("conv-foolrs-config".into(), "/project".into(), make_test_config(), None)
            .await
            .expect("foolrs manager should start in tests");
        AgentInstance::Foolrs(Arc::new(manager))
    }

    #[tokio::test]
    async fn foolrs_exposes_mode_as_config_option() {
        let instance = foolrs_instance().await;
        ...
        let mode = response
            ...
            .expect("foolrs should expose a mode config option");
    ...
    #[tokio::test]
    async fn foolrs_set_config_option_mode_switches_session_mode() {
        let instance = foolrs_instance().await;
    ...
    #[tokio::test]
    async fn foolrs_set_config_option_rejects_invalid_mode() {
        let instance = foolrs_instance().await;
    ...
    #[tokio::test]
    async fn foolrs_set_config_option_rejects_unavailable_option() {
        let instance = foolrs_instance().await;
...
mod required_full_auto_dispatch_tests {
    ...
    /// Non-ACP variants (proxy for claude/codex `Session`, `Foolrs`) never
