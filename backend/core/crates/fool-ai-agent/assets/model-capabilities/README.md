...
`image_input_models.json` is a hand-maintained The Fool Core asset. It is embedded at compile time and is never downloaded or refreshed at runtime. Its API roots mirror the fixed `base_url` presets in The Fool's `modelPlatforms.ts`; model IDs are maintained independently from `models.dev`.

The catalog is intentionally a positive allowlist:

- Match both the provider API root and the exact model ID.
- Add a model only when the provider's own documentation confirms image input on the API protocol used by The Fool Core.
...
- Keep an empty `models` array when the preset endpoint is known but no stable model ID can be positively verified for that endpoint. This still records the The Fool preset without claiming image support.
- Aggregator entries may be refreshed manually from that aggregator's own catalog. The reviewed result must be committed as a static snapshot; The Fool Core never fetches it at runtime.

Poe bot names and Ctyun deployment model IDs are account- or deployment-specific, so they intentionally have no static model entries. DeepSeek does not currently expose a positively verified image-input chat model on the corresponding The Fool preset endpoint.
...
- Provider presets: packages/desktop/src/renderer/utils/model/modelPlatforms.ts
