...
use fool_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use fool_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use fool_ai_agent::manager::acp::{AcpSession, SessionNewPreludeHook};
use fool_ai_agent::registry::AgentRegistry;
use fool_ai_agent::shared_kernel::ModelId;
use fool_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(fool_extension::resolve_skill_paths(tmp.path(), tmp.path()));
