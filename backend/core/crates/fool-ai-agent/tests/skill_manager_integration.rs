...
use fool_db::{ISkillRepository, SqliteSkillRepository, UpsertSkillParams, init_database_memory};
...
async fn discover_by_names_for_user_ignores_other_users_skills() {
    ...
    sqlx::query(
        "INSERT INTO users \
         ...
         VALUES ('user-b', 'pro', 'external-user-b', 'User B', NULL, NULL, NULL, NULL, 'active', 0, 1, 1, NULL)",
    )
    .execute(db.pool())
    .await
    .unwrap();
...
fn system_instructions_with_loaded_skills() {
    let skills = vec![fool_ai_agent::SkillDefinition {
        name: "helper".into(),
...
fn first_message_with_skills_index_for_acp() {
    let skills = vec![fool_ai_agent::SkillIndex {
        name: "review".into(),
