...
use fool_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use fool_ai_agent::manager::acp::AcpAgentManager;
use fool_ai_agent::registry::AgentRegistry;
use fool_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use fool_common::ConversationStatus;
use fool_db::{SqliteAgentMetadataRepository, init_database_memory};
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = fool_ai_agent::AcpBuildExtra {
        agent_id: None,
    ...
    let skill_paths = std::sync::Arc::new(fool_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = fool_ai_agent::AcpSkillManager::new(skill_paths);
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: fool_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), fool_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(fool_common::AgentKillReason::IdleTimeout)).unwrap();
