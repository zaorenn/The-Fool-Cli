...
//! - Foolrs stub returns appropriate errors
...
// ---------------------------------------------------------------------------
// Foolrs agent tests (real implementation with AgentEngine)
// ---------------------------------------------------------------------------

fn make_foolrs_config() -> FoolrsResolvedConfig {
...
async fn foolrs_agent_kill_succeeds() {
    let agent = FoolrsAgentManager::new("conv-1".into(), "/proj".into(), make_foolrs_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn foolrs_agent_confirm_succeeds() {
    let agent = FoolrsAgentManager::new("conv-1".into(), "/proj".into(), make_foolrs_config(), None)
        .await
        .unwrap();
    // `confirm` is an inherent method on `FoolrsAgentManager` (reached via
    // `AgentInstance::Foolrs(..)` in production); the test calls it
...
#[tokio::test]
async fn foolrs_agent_metadata() {
    let agent = FoolrsAgentManager::new("conv-abc".into(), "/work".into(), make_foolrs_config(), None)
        .await
        .unwrap();
...
fn agent_session_kind_is_limited_to_runnable_runtimes() {
    fn assert_runnable(kind: AgentSessionKind) {
        match kind {
            AgentSessionKind::Acp(_) | AgentSessionKind::Foolrs(_) => {}
        }
    }
...
#[tokio::test]
async fn collect_idle_ignores_foolrs_agent_type() {
    ...
    let make_opts = |agent_type: AgentType, id: &str| {
        let kind = match agent_type {
            ...
            AgentType::Foolrs => AgentSessionKind::Foolrs(Box::new(FoolrsSessionBuildContext {
                config: Default::default(),
                team: None,
                belongs_to_team: false,
            })),
    ...
    mgr.get_or_build_task("acp-1", make_opts(AgentType::Acp, "acp-1"))
        .await
        .unwrap();
    mgr.get_or_build_task("foolrs-1", make_opts(AgentType::Foolrs, "foolrs-1"))
        .await
        .unwrap();
