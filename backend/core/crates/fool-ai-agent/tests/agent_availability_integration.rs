...
use fool_ai_agent::{AgentRegistry, AgentService};
use fool_api_types::{AgentManagementStatus, AgentSnapshotCheckKind, AgentSnapshotCheckStatus};
use fool_db::{
...
async fn management_rows_derive_missing_available_and_unavailable_statuses() {
    ...
    repo.upsert(&custom_params(
        "agent-missing",
        "Missing Agent",
        "fool-missing-agent-binary",
        r#"{"binary_name":"fool-missing-agent-binary"}"#,
    ))
    .await
    .unwrap();
    repo.upsert(&custom_params(
...
async fn manual_health_check_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        ...
        "fool-definitely-missing-health-check-target",
        r#"{"binary_name":"fool-definitely-missing-health-check-target"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_enabled_toggle_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-toggle",
        "Target Toggle Agent",
        "fool-target-toggle-command",
        r#"{"binary_name":"fool-target-toggle-command"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_delete_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-delete",
        "Target Delete Agent",
        "fool-target-delete-command",
        r#"{"binary_name":"fool-target-delete-command"}"#,
    ))
    .await
    .unwrap();
