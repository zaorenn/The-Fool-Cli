...
use fool_ai_agent::AcpSessionSyncService;
use fool_ai_agent::AcpSkillManager;
...
use fool_common::{AgentType, ProviderWithModel, encrypt_string};
use fool_db::{
...
fn make_factory(
...
) -> fool_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(fool_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    let process_registry = Arc::new(fool_process::FileRegistryStore::new(tmp.path()));
    let session_spawner: Arc<dyn fool_process::Spawner> = Arc::new(fool_process::RealSpawner::new(
        process_registry,
        uuid::Uuid::now_v7(),
        fool_process::local_machine_id(tmp.path()),
    ));
...
fn make_foolrs_options(
    ...
    BuildTaskOptions::new(AgentSessionContext {
        ...
        runtime_env: vec![],
        team: None,
        kind: AgentSessionKind::Foolrs(Box::new(FoolrsSessionBuildContext {
            config,
            team: None,
