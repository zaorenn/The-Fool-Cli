...
//! backends. It lives in `fool-common` (below every consumer: `fool-session`,
//! `fool-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `fool-ai-agent::manager::acp::stderr_error_extractor` (git
