...
impl AgentType {
    ...
    pub fn supports_new_conversation(&self) -> bool {
        matches!(self, AgentType::Acp | AgentType::Foolrs)
    }
    ...
    pub fn native_skills_dirs(&self) -> Option<&'static [&'static str]> {
        match self {
            AgentType::Foolrs => Some(&[".foolrs/skills"]),
            AgentType::Acp
...
mod tests {
    ...
    fn test_agent_type_display_names() {
        ...
        assert_eq!(AgentType::Foolrs.display_name(), "The Fool CLI");
    ...
    fn test_agent_type_id_stability() {
        let id = AgentType::Foolrs.id();
        assert_eq!(id.len(), 8);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
        assert_eq!(AgentType::Foolrs.id(), AgentType::Foolrs.id());
    }
    ...
    fn agent_type_new_conversation_support_policy_is_explicit() {
        ...
        assert!(AgentType::Foolrs.supports_new_conversation());
    ...
    fn agent_type_deprecated_runtime_policy_matches_new_conversation_support() {
        ...
        assert!(!AgentType::Foolrs.is_deprecated_runtime());
    ...
    fn test_mcp_source_serde_roundtrip() {
        let cases = [
            ...
            (McpSource::Foolrs, r#""foolrs""#),
            (McpSource::Nanobot, r#""nanobot""#),
            (McpSource::Fool, r#""fool""#),
        ];
    ...
    fn agent_type_full_auto_mode_id_supports_non_acp_agents() {
        ...
        assert_eq!(AgentType::Foolrs.full_auto_mode_id(None), "yolo");
