...
///
/// Lifecycle: written once by `fool-app`'s `main()` before
...
pub fn init(data_dir: impl AsRef<Path>) {
        ...
        && existing != path
    {
        tracing::warn!(
            ...
            "fool_runtime::init called twice with different paths; keeping first"
        );
    }
}

/// Returns the root cache directory used for all fool runtime artifacts.
///
/// Priority:
/// 1. Path supplied via [`init`] (`{data_dir}/runtime`) when the backend
///    started with `--data-dir`.
/// 2. Platform cache dir (via `dirs::cache_dir()`):
///    - macOS:   `~/Library/Caches/fool/runtime`
///    - Linux:   `$XDG_CACHE_HOME/fool/runtime` (fallback `~/.cache/fool/runtime`)
///    - Windows: `%LOCALAPPDATA%\fool\runtime`
///
...
pub fn runtime_root() -> Option<PathBuf> {
    ...
    dirs::cache_dir().map(|d| d.join("fool").join("runtime"))
}
...
mod tests {
    ...
    fn runtime_root_ends_with_expected_suffix() {
        ...
        let tail: Vec<_> = root
            ...
            .collect();
        assert_eq!(tail, vec!["runtime".to_string(), "fool".to_string()]);
    }
    ...
    fn node_runtime_root_appends_node_directory() {
        ...
        assert_eq!(
            tail,
            vec!["node".to_string(), "runtime".to_string(), "fool".to_string()]
        );
    }
}
