...
mod tests {
    ...
    const CHILD_MARKER: &str = "FOOL_RUNTIME_AGENT_ENV_TEST_CHILD";
    ...
    async fn agent_process_env_merges_full_shell_env_and_cleans_pollution() {
        if std::env::var_os(CHILD_MARKER).is_none() {
            ...
            write_fake_shell(
                ...
                r#"#!/bin/sh
printf '%s\n' \
  'FOOL_SHELL_ONLY=from-shell' \
  'FOOL_OVERLAY=from-shell' \
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("FOOL_CURRENT_ONLY", "from-current")
                .env("FOOL_OVERLAY", "from-current")
        ...
        assert_eq!(value("FOOL_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("FOOL_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("FOOL_OVERLAY").as_deref(), Some("from-shell"));
