...
//! given conversation; otherwise an foolrs job that runs fine
...
//! we return an empty `ProviderWithModel`; non-foolrs factory branches
//! ignore the field, and the foolrs branch surfaces a clear "provider
...
/// `provider_id: ""`, `model: ""`, `use_model: None`. Non-foolrs factories
/// ignore the field, while the foolrs factory surfaces a clear "Provider
...
mod tests {
    ...
    /// label `"foolrs"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'foolrs' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "foolrs");
        assert_ne!(legacy_resolved.provider_id, "foolrs");
    }
}
