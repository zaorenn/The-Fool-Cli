...
// Priority: 1) FOOL_FONTS_DIR env var, 2) skills/canvas-design relative path, 3) system fonts
function getFontDir() {
  const candidates = [
    process.env.FOOL_FONTS_DIR,
    path.join(__dirname, '../../canvas-design/canvas-fonts'),
    path.join(process.env.HOME || '', 'Library/Application Support/The Fool/config/skills/canvas-design/canvas-fonts'),
    path.join(process.env.APPDATA || '', 'The Fool/config/skills/canvas-design/canvas-fonts'),
  ].filter(Boolean);
