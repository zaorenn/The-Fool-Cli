...
4. Never pass, inline, export, echo, or set any `FOOL_...` environment variable.
5. Commands must directly call `"$FOOL_HELPER_BIN" config cron current ...`.
...
1. Run `"$FOOL_HELPER_BIN" config cron current list`.
2. If the returned `data` array is empty, create the task with `"$FOOL_HELPER_BIN" config cron current create <<'JSON'`.
3. If one task exists and the user wants to change it, update that task with `"$FOOL_HELPER_BIN" config cron current update <<'JSON'`.
