...
| `ui.surfaceLayouts`          | `{"voice": "<layout id>"}`             | What shape each window is wearing — a separate question from colour. `voice` takes `instrument` (the shipped column) or `hud` (the dial), or the id of a layout the user saved. An id that no longer exists falls back to the default rather than failing.                                                                                                                                                                                                                                                                                           |
| `ui.layoutPresets`           | `{"<name>": {…}}`                      | Layouts the user built and named, keyed by the lower-cased name. Each is `{id, name, surface, builtin: false, options}`; `options` holds `shell`, `meter`, `panel`, `motion` and `density`. Saving one does not change what is on screen — write `ui.surfaceLayouts` for that.                                                                                                                                                                                                                                                                       |
| `ui.surfaceStyle`            | `{style, accent, tokens?}`             | What the interface is made of — a third question from colour and shape. `style` is one of `neu`, `glass`, `liquid`, `clay`, `aurora`, `brutal`, `minimal`; `accent` is a `#rrggbb` colour the whole palette is derived from; `tokens` holds only the dials moved away from that material's own — `edge`, `depth`, `spread`, `inner`, `blur`, `alpha`, `saturation`, `sheen`, `weight`, `tracking`, `leading`, `gap`, `bounce`, `lift`, `press`, `ambient`, `tint`. Prefer the `app_theme` tool, which moves a dial by name and by a sensible amount. |
| `ui.surfaceBackground`       | `{image, opacity, blur}`               | A picture behind the whole application. `image` is a base64 `data:image/...` URL and nothing else is accepted; `opacity` is 0 to 1 and decides how much of it shows through the material's ground; `blur` is 0 to 40 pixels. Do not write an image here by hand — it is set from the picker, which reduces the file first.                                                                                                                                                                                                                           |
| `voice.localSkills`          | `[{name, when, action}]`               | Things the user taught the assistant to do by itself, each `{id, name, when, action}`. `action` is `{kind: 'open-url', url}` or `{kind: 'open-path', path}` — web addresses only, absolute paths only, never a command. Anything else is dropped on read.                                                                                                                                                                                                                                                                                            |
| `voice.personas`             | `[{name, instructions}]`               | Assistants the user wrote, kept under names they chose. `name` is theirs and is how they ask for one out loud; `instructions` is the persona body, up to 4000 characters. Twelve at most, newest first. Saving one does not change who is answering — putting one on writes the instructions into the `customInstructions` field of `fool.voice`'s realtime block and sets that block's `personaPresetId` to `custom`.                                                                                                                               |
| `voice.orbSkin`              | string                                 | Which orb the pet window draws while a spoken conversation is running: `reactor` (the default) or `pulse`. An id that no longer exists falls back to the default rather than leaving an empty window, so an old value is safe to leave in place.                                                                                                                                                                                                                                                                                                     |
| `connectors.grants`          | `[{connector, capability, granted}]`   | What the user has allowed each connected service to be asked for, one answer per capability rather than one per service. `granted: false` is a refusal they gave, which is not the same as never having been asked — an absent entry means ask. Read and write are separate answers and neither implies the other. A capability the connector never declared is refused whatever is stored here.                                                                                                                                                     |
| `voice.memoryProposals`      | `[{target, line, evidence}]`           | Things the assistant thinks it learned about the user at the end of a conversation, waiting for them to agree. **Read, never write.** Nothing here has been agreed to, so nothing here is true yet; the user accepts or refuses each one in Settings → Memory, and only then does it reach `fool.voice.memory`.                                                                                                                                                                                                                                      |
| `voice.memoryRefusals`       | `["..."]`                              | Lines the user turned down, so the same one is not offered every evening. **Read, never write** — adding to it by hand silences an observation the user never saw.                                                                                                                                                                                                                                                                                                                                                                                   |
| `voice.curiosityRefusals`    | `["name", "work", …]`                  | Subjects the assistant has already had its one chance to ask the user about, from the fixed list `name`, `work`, `hours`, `project`, `register`, `never-aloud`. A subject is added when it is _asked_, not when it is answered, and is never asked again. **Read, never write** — writing one by hand takes away a question the user was never offered; clearing the list makes the assistant ask everything again.                                                                                                                                  |
| `fool.voice.memorySnapshots` | `[{memory, reason, takenAt}]`          | What the memory said before each of the last thirty changes, oldest first, so a wrong line can be taken back from Settings → Memory. **Read, never write** — an entry here is a memory the application will put back into force on one click, and clearing the list throws away every undo the user has.                                                                                                                                                                                                                                             |
| `permissions.userRules`      | `[{decision, tool, pattern?}]`         | What the user answered "always allow" to. Consulted after this application's own `deny` rules and before its allows, so a user rule can save keystrokes but cannot open a directory the defaults refuse. `decision` is `allow`, `ask` or `deny`; `pattern` is glob-shaped for a path and prefix-shaped for a command.                                                                                                                                                                                                                                |
| `workspaces.library`         | `{"<name>": {…}}`                      | The whole app aimed at one purpose, by name — layout, persona, agent and model bundled together. Each is `{id, name, description, builtin, layouts, voice, agent, skills, updatedAt}`. The shipped `default` is rebuilt from code and cannot be overwritten here. Never put a key in one: these are meant to be sent to other people.                                                                                                                                                                                                                |
| `workspaces.activeId`        | string                                 | Which workspace is in force. Writing it alone records the choice without applying it — the app applies a workspace when the user switches in Fool's Hub or says so out loud, so prefer that to writing this by hand.                                                                                                                                                                                                                                                                                                                                 |
