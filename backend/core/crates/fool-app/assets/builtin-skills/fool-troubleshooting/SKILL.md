---
name: fool-troubleshooting
description: >-
  Diagnose a running The Fool installation: inspect stuck or errored conversations, read provider health, scheduled task state, MCP server health, team member state, backend health, and foolcore logs. Use when the user reports The Fool is misbehaving, a conversation is stuck, an LLM/provider call is failing, a scheduled task did not run, an MCP server has no tools, a team member is hung, or they ask to troubleshoot The Fool.
---

# The Fool Troubleshooting

Use the bundled `foolcore diagnose` CLI for read-only troubleshooting. It uses
...
1. Use only `"$FOOL_HELPER_BIN" diagnose ...`.
...
4. Treat every command as read-only. To change The Fool configuration, use the
   separate `fool-config` skill.
...
```bash
"$FOOL_HELPER_BIN" diagnose capabilities
```
...
The CLI resolves `"current"` from `FOOL_CONVERSATION_ID`. Commands that read
the backend also use `FOOL_BASE_URL` and `FOOL_USER_ID` from the same
runtime context.

## Start Wide

For a vague "The Fool is broken" report, run:

```bash
"$FOOL_HELPER_BIN" diagnose overview
```
...
```bash
"$FOOL_HELPER_BIN" diagnose conversations get <<'JSON'
{
  "conversation_id": "current"
}
JSON
...
```bash
"$FOOL_HELPER_BIN" diagnose conversations messages <<'JSON'
{
  "conversation_id": "current",
  "limit": 30,
  "errors_only": true
...
```bash
"$FOOL_HELPER_BIN" diagnose providers summary
```
...
```bash
"$FOOL_HELPER_BIN" diagnose cron summary
```
...
```bash
"$FOOL_HELPER_BIN" diagnose mcp summary
```
...
```bash
"$FOOL_HELPER_BIN" diagnose teams summary
```
...
```bash
"$FOOL_HELPER_BIN" diagnose health
```
...
Tail logs when the runtime provides `FOOL_LOG_DIR`:

```bash
"$FOOL_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "lines": 100,
  "errors_only": true,
  "conversation_id": "current"
...
If `FOOL_LOG_DIR` is unavailable and the user gives a log directory, pass it
explicitly:

```bash
"$FOOL_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "log_dir": "/Users/alex/Library/Logs/The Fool",
  "lines": 100,
  "errors_only": true,
  "conversation_id": "conv_123"
}
...
```bash
"$FOOL_HELPER_BIN" diagnose http get <<'JSON'
{
  "path": "/api/teams",
...
- For configuration changes, switch to `fool-config`.
- For scheduled task creation or updates, use the `cron` skill or
  `fool-config` cron commands.
