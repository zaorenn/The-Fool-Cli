---
name: fool-webui-public
description: >-
  Expose the user's local The Fool WebUI to the public internet with a near-zero-effort flow. Detects whether the WebUI is running, guides the user to switch it on if needed (the only manual step), self-installs cloudflared cross-platform, opens a Cloudflare quick tunnel, verifies the public URL actually works, then explains the limitations honestly (temporary/random URL, must stay running, password is the only protection, traffic transits Cloudflare). Use whenever the user wants to reach their The Fool from outside the LAN, over the internet, or share a public link. Distinct from fool-webui-setup (which covers manual LAN / Tailscale / server config through the settings UI): this skill produces a one-click public link via an automatic tunnel.
---
...
# The Fool WebUI Public Access Guide

You help a user turn their local The Fool WebUI (LAN-only at best) into a public internet URL, with the user doing almost nothing. You have a shell (Bash) and run on the same machine as The Fool, so you do the work yourself. The user only does what you architecturally cannot: flip the WebUI toggle in the desktop UI.

## Core facts (verified, do not re-derive)

- The Fool WebUI is a local HTTP server on port 25808 (the foolcore default; override with the `--port` CLI flag — so don't hardcode it, confirm with the curl probe below). It has built-in user+password / JWT auth, which is the _only_ thing protecting it once it's on the public internet — so the password is load-bearing, not a formality (see Step 6 risks).
...
```bash
mkdir -p ~/.fool/tools && cd ~/.fool/tools
...
(Use the path to the binary you installed, e.g. `~/.fool/tools/cloudflared`.)
...
Retry 2-3 times with a few seconds between; a freshly created tunnel can return 530/000 for the first few seconds before the edge is ready. Consider it good only when you get 200. For extra confidence, confirm it is really The Fool:

```bash
curl -s --max-time 20 "<public-url>/" | grep -i "<title>The Fool</title>"
```
...
between the open internet and the user's The Fool. **Before** you give them the
...
- This is a Cloudflare _quick tunnel_ — meant for quick sharing and testing, not
...
- A random URL is _obscurity, not security_. It can leak through browser history,
...
> These `/api/webui/*` credential endpoints only work when foolcore runs in
> **local mode** (the normal desktop/Electron case, which is exactly the scenario
> this skill targets). If foolcore is deployed as a standalone server, they
