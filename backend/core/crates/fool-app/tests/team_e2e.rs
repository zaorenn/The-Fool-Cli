...
async fn ensure_default_team_agent_installed(services: &fool_app::AppServices) {
...
async fn mark_claude_backend_team_mcp_stdio_capable(services: &fool_app::AppServices) {
...
async fn tc6c_create_team_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("fool-team-missing-{}", fool_common::generate_short_id()));
