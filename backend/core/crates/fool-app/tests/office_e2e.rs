...
//! integration level in `fool-office/tests/`.
...
use fool_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use fool_office::{ConversionService, OfficeRouterState, OfficecliWatchManager, ProxyService, SnapshotService};
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use fool_office::error::OfficeError;
    use fool_office::types::DocType;
    use fool_office::{ProcessHandle, ProcessSpawner};
