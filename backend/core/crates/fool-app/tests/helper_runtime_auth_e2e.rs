//! Pro-mode auth for the in-conversation helper CLI (`foolcore config`).
//!
...
async fn build_pro_app() -> (axum::Router, fool_app::AppServices, String) {
    ...
    let provision_response = router
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(r#"{"user_type":"pro","username":"Helper Pro User"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn helper_token_channel_passes_auth_and_reaches_cron_domain() {
    let (app, services, user_id) = build_pro_app().await;
...
#[tokio::test]
async fn helper_without_token_is_rejected_in_pro_mode() {
    let (app, services, user_id) = build_pro_app().await;
    ...
    let resp = app
        .oneshot(
            Request::builder()
                ...
                .header("x-fool-user-id", &user_id)
                .header("x-fool-conversation-id", CONVERSATION_ID)
                .header("x-fool-runtime-token", &issue.token)
                .body(Body::from("{}"))
                .unwrap(),
        )
        .await
