...
//! at the service integration level in `fool-cron/tests/service_integration.rs`.
...
async fn ensure_conversation(services: &fool_app::AppServices, user_id: &str, conversation_id: &str, title: &str) {
...
fn cron_skill_does_not_instruct_agents_to_write_payload_files() {
    ...
    assert!(!skill.contains("/tmp/fool-cron"));
    assert!(!skill.contains("python3"));
    assert!(!skill.contains("fool_cron.py"));
    assert!(skill.contains("$FOOL_HELPER_BIN"));
...
async fn cj3b_create_accepts_workspace_with_whitespace_segment() {
    ...
    let (token, csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ensure_default_assistant(&mut app, &token, &csrf).await;
    let dir = std::env::temp_dir().join(format!("fool-cron-test-{}", fool_common::generate_short_id()));
...
async fn cj5b_run_now_legacy_workspace_with_whitespace_succeeds() {
    ...
    cron_repo
        .insert(&fool_db::models::CronJobRow {
