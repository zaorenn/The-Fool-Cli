...
//! `fool_app::create_router_with_states` against an in-memory DB.
...
async fn fixture_embedded() -> Fixture {
    ...
    fool_extension::materialize_if_needed(&data_dir, fool_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.fool/`,
    ...
    let skill_repo = Arc::new(fool_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    fool_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        !names.contains(&"fool-skills"),
        "fool-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
