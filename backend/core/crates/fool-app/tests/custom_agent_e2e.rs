...
//!
//! Gates the probe with FOOL_BYPASS_PROBE for happy paths so CI does not
//! need an ACP CLI installed. Unset bypass for the single test that
//! verifies the CLI-not-found path.
//!
//! Thread safety: `FOOL_BYPASS_PROBE` is a process-wide env var.  Tests
...
//
// Any test that reads or writes FOOL_BYPASS_PROBE must hold this lock for
...
async fn custom_agent_full_roundtrip() {
    ...
    unsafe {
        std::env::set_var("FOOL_BYPASS_PROBE", "1");
    }
    ...
    unsafe {
        std::env::remove_var("FOOL_BYPASS_PROBE");
    }
}
...
// probe, so they do not need FOOL_BYPASS_PROBE and do not need the lock.
...
async fn test_on_save_cli_not_found_blocks_upsert() {
    // Hold ENV_MUTEX to guarantee FOOL_BYPASS_PROBE is unset for the
    ...
    let (status, json) = create_agent(
        ...
        json!({
            "name": "bad",
            "command": "fool-definitely-nonexistent-xyz"
        }),
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
