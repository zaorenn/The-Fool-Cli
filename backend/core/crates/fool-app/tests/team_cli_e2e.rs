//! E2E coverage for the agent-facing `foolcore team` CLI fallback.
...
async fn tool_command_rejects_forged_identity_fields_before_http_call() {
    let mut child = team_command()
        .args(["send-message"])
        .env("FOOL_BASE_URL", "http://127.0.0.1:9")
        .env("FOOL_CONVERSATION_ID", "conv-1")
        .env("FOOL_USER_ID", "user-1")
        .env("FOOL_RUNTIME_TOKEN", "token-1")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
