//! E2E coverage for the agent-facing `foolcore diagnose` CLI.
...
async fn diagnose_health_reads_backend_health_from_runtime_base_url() {
    ...
    let output = diagnose_command()
        ...
        .env("FOOL_CONVERSATION_ID", "conv-health")
        .env("FOOL_USER_ID", "user-health")
        .output()
        .await
        .unwrap();
...
async fn diagnose_http_get_is_get_only_redacted_escape_hatch_for_api_paths() {
    ...
    let mut child = diagnose_command()
        .args(["http", "get"])
        .env("FOOL_BASE_URL", &base_url)
        .env("FOOL_CONVERSATION_ID", "conv-http")
        .env("FOOL_USER_ID", "user-http")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn diagnose_overview_aggregates_common_failure_signals() {
    ...
    let output = diagnose_command()
        .arg("overview")
        .env("FOOL_BASE_URL", &base_url)
        .env("FOOL_CONVERSATION_ID", "conv-overview")
        .env("FOOL_USER_ID", "user-overview")
        .output()
        .await
        .unwrap();
...
#[tokio::test]
async fn diagnose_logs_tail_reads_latest_foolcore_log_and_filters_errors() {
    let temp = tempfile::TempDir::new().unwrap();
    let log_dir = temp.path().join("2026").join("07").join("08");
    std::fs::create_dir_all(&log_dir).unwrap();
    std::fs::write(
        log_dir.join("2026-07-08.foolcore.log"),
...
fn builtin_troubleshooting_skill_uses_diagnose_cli_not_python_helper() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("assets/builtin-skills/fool-troubleshooting");
    let skill = std::fs::read_to_string(root.join("SKILL.md")).unwrap();

    for forbidden in ["python3", "fool_diag.py", "lsof", "ps -", "curl"] {
        assert!(
            !skill.contains(forbidden),
            "fool-troubleshooting skill must not mention {forbidden}"
        );
    }
    assert!(skill.contains("\"$FOOL_HELPER_BIN\" diagnose capabilities"));
    assert!(skill.contains("\"$FOOL_HELPER_BIN\" diagnose overview"));
    assert!(skill.contains("\"$FOOL_HELPER_BIN\" diagnose conversations get"));
    assert!(skill.contains("\"$FOOL_HELPER_BIN\" diagnose http get"));
}
