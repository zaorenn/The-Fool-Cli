//! Shared test helpers for fool-app E2E tests.
#![allow(dead_code)]
...
use fool_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use fool_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use fool_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use fool_file::FileService;
use fool_system::VersionCheckService;
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths, username: &str) {
    let repo = fool_db::SqliteSkillRepository::new(services.database.pool().clone());
    let user = services
        .user_repo
    ...
    fool_extension::sync_skill_catalog_into_repo_for_user(paths, &repo, &user.id)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(fool_shell::ShellService::new(std::sync::Arc::new(
        fool_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                fool_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, fool_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn fool_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> fool_common::AgentType {
        fool_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<fool_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> fool_common::TimestampMs {
        fool_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<fool_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: fool_ai_agent::types::SendMessageData,
    ) -> Result<(), fool_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), fool_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<fool_common::AgentKillReason>) -> Result<(), fool_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        ...
        Ok(fool_api_types::GetConfigOptionsResponse {
            config_options: vec![fool_api_types::AcpConfigOptionDto {
                ...
                options: vec![fool_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<fool_api_types::SetConfigOptionResponse, fool_ai_agent::AgentError> {
        Ok(fool_api_types::SetConfigOptionResponse {
            confirmation: fool_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![fool_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("fool-csrf-token="))
        .map(|s| {
            s.strip_prefix("fool-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("fool-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = fool_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
