...
//! mirroring `fool-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
use fool_app::{AppConfig, AppServices, create_router};
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(fool_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = fool_system::ClientPrefService::new(repo);
    let mut req = fool_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the fool-shell integration
