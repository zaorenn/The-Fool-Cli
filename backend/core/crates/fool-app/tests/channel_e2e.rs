...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn fool_db::IChannelRepository> =
        std::sync::Arc::new(fool_db::SqliteChannelRepository::new(pool));
    let pairing_svc = fool_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
