...
async fn update_conversation_workspace(services: &fool_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = fool_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
...
async fn upsert_artifact(services: &fool_app::AppServices, artifact: fool_db::ConversationArtifactRow) {
    let repo = fool_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(&artifact.conversation_id).await.unwrap().unwrap();
    fool_db::IConversationRepository::upsert_artifact(&repo, &user_id, &artifact)
        .await
        .unwrap();
}
