...
async fn user_id_for_username(services: &fool_app::AppServices, username: &str) -> String {
    services
        .user_repo
...
async fn management_list_returns_array() {
    ...
    assert!(agents.iter().any(|a| a["agent_type"] == "foolrs"));
}
...
async fn management_list_includes_missing_custom_agents() {
    ...
    repo.upsert_for_user(
        ...
        &UpsertAgentMetadataParams {
            ...
            agent_type: "acp",
            agent_source: "custom",
            agent_source_info: Some(r#"{"binary_name":"fool-missing-agent-binary"}"#),
            enabled: true,
            command: Some("fool-missing-agent-binary"),
            args: Some("[]"),
            env: Some("[]"),
