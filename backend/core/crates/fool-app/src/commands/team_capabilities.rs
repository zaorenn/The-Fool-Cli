use fool_api_types::{TEAM_TOOLS_SCHEMA_VERSION, team_tool_descriptors};
...
pub(crate) fn data() -> Value {
    ...
    json!({
        ...
        "commands": {
            ...
            "context": { "runtime_env_required": ["FOOL_BASE_URL", "FOOL_USER_ID", "FOOL_CONVERSATION_ID", "FOOL_RUNTIME_TOKEN"] },
            "tool_call": { "runtime_env_required": ["FOOL_BASE_URL", "FOOL_USER_ID", "FOOL_CONVERSATION_ID", "FOOL_RUNTIME_TOKEN"] }
        },
        "output_envelope": {
            "success": "boolean",
...
pub(crate) fn help_markdown() -> String {
    let mut text = String::from("# The Fool Core Team CLI\n\nUse `foolcore team capabilities` for exact schemas.\n\n");
    for tool in team_tool_descriptors() {
        ...
        text.push_str(&format!(
            "- `foolcore team {command}` -> `{}` ({permission}): {}\n",
