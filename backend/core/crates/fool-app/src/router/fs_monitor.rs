...
//!
//! Adapts the transport-agnostic [`FsMonitorActor`] (in `fool-project`) to the
...
use fool_project::monitor::{FsInbound, FsMonitorActor, FsWirePush};
use fool_realtime::{ConnectionId, MessageRouter, NoopMessageRouter, WebSocketManager};
...
#[cfg(test)]
mod tests {
    use fool_realtime::{PER_CONNECTION_BUFFER, WsOutbound};
    use serde_json::json;
    use tokio::sync::mpsc;
