...
use fool_db::models::{AgentMetadataRow, MessageRow};
use fool_db::{IAgentMetadataRepository, IConversationRepository};
use fool_team::{
...
impl NativeSlashCommandPort for TeamConversationAdapters {
    ...
    /// the SHARED grammar owner (`fool_session::slash_command_name`, same
    ...
    async fn recognize(&self, conversation_id: &str, content: &str) -> SlashCommandRecognition {
        let Some(name) = fool_session::slash_command_name(content) else {
...
impl TeamConversationAdapters {
    ...
    async fn resolve_slash_catalog(&self, conversation_id: &str) -> Option<(Vec<String>, SlashCatalogSource)> {
        ...
        {
            let names = fool_session::codex_capabilities()
                .slash_commands
                .into_iter()
