...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            host: fool_common::constants::DEFAULT_HOST.to_string(),
            port: fool_common::constants::DEFAULT_PORT,
...
pub fn derive_encryption_key(jwt_secret: &str) -> [u8; 32] {
    ...
    hasher.update(b"fool-encryption-key:");
...
mod tests {
    ...
    fn test_app_config_database_path() {
        let config = AppConfig {
            data_dir: PathBuf::from("/tmp/fool"),
            ..Default::default()
        };
        assert_eq!(config.database_path(), PathBuf::from("/tmp/fool/foolcore.db"));
    }
}
