...
/// (fool-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `FOOL_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("FOOL_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
/// its `McpServerConfig` map). Deliberately crate-local — `fool-session` stays
/// self-contained (no `fool-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// blob). Kept as `serde_json::Value` so `fool-session` need not depend on
    /// the `foolrs-agent` `Session` type; the foolrs backend deserializes it.
...
pub struct SessionConfig {
    ...
    pub spawn_env: Vec<fool_common::EnvVar>,
