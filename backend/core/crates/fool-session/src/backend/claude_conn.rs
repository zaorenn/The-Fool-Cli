...
/// Pure `serde_json`, no ACP SDK — `fool-session` stays SDK-free.
...
/// DIAGNOSTIC (env-gated, default OFF): when `FOOL_CLAUDE_WIRE_DUMP` is set, log
...
fn claude_wire_dump_enabled() -> bool {
    std::env::var("FOOL_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty())
}
                        ...
                        match tokio::time::timeout(std::time::Duration::from_millis(200), stdout.read(&mut chunk)).await {
                            ...
                            Ok(Ok(n)) if n > 0 => {
                                ...
                                match parsed {
                                    ...
                                    Err(_) => {
                                        tracing::error!(
                                            target: "fool_session::backend::claude_conn",
                                            conversation_id = %session_id,
...
mod tests {
    ...
    async fn f4_turn_in_flight_blocks_idle_suspend() {
        ...
        let suspended = backend
            .suspend
            .suspend_if_idle(fool_common::now_ms() + 10_000, true)
            .await;
