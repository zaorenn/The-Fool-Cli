...
use fool_process::{BoxedStdin, ProcessError, Spawner};
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let proc = spawner.spawn(spec, &[], "fool-session").await?;
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via FOOL_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("FOOL_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
        let preview = String::from_utf8_lossy(&bytes[..bytes.len().min(4096)]);
        tracing::info!(
            target: "fool_session::claude_wire",
            direction = "stdin",
            byte_len = bytes.len(),
            preview = %preview,
            "claude wire bytes"
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: fool_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
