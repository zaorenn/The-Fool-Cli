...
//! still can, via the `FOOL_BUILTIN_ASSISTANTS_PATH` env var → disk
//! fallback path.
...
/// `crates/fool-app/assets/builtin-assistants/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../fool-app/assets/builtin-assistants");
...
/// `FOOL_BUILTIN_ASSISTANTS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinAssistantRegistry {
    ...
    ///
    /// If `FOOL_BUILTIN_ASSISTANTS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("FOOL_BUILTIN_ASSISTANTS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "FOOL_BUILTIN_ASSISTANTS_PATH points to missing directory; \
    ...
    /// `FOOL_BUILTIN_ASSISTANTS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // FOOL_BUILTIN_ASSISTANTS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "FOOL_BUILTIN_ASSISTANTS_PATH";
