...
use fool_common::{generate_prefixed_id, now_ms};
use fool_db::{
...
use fool_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionError};
use serde_json;
...
fn assistant_error_is_busy(error: &AssistantError) -> bool {
    matches!(error, AssistantError::Internal(message) if fool_db::message_indicates_busy(message))
}
...
fn assistant_error_is_unique(error: &AssistantError) -> bool {
    match error {
        ...
        AssistantError::Internal(message) => fool_db::message_indicates_unique_violation(message),
        _ => false,
    }
}
...
pub struct AssistantService {
    ...
    /// Defaults to `~/.fool/` but can be overridden for tests.
...
impl AssistantService {
    ...
    /// directory.
    ///
    /// There is no implicit `~/.fool` fallback on purpose: an earlier
    /// version had one, and dev builds silently wrote rule files to the
    /// release directory while the db lived under `~/.fool-dev/`,
    ...
    /// Bootstrap for Pro machines: identical to
    ...
    ///    Bedrock, Vertex, …), return `"foolrs"`. FoolRS speaks both
    ...
    pub async fn resolve_default_agent_id_for_user(&self, user_id: &str) -> Result<String, AssistantError> {
        ...
        if providers.iter().any(|p| p.enabled) {
            self.resolve_agent_id_for_agent_ref(user_id, "foolrs").await
        } else {
    ...
    pub async fn import_for_user(
        ...
        for entry in req.assistants {
            ...
            match self.repo.create_for_user(user_id, &params).await {
                ...
                Err(fool_db::DbError::Conflict(_)) => {
    ...
    ///   override when `FOOL_BUILTIN_ASSISTANTS_PATH` is set).
    ...
    fn user_rules_dir_for_user(&self, user_id: &str) -> PathBuf {
        let dir = fool_common::user_dir_name(user_id).unwrap_or_else(|_| user_id.to_owned());
...
impl AssistantService {
    ...
    fn definition_to_detail_response(
        ...
        definition: &AssistantDefinitionRow,
        state: Option<&AssistantOverlayRow>,
        preference: Option<&fool_db::AssistantPreferenceRow>,
        rules_content: &str,
        projection: &AssistantRuntimeProjection,
    ) -> Result<AssistantDetailResponse, AssistantError> {
        let builtin_default = self.builtin_listing_default(definition);
...
fn assistant_projection_for_definition(
    ...
    // agents, but foolrs (the built-in Rust agent) has a NULL `backend` and is
    // keyed by its `agent_type` ("foolrs") instead. Match on either so foolrs
    // assistants resolve to the foolrs row rather than falling back to Missing.
...
mod tests {
    ...
    struct Fixture {
        ...
        override_repo: Arc<dyn IAssistantOverrideRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<fool_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
    ...
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<fool_api_types::AgentManagementRow>>>,
    }
    ...
    /// `resolve_default_agent_type` returns `"foolrs"`. Tests that need to
    ...
    struct FixtureOpts {
        ...
        /// Defaults to `"openai"` so existing tests get an `"foolrs"`
        /// default agent type.
        seed_platform: Option<&'static str>,
        agent_rows: Vec<fool_api_types::AgentManagementRow>,
    }
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: fool_api_types::AgentManagementStatus,
    ) -> fool_api_types::AgentManagementRow {
        fool_api_types::AgentManagementRow {
            id: id.into(),
            ...
            behavior_policy: fool_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(fool_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(fool_api_types::AgentSnapshotCheckKind::Manual),
    ...
    fn mk_uninstalled_agent_row(id: &str, backend: &str) -> fool_api_types::AgentManagementRow {
        let mut row = mk_agent_row(id, backend, fool_api_types::AgentManagementStatus::Unchecked);
        row.installed = false;
    ...
    async fn bootstrap_skips_dirty_generated_assistant_definitions() {
        let fx = fixture().await;
        {
            ...
            *rows = vec![
                mk_agent_row("agent-dirty", "dirty", fool_api_types::AgentManagementStatus::Online),
                mk_agent_row("agent-valid", "valid", fool_api_types::AgentManagementStatus::Online),
            ];
        }
    ...
    async fn builtin_listing_uses_manifest_default_enabled_and_sort_order() {
        ...
        let mut butler = mk_builtin("fool-assistant", "Butler");
        ...
        let butler_resp = list.iter().find(|a| a.id == "fool-assistant").unwrap();
        assert!(butler_resp.enabled, "butler defaults to enabled");
        assert_eq!(butler_resp.sort_order, 0);
        let butler_idx = list.iter().position(|a| a.id == "fool-assistant").unwrap();
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        ...
        assert_eq!(bare.agent_status, fool_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_unchecked_agent() {
        ...
        assert_eq!(bare.agent_status, fool_api_types::AgentManagementStatus::Unchecked);
        assert!(bare.team_selectable);
    ...
    async fn list_hides_existing_generated_assistant_when_agent_is_uninstalled_until_installed() {
        ...
        uninstalled_row.status = fool_api_types::AgentManagementStatus::Offline;
        let fx = fixture_with_options(FixtureOpts {
...

        {
            ...
            rows[0].status = fool_api_types::AgentManagementStatus::Unchecked;
        }
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = fool_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        // Engines like Fool CLI carry their identity in `agent_type` and leave
        ...
        let bare = list
            .iter()
            .find(|assistant| assistant.id == "bare:agent-foolrs")
            .unwrap();
        assert_eq!(bare.agent_id, "agent-foolrs");
    }

    #[tokio::test]
    async fn foolrs_assistant_resolves_agent_status_via_agent_type_not_backend() {
        // Regression: an assistant whose engine is foolrs must match the foolrs
        ...
        let mut builtin = mk_builtin("builtin-foolrs", "Fool Assistant");
        builtin.agent_ref = "foolrs".into();

        let fx = fixture_with_options(FixtureOpts {
            builtins: vec![builtin],
            agent_rows: vec![foolrs_row],
            ..Default::default()
        })
        .await;
        ...
        let assistant = list
            .iter()
            .find(|assistant| assistant.id == "builtin-foolrs")
            .expect("foolrs builtin assistant should be listed");
        assert_eq!(
            assistant.agent_status,
            fool_api_types::AgentManagementStatus::Online,
            "foolrs assistant should resolve to the online foolrs agent row, not Missing"
        );
    }
    ...
    async fn bootstrap_reactivates_soft_deleted_builtin_definition_by_source_ref() {
        let mut builtin = mk_builtin("fool-assistant", "The Fool Butler");
        builtin.rule_file = Some("rules/fool-assistant.{locale}.md".into());
        let fx = fixture_with_builtins(vec![builtin]).await;

        let original = fx
            .definition_repo
            .get_by_assistant_id("fool-assistant")
            .await
            .unwrap()
        ...
        assert_eq!(restored.rule_resource_ref.as_deref(), Some("fool-assistant"));
    ...
    /// Anthropic provider routes to FoolRS, not the Claude Code CLI:
    /// having an Anthropic API key does not imply the user has
    /// `claude` on `PATH`. CLI-based agents must be opted into
    /// explicitly.
    #[tokio::test]
    async fn resolve_default_agent_id_routes_anthropic_provider_to_foolrs() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// OpenAI / custom provider falls back to FoolRS, the only The Fool
    /// agent that doesn't require a third-party CLI.
    #[tokio::test]
    async fn resolve_default_agent_id_falls_back_to_foolrs_for_openai_provider() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// Custom (non-anthropic, non-openai) platform also routes to FoolRS,
    /// which handles OpenAI-compatible custom URLs.
    #[tokio::test]
    async fn resolve_default_agent_id_handles_custom_platform_as_foolrs() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// or otherwise — should resolve to `"foolrs"`, the only built-in
    ...
    async fn create_without_agent_id_does_not_default_to_gemini_when_provider_exists() {
        for platform in ["anthropic", "openai"] {
            ...
            assert_eq!(
                ...
                "{platform} provider should resolve to foolrs"
            );
        }
    }
