...
use fool_system::ClientPrefService;
...
///
/// Key mismatch fix: the The Fool frontend stores the config under
...
pub(crate) async fn load_stt_config(
    ...
    user_id: &str,
) -> Result<SpeechToTextConfig, fool_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: fool_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(fool_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = fool_system::ClientPrefService::new(repo);
