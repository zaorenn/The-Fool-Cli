...
const RAW_FOOLRS_PAYLOAD_TARGETS: &[&str] = &["aion_agent", "aion_providers"];
...
mod tests {
    ...
    fn env_filter_suppresses_raw_foolrs_provider_debug_even_when_debug_enabled() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            assert!(
                tracing::enabled!(target: "aionui_ai_agent::manager::foolrs::agent", Level::DEBUG),
                "AionUi foolrs lifecycle debug logs should still be available"
            );
        });
    }
