...
use fool_common::{PaginatedResult, ProviderWithModel, TimestampMs, now_ms};
use fool_conversation::ConversationService;
use fool_cron::{CronRouterState, cron_routes};
use fool_db::{
...
use fool_cron::scheduler::CronScheduler;
use fool_cron::service::{CronService, CronServiceDeps};
use fool_cron::types::CronAgentConfig;
use fool_cron::types::JobStatus;
...
async fn seed_sqlite_conversations(db: &fool_db::Database, conversation_ids: &[&str]) {
    sqlx::query(
    ...
    for conversation_id in conversation_ids {
        repo.create(&fool_db::models::ConversationRow {
...
impl fool_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<fool_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, fool_db::models::ConversationRow>>,
    assistant_snapshots: Mutex<HashMap<String, ConversationAssistantSnapshotRow>>,
    update_failures: Mutex<Vec<String>>,
    sqlite_pool: fool_db::SqlitePool,
}

impl StubConvRepo {
    fn new(sqlite_pool: fool_db::SqlitePool) -> Self {
        Self {
    ...
    async fn seed_sqlite_row(&self, row: &fool_db::models::ConversationRow) -> Result<(), fool_db::DbError> {
    ...
    fn upsert_artifact_row(&self, artifact: fool_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<fool_db::ConversationArtifactRow> {
    ...
    fn set_conversation_extra(&self, conversation_id: &str, extra: serde_json::Value) {
        ...
        let row = rows
            ...
            .or_insert_with(|| fool_db::models::ConversationRow {
                id: conversation_id.to_owned(),
                user_id: "u1".into(),
                name: "stub".into(),
                r#type: "default".into(),
...
impl IConversationRepository for StubConvRepo {
    async fn get(
        ...
        let row = if id == "conv_mode" {
            fool_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("fool-cron-service-gemini-workspace"),
                    "session_mode": "yolo",
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "hermes",
                    "agent_name": "Hermes",
                    "workspace": ensure_named_workspace_path("fool-cron-service-hermes-workspace"),
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("fool-cron-service-gemini-default-workspace"),
                    "session_mode": "default",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "codex",
                    "agent_name": "Codex",
                    "workspace": ensure_named_workspace_path("fool-cron-service-codex-workspace"),
                    "session_mode": "default",
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Claude",
                    "workspace": ensure_named_workspace_path("fool-cron-service-claude-workspace"),
                    "session_mode": "default",
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("fool-cron-service-stale-backend-workspace"),
                    "session_mode": "yolo",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
        ...
        } else if id == "conv_mode_foolrs" {
            fool_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
                name: "Foolrs Chat".into(),
                r#type: "foolrs".into(),
                model: Some(
                    serde_json::json!({
                ...
                extra: serde_json::json!({
                    "backend": "anthropic",
                    "agent_name": "Fool CLI",
                    "workspace": ensure_named_workspace_path("fool-cron-service-foolrs-workspace"),
                    "session_mode": "default",
                    "current_model_id": "claude-sonnet-4-20250514"
                })
                .to_string(),
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("fool-cron-service-assistant-stale-backend-workspace"),
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("fool-cron-service-missing-assistant-stale-backend-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
            ...
            fool_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("fool-cron-service-assistant-snapshot-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
    ...
    async fn create(&self, row: &fool_db::models::ConversationRow) -> Result<(), fool_db::DbError> {
    ...
    async fn delete_messages_by_conversation(&self, _user_id: &str, _conv_id: &str) -> Result<(), fool_db::DbError> {
        Ok(())
    }
    ...
    async fn list_artifacts(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<fool_db::ConversationArtifactRow>, fool_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<fool_db::ConversationArtifactRow>, fool_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        _user_id: &str,
        artifact: &fool_db::ConversationArtifactRow,
    ) -> Result<fool_db::ConversationArtifactRow, fool_db::DbError> {
...
async fn setup_with_conv_runtime_and_agent_metadata() -> (
    ...
    Arc<dyn fool_db::ISkillRepository>,
) {
    ...
    let data_dir = std::env::temp_dir().join(format!("fool-cron-test-{}", now_ms()));
    ...
    let task_manager: Arc<dyn fool_ai_agent::task_manager::IWorkerTaskManager> = Arc::new(StubTaskManager);
    ...
    let skill_repo: Arc<dyn fool_db::ISkillRepository> = Arc::new(SqliteSkillRepository::new(pool.clone()));
...
async fn setup_with_skill_repo() -> (CronService, Arc<dyn fool_db::ISkillRepository>) {
...
fn make_create_req(name: &str, schedule: CronScheduleDto) -> CreateCronJobRequest {
    CreateCronJobRequest {
        ...
        execution_mode: None,
        queue_enabled: false,
        agent_config: Some(fool_api_types::CronAgentConfigWriteDto {
...
async fn seed_bare_assistant_definitions(repo: &Arc<dyn IAssistantDefinitionRepository>) {
    for (definition_id, assistant_id, agent_backend) in [
        ...
        ("asstdef_bare_foolrs", "bare:632f31d2", "foolrs"),
    ] {
...
async fn create_job_strips_legacy_agent_ids_when_assistant_id_present() {
    ...
    req.agent_config = Some(fool_api_types::CronAgentConfigWriteDto {
        name: "Helper".into(),
        cli_path: None,
...
async fn create_job_requires_assistant_id_for_new_jobs() {
    ...
    assert!(matches!(err, fool_cron::error::CronError::InvalidAgentConfig(_)));
    assert!(err.to_string().contains("assistant_id is required for new cron jobs"));
}

#[tokio::test]
async fn create_job_derives_runtime_type_from_foolrs_assistant() {
    let (svc, _, _, _, definition_repo, _) = setup_with_assistant_repos().await;
    seed_assistant_definition(&definition_repo, "asstdef_runtime_foolrs", "assistant-foolrs", "foolrs").await;

    let mut req = make_create_req("Assistant Foolrs", every_60s());
    req.agent_config = Some(fool_api_types::CronAgentConfigWriteDto {
        name: "Foolrs Assistant".into(),
        cli_path: None,
        assistant_id: Some("assistant-foolrs".into()),
        mode: Some("yolo".into()),
    ...
    assert_eq!(job.agent_type, "foolrs");
}
...
async fn create_job_derives_runtime_type_from_assistant_overlay_override() {
    ...
    .await;
    seed_assistant_overlay(&overlay_repo, "asstdef_runtime_override", Some("foolrs")).await;
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(err, fool_cron::error::CronError::JobNotFound(_)));
}
...
async fn update_existing_conversation_job_rejects_agent_config_changes() {
    ...
    assert!(
        matches!(err, fool_cron::error::CronError::InvalidAgentConfig(message) if message.contains("ongoing conversation"))
    );
}
...
async fn update_existing_job_to_new_conversation_preserves_custom_workspace() {
    ...
    let custom_workspace = ensure_named_workspace_path(&format!("fool-cron-switch-custom-{conversation_id}"));
    conv_repo.set_conversation_extra(
        &conversation_id,
        serde_json::json!({
...
async fn update_team_conversation_job_rejects_execution_mode_change() {
    ...
    conv_repo.set_conversation_extra(
        ...
        serde_json::json!({
            "team_id": "team-1",
            "workspace": ensure_named_workspace_path("fool-cron-service-team-workspace")
        }),
    );
    let created = svc.add_job("u1", create_req).await.unwrap();
    ...
    let err = svc.update_job("u1", &created.id, req).await.unwrap_err();
    assert!(matches!(err, fool_cron::error::CronError::InvalidExecutionMode(message) if message.contains("Team")));
}
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        fool_cron::error::CronError::Database(fool_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(fool_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    let err = svc
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, fool_cron::error::CronError::InvalidSkillContent(_)));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    let err = svc.add_job("u1", req).await.unwrap_err();
    assert!(matches!(err, fool_cron::error::CronError::InvalidCronExpression(_)));
}
...
async fn sc7_every_zero_interval() {
    ...
    let err = svc.add_job("u1", req).await.unwrap_err();
    assert!(matches!(err, fool_cron::error::CronError::InvalidSchedule(_)));
}
...
async fn oc1_rejects_lazy_existing_jobs() {
    ...
    let err = svc.add_job("u1", req).await.unwrap_err();
    assert!(matches!(err, fool_cron::error::CronError::Conversation(_)));
}
...
async fn oc2b_rejects_existing_jobs_with_cross_user_conversation_code() {
    ...
    .unwrap();
    conv_repo
        .create(&fool_db::models::ConversationRow {
    ...
    assert!(matches!(
        err,
        fool_cron::error::CronError::CrossAccountReference(message)
...
async fn create_for_conversation_helper_fails_when_conversation_binding_fails() {
    ...
    assert!(matches!(err, fool_cron::error::CronError::Database(_)));
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("x-fool-user-id", "u1")
                .header("x-fool-conversation-id", "conv_1")
                .body(Body::from(
...
async fn conversation_cron_routes_reject_missing_headers_unclaimed_and_wrong_user() {
    ...
    assert!(body.contains("x-fool-user-id"));
    ...
    let wrong_user = app
        .oneshot(
            Request::builder()
                ...
                .uri("/api/internal/conversation-cron/create")
                .header("content-type", "application/json")
                .header("x-fool-user-id", "other_user")
                .header("x-fool-conversation-id", "conv_1")
                .body(Body::from(
                    serde_json::to_vec(&conversation_cron_request("hello")).unwrap(),
                ))
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = fool_db::UpdateCronJobParams {
...
async fn cd3c_on_conversation_delete_preserves_custom_workspace_on_jobs() {
    ...
    let custom_workspace = ensure_named_workspace_path(&format!("fool-cron-custom-workspace-{conversation_id}"));
    conv_repo.set_conversation_extra(
        &conversation_id,
        serde_json::json!({
            "workspace": custom_workspace,
