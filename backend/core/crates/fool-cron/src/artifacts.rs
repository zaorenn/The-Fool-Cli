...
use fool_api_types::{ConversationArtifactResponse, WebSocketMessage};
use fool_common::generate_id;
use fool_db::ConversationArtifactRow;
...
mod tests {
    ...
    fn builds_skill_suggest_response() {
        ...
        assert_eq!(response.kind, fool_api_types::ConversationArtifactKind::SkillSuggest);
        assert_eq!(response.status, fool_api_types::ConversationArtifactStatus::Pending);
    ...
    fn builds_cron_trigger_payload() {
        ...
        let response = artifact_response_from_row(&row).unwrap();
        assert_eq!(response.kind, fool_api_types::ConversationArtifactKind::CronTrigger);
