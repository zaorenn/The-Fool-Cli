...
impl CronService {
    ...
    async fn verify_conversation_helper_context(
        ...
        user_id: &str,
        conversation_id: &str,
    ) -> Result<fool_db::models::ConversationRow, CronError> {
        ...
        self.executor
            ...
            .await?
            .ok_or_else(|| {
                CronError::Conversation(fool_conversation::ConversationError::NotFound {
    ...
    async fn add_job_internal(
        ...
        validate_foolrs_agent_config(&resolved_agent_type, req.agent_config.as_ref())?;
    ...
    pub async fn update_job(
        ...
        if let Some(config_dto) = &req.agent_config {
            ...
            validate_foolrs_agent_config(&job.agent_type, Some(&config_dto))?;
    ...
    async fn resolve_new_job_agent_type(
        &self,
        user_id: &str,
        agent_config: Option<&fool_api_types::CronAgentConfigWriteDto>,
    ...
    async fn require_existing_conversation_scope(&self, user_id: &str, conversation_id: &str) -> Result<(), CronError> {
        if conversation_id.is_empty() {
            return Err(CronError::Conversation(
                fool_conversation::ConversationError::NotFound {
                    id: conversation_id.to_owned(),
                },
            ));
        }
    ...
    async fn build_agent_config_from_conversation(
        ...
        // the cached vendor-label fallback (`"foolrs"`) from sneaking back in
        // (Sentry ELECTRON-1HM).
        let model_resolved = fool_conversation::task_options::provider_model_from_conversation_row(row);
        ...
        let legacy_agent_label = if row.r#type == "foolrs" {
            Some("foolrs".to_owned())
        } else {
            model
        ...
        let backend = if row.r#type == "foolrs" {
            model
                .map(|value| value.provider_id.clone())
                .filter(|value| !value.is_empty())
                .or_else(|| get_string(&extra, &["backend"]))
                .or_else(|| assistant_backend.clone())
                .unwrap_or_else(|| "foolrs".to_owned())
        } else {
            assistant_backend
                .clone()
                .or_else(|| {
        ...
        let agent_config = fool_api_types::CronAgentConfigWriteDto {
            name: assistant_name
            ...
            model: (row.r#type == "foolrs").then(|| model.cloned()).flatten(),
            config_options: None,
    ...
    async fn build_cron_agent_config(
        ...
        config: fool_api_types::CronAgentConfigWriteDto,
...
impl fool_common::OnConversationDelete for CronService {
...
fn runtime_agent_type_for_backend(backend: &str) -> &'static str {
    if backend == "foolrs" { "foolrs" } else { "acp" }
}
...
fn normalize_model(
    ...
    if runtime_agent_type == "foolrs" && (model.provider_id.is_empty() || model.model.is_empty()) {
        return Err(CronError::InvalidAgentConfig(
            "foolrs cron jobs require agent_config.model.provider_id and agent_config.model.model".into(),
        ));
    }
...
fn validate_foolrs_agent_config(
    agent_type: &str,
    agent_config: Option<&fool_api_types::CronAgentConfigWriteDto>,
) -> Result<(), CronError> {
    if agent_type != "foolrs" {
        return Ok(());
    }
...
fn sanitize_agent_config_dto(
    mut config: fool_api_types::CronAgentConfigWriteDto,
) -> fool_api_types::CronAgentConfigWriteDto {
...
mod tests {
    ...
    // -- validate_foolrs_agent_config ----------------------------------------

    fn agent_cfg_dto(provider_id: &str) -> fool_api_types::CronAgentConfigWriteDto {
        fool_api_types::CronAgentConfigWriteDto {
            name: "provider".into(),
            cli_path: None,
    ...
    #[test]
    fn validate_foolrs_accepts_valid_config() {
        let cfg = agent_cfg_dto("4056cdea");
        assert!(validate_foolrs_agent_config("foolrs", Some(&cfg)).is_ok());
    }

    #[test]
    fn validate_foolrs_rejects_missing_config() {
        let err = validate_foolrs_agent_config("foolrs", None).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }

    #[test]
    fn validate_foolrs_rejects_empty_provider_id() {
        let cfg = agent_cfg_dto("");
        let err = validate_foolrs_agent_config("foolrs", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }

    #[test]
    fn validate_foolrs_rejects_whitespace_provider_id() {
        let cfg = agent_cfg_dto("   ");
        let err = validate_foolrs_agent_config("foolrs", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }
    ...
    #[test]
    fn validate_foolrs_ignores_non_foolrs_type() {
        // ACP / other types may legitimately omit agent_config or leave model empty.
        assert!(validate_foolrs_agent_config("acp", None).is_ok());
        let cfg = agent_cfg_dto("");
        assert!(validate_foolrs_agent_config("claude", Some(&cfg)).is_ok());
    }

    #[test]
    fn sanitize_agent_config_dto_clears_legacy_ids_when_assistant_id_present() {
        let config = fool_api_types::CronAgentConfigWriteDto {
            name: "Helper".into(),
            cli_path: None,
            assistant_id: Some("assistant-1".into()),
    ...
    fn sanitize_agent_config_dto_rejects_legacy_custom_agent_id_without_assistant_id() {
        let err = serde_json::from_value::<fool_api_types::CronAgentConfigWriteDto>(serde_json::json!({
            "name": "Helper",
