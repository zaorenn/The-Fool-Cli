...
| 30  | `isTeamCapableBackend(backend, caps)` 白名单 + ACP stdio 能力判断                                                                                                                |   ❌    |   ✅    | 后端硬编码 `["claude", "codex"]`（[tools.rs:167](../../../crates/aionui-team/src/mcp/tools.rs)），仅用于 `team_spawn_agent` 白名单。AionUi 白名单是 `{gemini, claude, codex, foolrs}` + 其他 backend 按 `cachedInitResults.capabilities.mcpCapabilities.stdio===true` 动态判定                                                                              | §3.1 "Team Guide 能力判断"                  |
