//! Integration tests for file-based MCP Agent adapters (Opencode, Foolrs, Fool).
//!
...
//!
//! For Fool, we use a mock repository since it reads from the DB.
...
use fool_mcp::{FoolAdapter, McpAgentAdapter, McpServerTransport};

// ===========================================================================
// Fool adapter (DB-backed)
// ===========================================================================

mod fool {
    use super::*;
    use fool_db::models::McpServerRow;
    use fool_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: &str,
            _s: &str,
            _lc: Option<fool_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = FoolAdapter::new(repo.clone());
...
// ===========================================================================
// Foolrs adapter (CLI + TOML-backed)
// ===========================================================================

// Note: Full lifecycle tests for Foolrs require the `foolrs` CLI to be
// installed (for `--config-path`). The unit tests in foolrs.rs thoroughly
// cover TOML parsing, serialization, and roundtrip behavior. Here we
// verify the public API surface.

mod foolrs {
    use super::*;
    use fool_mcp::FoolrsAdapter;

    #[test]
    fn source_is_foolrs() {
        assert_eq!(FoolrsAdapter.source(), McpSource::Foolrs);
    }

    #[test]
    fn trait_object_safety() {
        let adapter: Box<dyn McpAgentAdapter> = Box::new(FoolrsAdapter);
        assert_eq!(adapter.source(), McpSource::Foolrs);
    }
}
