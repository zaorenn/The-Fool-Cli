...
use fool_db::SqliteMcpServerRepository;
use fool_mcp::{DetectedServer, McpAgentAdapter, McpError, McpServerTransport, McpSyncService};
...
async fn make_service(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> McpSyncService {
    ...
    let repo: Arc<dyn fool_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
