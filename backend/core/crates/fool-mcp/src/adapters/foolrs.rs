...
const CLI_NAME: &str = "foolrs";

/// MCP Agent adapter for Foolrs.
///
/// Foolrs stores MCP configuration in a TOML config file. The config path
/// is obtained via `foolrs --config-path`.
///
/// # Config Format (TOML)
///
/// ```toml
...
/// ```
pub struct FoolrsAdapter;

#[async_trait::async_trait]
impl McpAgentAdapter for FoolrsAdapter {
...
/// Resolve the TOML config file path from the `foolrs` CLI.
///
/// Newer `foolrs` exposes this via the `config path` subcommand; older
...
async fn get_config_path() -> Result<String, McpError> {
    ...
    // Legacy fallback for `foolrs` versions predating the `config` subcommand.
    ...
    if path.is_empty() {
        return Err(McpError::AgentOperationFailed(
            "foolrs config path returned empty output".into(),
        ));
    }
    Ok(path)
}
