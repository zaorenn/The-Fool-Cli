...
use fool_db::{CreateMcpServerParams, IMcpServerRepository, UpdateMcpServerParams};
...
mod tests {
    use super::*;
    use fool_api_types::{ImportMcpServerRequest, McpTransport};
    use fool_common::{McpServerStatus, TimestampMs};
    use fool_db::models::McpServerRow;
    use fool_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![fool_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, fool_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(TEST_USER_ID, &created.id).await.unwrap();
        assert_eq!(updated.last_test_status, fool_common::McpServerStatus::Error);
