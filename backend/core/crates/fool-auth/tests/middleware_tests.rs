...
use fool_db::{IUserRepository, SqliteUserRepository, UserStatus, UserType, init_database_memory};
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", format!("fool-csrf-token={token}"))
...
async fn t12_2_post_with_mismatched_csrf_tokens_rejected() {
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", "fool-csrf-token=token_a")
...
#[tokio::test]
async fn auth_middleware_pro_rejects_local_user_token() {
    let jwt_service = Arc::new(JwtService::new("middleware_test_secret".into()));
    let token = jwt_service
        .sign_with_session_generation("system_default_user", "system_default_user", 0)
        .unwrap();
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteUserRepository::new(db.pool().clone()));
    let app = protected_auth_app_with_mode(jwt_service, repo as Arc<dyn IUserRepository>, AuthIdentityMode::Pro);
...
async fn t12_2_csrf_cookie_set_on_first_request() {
    ...
    assert!(set_cookie.contains("fool-csrf-token="));
...
fn t13_1_authorization_header_takes_priority() {
    ...
    headers.insert(header::COOKIE, "fool-session=cookie_tok".parse().unwrap());
    assert_eq!(
        fool_auth::extract_token_from_headers(&headers),
...
fn t13_2_cookie_fallback() {
    let mut headers = axum::http::HeaderMap::new();
    headers.insert(header::COOKIE, "fool-session=fallback_tok".parse().unwrap());
    assert_eq!(
        fool_auth::extract_token_from_headers(&headers),
...
fn t13_3_no_token_returns_none() {
    ...
    assert_eq!(fool_auth::extract_token_from_headers(&headers), None);
}
...
#[tokio::test]
async fn runtime_token_channel_pro_authenticates_external_user() {
    let jwt_service = Arc::new(JwtService::new("middleware_test_secret".into()));
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteUserRepository::new(db.pool().clone()));
    let user = repo
        .ensure_external_user(
            UserType::Pro,
            "ext-user-1",
            fool_db::ExternalUserProjection {
...
#[tokio::test]
async fn runtime_token_channel_pro_rejects_local_user_token_binding() {
    let jwt_service = Arc::new(JwtService::new("middleware_test_secret".into()));
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteUserRepository::new(db.pool().clone())) as Arc<dyn IUserRepository>;
    let app = identity_echo_app(
    ...
    let resp = app
        .oneshot(
            Request::post("/api/thing")
                .header("x-fool-runtime-token", "tok-1")
                .body(Body::empty())
                .unwrap(),
        )
        .await
