...
use fool_ai_agent::{AgentError, AgentSendError, AgentStreamEvent, IMockAgent, IWorkerTaskManager};
...
use fool_channel::message_service::ChannelMessageService;
...
use fool_conversation::skill_resolver::{ResolvedAgentSkill, SkillResolver};
use fool_db::models::AssistantSessionRow;
use fool_db::models::UpsertAssistantDefinitionParams;
use fool_db::{
...
#[tokio::test]
async fn send_to_agent_without_saved_binding_defaults_to_bare_foolrs_assistant() {
    let db = init_database_memory().await.unwrap();
    let pool = db.pool().clone();
    ...
    let session = AssistantSessionRow {
        id: "session-assisted-default-foolrs".to_owned(),
