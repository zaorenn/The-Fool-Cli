...
use fool_db::models::AssistantUserRow;
use fool_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn fool_db::IClientPreferenceRepository> =
        Arc::new(fool_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
async fn action_session_new() {
    ...
    match result {
        MessageResult::Action(resp) => {
            ...
            // With no client_preferences, defaults to "foolrs"
            assert!(text.contains("foolrs"));
        }
        _ => panic!("Expected Action result"),
    }
}
...
// fool-db sqlite_channel.rs::update_session_conversation_persists.
