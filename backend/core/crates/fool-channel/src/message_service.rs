...
use fool_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use fool_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use fool_common::{AgentType, ConversationSource};
...
impl ChannelMessageService {
    ...
    async fn create_conversation_for_session(
        ...
        // Top-level `model` is only accepted for foolrs; other types pass via `extra`.
        let top_level_model = if agent_type == AgentType::Foolrs {
            Some(model)
        } else {
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved variants default to Fool
        PluginType::Slack | PluginType::Discord => ConversationSource::Fool,
    }
}
...
mod tests {
    use super::*;
    use fool_ai_agent::protocol::events::{
    ...
    #[test]
    fn platform_to_source_reserved_defaults_to_fool() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::Fool);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::Fool);
    }
    ...
    fn parse_known_agent_types() {
        ...
        assert_eq!(parse_agent_type("foolrs").unwrap(), AgentType::Foolrs);
    }
    ...
    // ── model placement by agent_type (regression: non-foolrs must not
    //    use top-level model) ──────────────────────────────────────────
    ...
    #[test]
    fn foolrs_model_stays_at_top_level() {
        let agent_type = AgentType::Foolrs;
        let model = ProviderWithModel {
        ...
        assert!(top_level_model.is_some(), "foolrs must use top-level model");
    ...
    #[test]
    fn conv_name_telegram_foolrs() {
        let name = channel_conversation_name(PluginType::Telegram, "foolrs", None, Some("70880480"));
        assert_eq!(name, "tg-foolrs-70880480");
    }
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "foolrs", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-foolrs-70880480");
    }
}
