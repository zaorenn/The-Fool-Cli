...
const DEFAULT_AGENT_TYPE: &str = "foolrs";
...
/// Non-ACP agent types (foolrs, nanobot, remote, …) have `backend = None`.
#[derive(Debug, Clone)]
...
impl ChannelSettingsService {
    ...
    ///
    /// Falls back to `agent_type=foolrs, backend=None` when no config exists.
    ...
    async fn resolve_default_assistant_identity(&self, user_id: &str) -> Result<Option<String>, ChannelError> {
        ...
        let mut any_foolrs = None;
        for definition in &definitions {
            if self.effective_assistant_backend(user_id, definition, &overlays).await? == DEFAULT_AGENT_TYPE {
                any_foolrs = Some(definition);
                break;
            }
        }
        if let Some(definition) = any_foolrs {
            return Ok(Some(definition.assistant_id.clone()));
        }
    ...
    async fn effective_assistant_backend(
        &self,
        user_id: &str,
        definition: &fool_db::models::AssistantDefinitionRow,
        overlays: &[fool_db::models::AssistantOverlayRow],
...
fn backend_to_agent_type(backend: &str) -> String {
    match backend {
        "foolrs" | "foolrs-cli" => "foolrs".to_owned(),
...
mod tests {
    ...
    use fool_db::{IAssistantDefinitionRepository, IAssistantOverlayRepository};
    use std::sync::Mutex;
    ...
    #[test]
    fn foolrs_backends_map_to_foolrs() {
        assert_eq!(backend_to_agent_type("foolrs"), "foolrs");
        assert_eq!(backend_to_agent_type("foolrs-cli"), "foolrs");
    }
    ...
    async fn agent_config_returns_default_when_no_pref() {
        ...
        assert_eq!(config.agent_type, "foolrs");
    ...
    #[tokio::test]
    async fn agent_config_foolrs_has_no_backend() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"backend":"foolrs","name":"Fool CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn agent_config_reads_new_format_foolrs() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"agent_type":"foolrs","name":"Fool CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn get_assistant_setting_defaults_to_generated_foolrs_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
                make_definition("bare-foolrs", "foolrs"),
            ],
        });
        ...
        assert_eq!(setting.assistant_id.as_deref(), Some("bare-foolrs"));
        assert!(setting.custom_agent_id.is_none());
        assert!(setting.backend.is_none());
        assert!(setting.agent_type.is_none());
    ...
    #[tokio::test]
    async fn get_platform_settings_defaults_to_generated_foolrs_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
        ...
        assert_eq!(assistant.assistant_id.as_deref(), Some("bare-foolrs"));
        assert!(assistant.custom_agent_id.is_none());
        assert!(assistant.backend.is_none());
        assert!(assistant.agent_type.is_none());
