...
//!
//! These are promoted from `fool-team::mcp::bridge` so that downstream
//! crates (`fool-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference
//! the same shape without depending on `fool-team`.
...
pub const TEAM_MCP_SERVER_NAME: &str = "fool-team";
...
mod tests {
    ...
    fn deserialization_tolerates_unknown_fields() {
        ...
        let json = r#"{"team_id":"t-1","port":1,"token":"t","slot_id":"s","binary_path":"/usr/bin/foolcore","future_field":42}"#;
        let parsed: TeamMcpStdioConfig = serde_json::from_str(json).unwrap();
