...
use fool_common::{AgentType, TimestampMs};
...
pub enum AgentSource {
    /// Ships with the backend binary (no CLI install required — e.g. `foolrs`).
    Internal,
...
/// the frontend verbatim, and typed-decoded inside `fool-ai-agent`
...
pub struct AgentMetadata {
    ...
    /// Native mode id that The Fool's legacy `yolo` / `yoloNoSandbox`
