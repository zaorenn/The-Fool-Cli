...
mod tests {
    ...
    fn test_system_info_response_serialization() {
        let resp = SystemInfoResponse {
            cache_dir: "/home/user/.cache/fool".into(),
            work_dir: "/home/user/.local/share/fool".into(),
            log_dir: "/home/user/.local/state/fool/logs".into(),
            platform: "linux".into(),
            arch: "x64".into(),
        };
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["cache_dir"], "/home/user/.cache/fool");
        assert_eq!(json["work_dir"], "/home/user/.local/share/fool");
        assert_eq!(json["log_dir"], "/home/user/.local/state/fool/logs");
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "zaorenn/The-Fool-Cli"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("zaorenn/The-Fool-Cli"));
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/zaorenn/The-Fool-Cli/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "fool-2.0.0-darwin-arm64.dmg".into(),
                    url: "https://github.com/download/fool.dmg".into(),
                    size: 85_000_000,
        ...
        assert_eq!(json["latest"]["assets"][0]["name"], "fool-2.0.0-darwin-arm64.dmg");
