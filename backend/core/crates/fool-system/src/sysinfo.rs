use fool_api_types::SystemInfoResponse;
...
/// Resolve the cache directory for The Fool.
///
/// Priority: `FOOL_CACHE_DIR` env → `dirs::cache_dir()/fool`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("FOOL_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("fool").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for The Fool.
///
/// Priority: `FOOL_WORK_DIR` env → `dirs::data_dir()/fool`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("FOOL_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for The Fool.
///
/// Priority: `FOOL_LOG_DIR` env →
///   macOS: `~/Library/Logs/fool`
///   Linux: `dirs::state_dir()/fool/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/fool/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("FOOL_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/fool").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("fool/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/fool/logs
    dirs::data_dir()
        .map(|p| p.join("fool/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "fool".
        let dir = resolve_cache_dir();
        assert!(dir.contains("fool"), "cache_dir should contain 'fool': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("fool"), "work_dir should contain 'fool': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("fool"), "log_dir should contain 'fool': {dir}");
    }
}
