use fool_api_types::{GitHubReleaseAsset, UpdateCheckRequest, UpdateCheckResult, UpdateReleaseInfo};
use serde::Deserialize;

use crate::error::SystemError;

/// Where releases are published. Must match `PRODUCT_REPO` in the desktop
/// `common/brand.ts` — the two update paths have to agree on one repository, or
/// the app offers a version it cannot download.
const DEFAULT_REPO: &str = "zaorenn/The-Fool-Cli";
...
fn resolve_repo(from_request: Option<&str>) -> String {
    ...
    if let Ok(v) = std::env::var("FOOL_GITHUB_REPO")
        && !v.is_empty()
    {
        return v;
    }
