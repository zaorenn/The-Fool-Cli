...
async fn seed_legacy_assistant_identity(pool: &sqlx::SqlitePool) {
    ...
    for (id, backend, agent_type, name, source, sort_order) in [
        ("agent-foolrs", "", "foolrs", "Aion CLI", "internal", 100),
    ...
    for (definition_id, assistant_key, agent_backend, source_ref) in [
        ("def-foolrs", "aionui-assistant", "foolrs", "aionui-assistant"),
    ...
    for (conversation_id, name, agent_type, extra) in [
        ("conv_foolrs", "Aion cron", "foolrs", r#"{"workspace":"/tmp/foolrs"}"#),
        (
            "conv_snapshot",
            "Snapshot cron",
            "acp",
...
#[tokio::test]
async fn migration_015_populates_foolrs_catalog_by_agent_type() {
    ...
    sqlx::query(
        "INSERT INTO agent_metadata (
            id, name, backend, command, agent_type, enabled, agent_source, sort_order, created_at, updated_at
         ) VALUES ('agent-foolrs', 'Aion CLI', NULL, '', 'foolrs', 1, 'internal', 100, 1, 1)",
    )
    .execute(&pool)
    .await
    .unwrap();
...
async fn migration_013_normalizes_legacy_cron_agent_identity() {
    ...
    let foolrs = sqlx::query(
        "SELECT
            ...
            enabled
         FROM cron_jobs WHERE id = 'cron_foolrs'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(foolrs.get::<String, _>("assistant_id"), "aionui-assistant");
    assert_eq!(foolrs.get::<String, _>("provider_id"), "provider-1");
    assert_eq!(foolrs.get::<String, _>("model"), "gpt-5");
    assert_eq!(foolrs.get::<i64, _>("enabled"), 1);
