...
2. 主要涉及个人用户的 IM 通信工具（Telegram、Slack 等）
3. 打造 7×24 小时个人终端助手
4. **MVP 阶段优先实现 Telegram 接入，仅支持 Gemini Agent**
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐            │
│  │PluginManager│ │ActionRouter │ │SessionManager│           │
│  └─────────────┘ └─────────────┘ └─────────────┘            │
│                                                              │
...
```
入站流程:
  平台消息 → Plugin(转换) → Gateway(路由) → Agent(处理)

出站流程:
  Agent 响应 → Gateway(路由) → Plugin(转换) → 平台发送
```
...
| 接口方法                | 方向             | 说明           |
| ----------------------- | ---------------- | -------------- |
| `initialize(config)`    | Gateway → Plugin | 初始化插件配置 |
| `start()`               | Gateway → Plugin | 启动平台连接   |
| `stop()`                | Gateway → Plugin | 停止平台连接   |
| `onMessage(handler)`    | Gateway → Plugin | 注册消息回调   |
| `sendMessage(...)`      | Gateway → Plugin | 发送消息到平台 |
| `sendMessageChunk(...)` | Gateway → Plugin | 发送流式消息块 |
| `getStatus()`           | Gateway → Plugin | 获取插件状态   |
...
| ------------------- | ------------------------------------- |
| `id`                | 系统生成的唯一 ID                     |
| `platform`          | 来源平台（telegram/slack/discord）    |
| `platformMessageId` | 平台原始消息 ID                       |
| `user`              | 用户信息（id、username、displayName） |
| `chat`              | 聊天信息（id、type、title）           |
| `content`           | 消息内容（text/image/file/command）   |
| `timestamp`         | 时间戳                                |
...
| `text`             | 文本内容                            |
| `parseMode`        | 解析模式（plain/markdown/html）     |
| `buttons`          | 按钮组（可选）                      |
| `replyToMessageId` | 回复的消息 ID（可选）               |

### 3.5 扩展新平台步骤

1. 创建 `plugins/[platform]/` 目录
2. 实现 `[Platform]Plugin` 继承 `BasePlugin`
3. 实现 `[Platform]Adapter` 处理消息转换
4. 在 `PluginManager` 注册新插件
5. 添加设置页面
6. 添加 i18n 翻译
...
│      // 同时发送到 IPC 和 AssistantEventBus                     │
│      ipcBridge.geminiConversation.responseStream.emit(data);    │
│      assistantEventBus.emitAgentMessage(conversationId, data);  │
│    });                                                           │
│  }                                                               │
└───────────────────────────┬─────────────────────────────────────┘
                            │
...
│  initialize() {                                                  │
│    // 服务初始化时注册全局事件监听                               │
│    assistantEventBus.onAgentMessage(this.handleAgentMessage);   │
│  }                                                               │
│                                                                  │
...
│        const outgoing = convertTMessageToOutgoing(message);      │
...
│  }                                                               │
│                                                                  │
│  convertTMessageToOutgoing(message) {                            │
...
2. **全局事件总线解耦**
   - `AssistantMessageService` 不直接与 Agent Task 交互
   - 通过 `AssistantEventBus` 全局事件总线解耦
...
src/assistant/
├── core/                     # 核心模块
│   ├── AssistantGateway      # 消息网关
│   ├── PluginManager         # 插件管理器
│   ├── SessionManager        # 会话管理
│   └── MessageRouter         # 消息路由
│
├── actions/                  # Action 处理（平台无关）
│   ├── types                 # Action/Response 类型定义
│   ├── ActionRouter          # Action 分发器
│   └── handlers/
│       ├── SessionHandler    # 会话相关 Action
│       ├── SettingsHandler   # 设置相关 Action
│       └── HelpHandler       # 帮助 Action
│
├── plugins/                  # 插件目录
│   ├── base/
│   │   └── BasePlugin        # 插件抽象基类
│   ├── telegram/
│   │   ├── TelegramPlugin    # Telegram 插件
│   │   ├── TelegramAdapter   # 消息适配器
│   │   └── TelegramPairing   # Telegram 配对处理（平台特有）
│   └── types/                # 类型定义
│
├── security/                 # 安全模块（核心服务，平台无关）
│   ├── PairingService        # 配对核心逻辑（生成码、验证、过期）
│   ├── RateLimiter           # 速率限制
│   └── AllowlistManager      # 白名单管理
│   # 注: 各平台 Plugin 调用 PairingService，自行处理展示逻辑
│
└── types/                    # 类型定义
```
...
| `@slack/bolt` | Slack Bot（后续）   | 官方 SDK                |
| `discord.js`  | Discord Bot（后续） | 官方 SDK                |
...
- [ ] Telegram Bot Token 配置和验证
- [ ] Bot 启动/停止控制
- [ ] 配对码生成和本地批准流程
- [ ] 已授权用户管理
- [ ] 按钮交互（Reply Keyboard + Inline Keyboard）
- [ ] 与 Gemini Agent 对话
- [ ] 新建会话功能
- [ ] 流式消息响应
...
| ----------- | ------------------------ |
| **Phase 2** | 多会话管理、会话切换     |
| **Phase 3** | Agent 切换（ACP、Codex） |
| **Phase 4** | Model 动态切换           |
| **Phase 5** | Slack 平台接入           |
| **Phase 6** | 会话与 AionUi 同步       |
| **Phase 7** | Headless 独立服务模式    |
