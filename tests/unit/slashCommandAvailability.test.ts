...
describe('isSlashCommandListEnabled', () => {
  it('returns true for non-codex conversations', () => {
    expect(isSlashCommandListEnabled({ conversation_type: 'gemini' })).toBe(true);
    expect(isSlashCommandListEnabled({ conversation_type: 'acp' })).toBe(true);
  });

  it('returns false for codex before session_active', () => {
    expect(isSlashCommandListEnabled({ conversation_type: 'codex', codexStatus: null })).toBe(false);
    expect(isSlashCommandListEnabled({ conversation_type: 'codex', codexStatus: 'connecting' })).toBe(false);
    expect(isSlashCommandListEnabled({ conversation_type: 'codex', codexStatus: 'connected' })).toBe(false);
  });
  ...
  it('handles edge cases', () => {
    ...
    expect(isSlashCommandListEnabled({ conversation_type: undefined })).toBe(true);

    // Unknown conversation types are enabled
    expect(isSlashCommandListEnabled({ conversation_type: 'unknown-type' })).toBe(true);

    // Empty string codexStatus is not session_active
    expect(isSlashCommandListEnabled({ conversation_type: 'codex', codexStatus: '' })).toBe(false);
  });
});
