...
describe('configMigrationIntegration', () => {
  ...
  it('runs migrations when database version is outdated', async () => {
    const result = await runLegacyDatabaseMigrations('/test/fool.db');
  ...
  it('closes driver after migration completes', async () => {
    await runLegacyDatabaseMigrations('/test/fool.db');
  ...
  it('closes driver even if migration throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/fool.db')).rejects.toThrow('Migration failed');
    expect(mockDriver.close).toHaveBeenCalled();
  });
});
