...
describe('extractWebSocketToken – cookie parsing with special characters', () => {
  let TokenMiddleware: typeof import('@process/webserver/auth/middleware/TokenMiddleware').TokenMiddleware;

  beforeEach(async () => {
    vi.resetModules();
    const mod = await import('@process/webserver/auth/middleware/TokenMiddleware');
