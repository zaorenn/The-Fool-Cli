...
describe("fsBridge skills functionality", () => {
      ...
      return {
        default: {
          access: vi.fn(async (filePath: string) => {
            ...
            throw new Error(
              `ENOENT: no such file or directory, access '${fp}'`,
            );
          }),
          readFile: vi.fn(async (filePath: string) => {
            ...
            if (fp in mockFsStore) {
              ...
              throw new Error(
                `EISDIR: illegal operation on a directory, read '${fp}'`,
              );
            }
          ...
          writeFile: vi.fn(async (filePath: string, content: string) => {
            const fp = resolvePath(filePath);
            if (fp.endsWith("custom_external_skill_paths.json")) {
              mockCustomExternalPaths = JSON.stringify(content)
          ...
          readdir: vi.fn(
            async (dirPath: string, options?: { withFileTypes?: boolean }) => {
              ...
              if (!(dp in mockFsStore))
                throw new Error(
                  `ENOENT: no such file or directory, scandir '${dp}'`,
                );
                ...
                if (key !== dp && key.startsWith(dp + path.sep)) {
                  ...
                  if (!relativePath.includes(path.sep)) {
                    entries.push({
                      name: relativePath,
                      isDirectory: () =>
                        !!mockFsStore[key].isDirectory &&
                        !mockFsStore[key].isSymlink,
                      isFile: () =>
                        !mockFsStore[key].isDirectory &&
                        !mockFsStore[key].isSymlink,
          ...
          copyFile: vi.fn(async (src: string, dest: string) => {
            ...
            if (!(s in mockFsStore))
              throw new Error(`ENOENT: src not found '${s}'`);
          ...
          symlink: vi.fn(async (src: string, dest: string) => {
            ...
            mockFsStore[d] = {
              isSymlink: true,
              target: s,
              isDirectory: mockFsStore[s]?.isDirectory,
            };
          }),
        ...
        return {
          ...
          invoke: vi.fn((payload) =>
            handlers[channel]
              ...
              : Promise.reject(`No handler for ${channel}`),
          ),
          emit: vi.fn(),
        };
      };
      ...
      return {
        ipcBridge: {
          fs: {
            ...
            detectCommonSkillPaths: createCommandMock(
              "detect-common-skill-paths",
            ),
            detectAndCountExternalSkills: createCommandMock(
              "detect-and-count-external-skills",
            ),
            importSkillWithSymlink: createCommandMock(
              "import-skill-with-symlink",
            ),
            deleteSkill: createCommandMock("delete-skill"),
            getSkillPaths: createCommandMock("get-skill-paths"),
            exportSkillWithSymlink: createCommandMock(
              "export-skill-with-symlink",
            ),
            getCustomExternalPaths: createCommandMock(
              "get-custom-external-paths",
            ),
            addCustomExternalPath: createCommandMock(
              "add-custom-external-path",
            ),
            removeCustomExternalPath: createCommandMock(
    ...
    const mockCmd =
      ...
      (
        Object.values(ipcMod.ipcBridge.fs).find(
          (v: any) =>
            v.provider?.mock?.calls?.length && v.provider.mock.calls[0][0],
        ) as any
      )?.invoke; // Fallback
  ...
  describe("listAvailableSkills", () => {
    it("should correctly parse SKILL.md and distinguish builtin vs custom", async () => {
      ...
      // builtinBase matches getBuiltinSkillsDir() mock → /mock/userData/config/skills/_builtin
      ...
      mockFsStore[path.join(builtinBase, "test-skill-1")] = {
        isDirectory: true,
      };
      ...
      mockFsStore[path.join(userBase, "duplicate-skill")] = {
        isDirectory: true,
      };
  ...
  describe("detectAndCountExternalSkills", () => {
    it("should detect direct skills and nested skill packs from common and custom paths", async () => {
      ...
      mockCustomExternalPaths = [
        { name: "My Custom Path", path: customSrcPath },
      ];
      const workBase = path.resolve("/mock/work");
      mockFsStore[workBase] = { isDirectory: true };
      mockFsStore[path.join(workBase, "custom_external_skill_paths.json")] = {
        isDirectory: false,
      }; // Let it use mockCustomExternalPaths
      ...
      mockFsStore[path.join(geminiPath, "direct-skill")] = {
        isDirectory: true,
      };
      mockFsStore[path.join(geminiPath, "direct-skill", "SKILL.md")] = {
        content: yamlDirect1,
        isDirectory: false,
      };
      ...
      mockFsStore[path.join(geminiPath, "pack-skill", "skills")] = {
        isDirectory: true,
      };
      mockFsStore[
        path.join(geminiPath, "pack-skill", "skills", "nested-skill")
      ] = { isDirectory: true };
      mockFsStore[
        path.join(
          geminiPath,
      ...
      mockFsStore[path.join(customSrcPath, "custom-ext-skill")] = {
        isDirectory: true,
      };
      ...
      expect(
        geminiGroup.skills.some((s: any) => s.name === "DirectGemini"),
      ).toBe(true);
      expect(
        geminiGroup.skills.some((s: any) => s.name === "NestedGemini"),
      ).toBe(true);

      const customGroup = result.data.find((d: any) =>
        d.source.startsWith("custom-"),
      );
  ...
  describe("importSkillWithSymlink", () => {
    it("should successfully copy a valid skill directory to user config and fail if missing SKILL.md", async () => {
      ...
      mockFsStore[path.join(srcPath, "extra.txt")] = {
        content: "hello",
        isDirectory: false,
      };
  ...
  describe("deleteSkill", () => {
    it("should delete existing skill from user directory", async () => {
      ...
      mockFsStore[path.join(skillPath, "SKILL.md")] = {
        content: "",
        isDirectory: false,
      };
  ...
  describe("scanForSkills", () => {
    it("should find skills nested in subdirectories or directly at the root", async () => {
      ...
      mockFsStore[path.join(rootSkillDir, "SKILL.md")] = {
        content: "---\nname: RootSkill\n---",
        isDirectory: false,
      };
