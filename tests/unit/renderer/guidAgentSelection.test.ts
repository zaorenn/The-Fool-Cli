...
describe('guid assistant selection helpers', () => {
  const assistants: Assistant[] = [
    ...
    assistant({ id: 'bare-foolrs', source: 'generated', runtimeKey: 'foolrs', sort_order: 10 }),
  ...
  it('does not accept legacy backend keys as assistant selection ids', () => {
    ...
    expect(resolveAssistantSelectionKey('foolrs', assistants)).toBeUndefined();
  });

  it('defaults to the generated foolrs assistant when available', () => {
    expect(pickDefaultAssistantSelectionKey(assistants)).toBe('bare-foolrs');
  });
...
function assistant(
  ...
  const isFoolrs = overrides.runtimeKey === 'foolrs';
  return {
    id: overrides.id,
