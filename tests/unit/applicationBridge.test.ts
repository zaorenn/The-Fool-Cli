...
describe("CDP configuration functions", () => {
  ...
  it("restart handler calls workerTaskManager.clear() via injected dependency", async () => {
    ...
    vi.doMock("@/common/platform", () => ({
      getPlatformServices: () => ({
        paths: {
          ...
          getLogsDir: () => "/tmp/aionui-logs",
          getName: () => "AionUI",
          getVersion: () => "0.0.0",
        },
        worker: { fork: vi.fn() },
        power: { preventSleep: () => null, allowSleep: vi.fn() },
    ...
    vi.resetModules();
    const { initApplicationBridge } =
      await import("../../src/process/bridge/applicationBridge");
    const taskMgr = makeTaskManager();
