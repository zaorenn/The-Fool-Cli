...
function seedRuntimeKey(
  ...
  const managedResourcesDir = join(resourcesDir, 'bundled-foolcore', runtimeKey, 'managed-resources');
  mkdirSync(join(resourcesDir, 'bundled-foolcore', runtimeKey), { recursive: true });
  writeFile(join(resourcesDir, 'bundled-foolcore', runtimeKey, platform === 'win32' ? 'foolcore.exe' : 'foolcore'));
  writeJson(join(resourcesDir, 'bundled-foolcore', runtimeKey, 'manifest.json'), { platform, arch });
...
describe('verifyBundledFoolcoreResources', () => {
  ...
  it('fails when managed resources contract is missing', () => {
    ...
    expect(result.missing).toContain('bundled-foolcore/win32-x64/managed-resources/manifest.json');
  ...
  it('reports bundle manifest platform and architecture mismatches', () => {
    writeJson(join(resourcesDir, 'bundled-foolcore', 'win32-x64', 'manifest.json'), {
      platform: 'darwin',
      arch: 'arm64',
    });
    ...
    expect(result.missing).toContain('bundled-foolcore/win32-x64/manifest.json<platform:win32>');
    expect(result.missing).toContain('bundled-foolcore/win32-x64/manifest.json<arch:x64>');
    expect(result.failures).toContainEqual(
      expect.objectContaining({
  ...
  it('passes with the Windows arm64 CLI layout', () => {
    ...
    expect(result.checked).toContain(
      'bundled-foolcore/win32-arm64/managed-resources/cli/codex/0.144.6/win32-arm64/vendor/aarch64-pc-windows-msvc/bin/codex.exe'
    );
  });
  ...
  it('passes for non-Windows node runtime layout', () => {
    ...
    expect(result.failures).toEqual([]);
    expect(result.checked).toContain(
      'bundled-foolcore/darwin-arm64/managed-resources/node/node-v24.11.0-darwin-arm64/bin/node'
    );
    expect(result.checked).toContain(
      'bundled-foolcore/darwin-arm64/managed-resources/cli/claude/2.1.215/darwin-arm64/claude'
    );
  });
  ...
  it('reports missing non-Windows managed node runtime executable', () => {
    ...
    expect(result.missing).toContain(
      'bundled-foolcore/linux-x64/managed-resources/node/node-v24.11.0-linux-x64/bin/node'
    );
    expect(result.failures).toContainEqual(
      expect.objectContaining({
  ...
  it('fails when the pinned codex version directory is absent', () => {
    ...
    expect(result.missing).toContain(
      'bundled-foolcore/win32-x64/managed-resources/cli/codex/0.144.6/win32-x64/vendor/x86_64-pc-windows-msvc/bin/codex.exe'
    );
  });
  ...
  it('fails when contract node root points to the required version but only a wrong node directory exists', () => {
    ...
    expect(result.missing).toContain(
      'bundled-foolcore/win32-x64/managed-resources/node/node-v24.11.0-win-x64/node.exe'
    );
  });
  ...
  it('ignores unknown contract fields but rejects duplicate cli names', () => {
    ...
    expect(result.missing).toContain('bundled-foolcore/win32-x64/managed-resources/manifest.json<contract_failure>');
  });
