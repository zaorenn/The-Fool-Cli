...
const scriptPath = 'resources/windows/support/verify-bundled-foolcore-install.ps1';
...
describe('Windows bundled foolcore install verifier', () => {
  ...
  runOnWindows('fails an old-version-only Codex CLI install directory', () => {
    ...
    try {
      writeFile(join(installDir, 'resources', 'bundled-foolcore', 'win32-x64', 'foolcore.exe'), 'x');
      writeJson(join(installDir, 'resources', 'bundled-foolcore', 'win32-x64', 'manifest.json'), {
        platform: 'win32',
        arch: 'x64',
      });
