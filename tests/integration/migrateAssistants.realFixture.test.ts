 ...
 * foolcore binary using the user-provided fixtures
 * (`/Users/zhoukai/Downloads/fool-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `fool-config.txt` exactly as ConfigStorage would.
 *   2. Stage `Archive/*.md` as `<userData>/config/assistants/<id>.<locale>.md`.
 *   3. Spawn a real foolcore bound to a throw-away data-dir.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/fool-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.FOOL_BACKEND_BINARY,
    path.join(os.homedir(), '.cargo', 'bin', 'foolcore'),
    path.resolve(__dirname, '../../../The Fool Core/target/debug/foolcore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('foolcore binary not found (set FOOL_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `fool-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'fool-migrate-fixture-'));
