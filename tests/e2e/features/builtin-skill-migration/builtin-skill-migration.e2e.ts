 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'fool-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'fool-skills';
...
function resolveBackendBinary(): string {
  const candidates = [process.env.FOOL_BACKEND_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'foolcore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('foolcore binary not found. Set FOOL_BACKEND_BINARY or install to ~/.cargo/bin/foolcore.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fool-e2e-s7-export-'));
    try {
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      delete parentEnv.FOOL_EXTENSIONS_PATH;
      delete parentEnv.FOOL_EXTENSION_STATES_FILE;
      delete parentEnv.FOOL_E2E_TEST;
      delete parentEnv.FOOL_CDP_PORT;
      delete parentEnv.FOOL_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fool-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.fool-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.fool-config', 'builtin-skills'),
        path.join(os.homedir(), '.fool-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.fool-config*'
