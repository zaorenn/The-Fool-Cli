...
**Critical bottleneck**: `workers: 1` enforced + shared `fool.db` → parallel execution impossible without refactoring.
...
1. Extend `FOOL_E2E_TEST` to include worker ID:
   ```typescript
   FOOL_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.FOOL_E2E_TEST_WORKER_ID || '0';
   return `TheFool-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/TheFool-E2E-Worker-0/config/fool.db`
   - `~/Library/Application Support/TheFool-E2E-Worker-1/config/fool.db`
...
**Complexity**: High (requires main process to read `FOOL_DATABASE_PATH`, conflicts with userData convention)
