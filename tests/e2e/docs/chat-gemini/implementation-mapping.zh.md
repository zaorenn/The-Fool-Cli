...
基于 2026-04-22 的 E2E 收尾执行结果，以下用例因下层实现/Helper 限制暂时跳过：
...
### 2026-04-22 修复（TC-G-03 / TC-G-10 / TC-G-11 / TC-G-12）

**根因**：`createGeminiConversationViaBridge` 此前向 `conversation.create` 传递 `model: 'auto'`（字符串），而 `GeminiAgentManager.createBootstrap()` 通过 `getProviderAuthType(this.model)` 读取 `model.platform` 选择 AuthType。字符串模型的 `.platform` 为 `undefined`，auth 回退到 `USE_OPENAI`，Gemini worker 始终无法进入可发送状态——所有桥接驱动的 Gemini 对话首条 `chat.send.message` 挂起，`waitForGeminiReply` 在 90s 后超时。

**修复**：

1. `tests/e2e/helpers/chatGemini.ts`：`createGeminiConversationViaBridge` 新增 `resolveGeminiProviderForBridge` 辅助——优先使用本地配置的 Gemini provider，若无则回落到 `GEMINI_GOOGLE_AUTH_PLACEHOLDER`（`platform: 'gemini-with-google-auth'`，读取 `~/.gemini/oauth_creds.json`）。调用方现在传递完整的 `TProviderWithModel` 对象而非字符串。
2. `sendGeminiMessage` 在 `chat.send.message` 之前先调用 `conversation.warmup`，镜像 sendbox 组件挂载时的预热行为，确保 worker fork / bootstrap 完成后再发消息。
3. TC-G-10 / TC-G-11 / TC-G-12 由 UI 驱动（`attachGeminiFolder` + `uploadGeminiFiles`）改为桥接驱动（`createGeminiConversationViaBridge({ workspace, sessionMode, provider })` + `sendGeminiMessage({ files })`），绕开被冻结的 `electronAPI.dialog.showOpen` mock 失效问题。
4. TC-G-03 原有断言 `expect(extra.workspace).toBeUndefined()` 修正为 `toMatch(/gemini-temp-\d+/)`（Gemini agent 未传 workspace 时会自动创建，参见 `src/process/utils/initAgent.ts`）；去掉了 `userContent.content` 中包含 `'test.txt'` 文件名的错误断言（文件仅通过 `copyFilesToDirectory` 复制到 workspace，不写入消息 content.files）。

### 遗留建议

1. **桥接层多轮 AI 回复集成**：TC-G-07/08/09 在第二轮 `sendGeminiMessage` 仍会超时。建议在 `sendGeminiMessage` helper 中改为 UI 驱动（填入 input → 点击 send 按钮），或给 `GeminiAgentManager.sendMessage` 的 `silent: true` 路径加一条专门给 E2E 用的显式注入会话的接口。
2. **TC-G-02 二次验证**：TC-G-02 与 TC-G-10 同样使用桥接 + workspace，但 TC-G-02 提示词是 `"List files in the workspace."`（依赖 gemini workspace 列目录能力）——可能触发额外工具调用导致 reply 流收尾异常。建议排查后逐步解封。
...
| `<COMMIT4> test(e2e): unskip TC-G-03/10/11/12 via provider-object bridge path + warmup` | TC-G-03/10/11/12                                | 桥接层 `createGeminiConversationViaBridge` 传入完整 provider 对象 + 预调 `conversation.warmup`，解决 worker bootstrap 卡住的问题，4 个用例转为正常执行。                                               |
